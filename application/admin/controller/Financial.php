...
class Financial extends BaseController {
    ...
    public function applyShopAccountWithdraw() {
        ...
        if (request()->isAjax()) {
            ...
            if($witndraw_type['value']['withdraw_message']){
                $withdraw_message = explode(',',$witndraw_type['value']['withdraw_message']);//1银行卡(自动提现) 2微信 3支付宝 4银行卡(手动提现)
                if(in_array(1,$withdraw_message)){
                    $this->assign('bank_type',1);
                }
                if(in_array(2,$withdraw_message)){
    ...
    public function ShopAccountRecordCount() {
        if (request()->isAjax()) {
            ...
            $list = $shop->getShopAccountRecordsList($pageindex, PAGESIZE, $condition, 'create_time desc,account_type desc');
            return [
                "list" => $list,
                "count" => $count
            ];
