...
class Index extends BaseController {
    ...
    public function __construct() {
        ...
        $list['goods_set'] = $goodsSer->getGoodsDetailByCondition(['shop_id' => $this->instance_id,'website_id'=>$this->website_id,'state'=>1]);
        if($list['goods_set']){
            $this->assign("goods_set",1);
        }
        $customSer =  new \data\service\Customtemplate();
        $template_id = $customSer->getCustomTemplateInfo(['shop_id' => $this->instance_id,'website_id'=>$this->website_id,'type'=>2,'in_use'=>1],'id');
        $this->assign("custom_shop_set",$template_id ? 1 : 0);
    }
    ...
    public function preview()
    {
        ...
        $pcportStatus = getAddons('pcport', $this->website_id);
        $miniprogramStatus = getAddons('miniprogram', $this->website_id);
        $this->assign('pcportStatus', $pcportStatus);
        $this->assign('miniprogramStatus', $miniprogramStatus);
    ...
    public function getUserInfo() {
        $user_info = $this->user->getUserDetail($this->uid);
        return $user_info;
    }
