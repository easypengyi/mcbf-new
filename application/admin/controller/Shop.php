...
class Shop extends BaseController {
    ...
    public function shopConfig() {
        ...
        if (request()->isAjax()) {
            ...
            $order_mobile = request()->post('order_mobile','');//订单通知手机
            ...
            if ($retval && $order_mobile) {
                $param = [
                    'website_id' => $this->website_id,
                    'instance_id' => $this->instance_id,
                    'key' => 'MOBILEMESSAGE',
                    'value' => json_encode(['order_mobile' => $order_mobile]),
                    'is_use' => 1
                ];
                $configSer = new ConfigSer();
                $configSer->setConfigOne($param);
                $this->addUserLog('店铺基础设置', $shop_id . '-' . $shop_name);
            }
            return AjaxReturn($retval);
        }
        $userSer = new UserSer();
        $order_mobile = $userSer->getWebsiteOrShopOrderMobile($this->website_id, $this->instance_id);
        $this->assign('order_mobile', $order_mobile);
