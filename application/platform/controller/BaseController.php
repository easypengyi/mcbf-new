...
class BaseController extends Controller
{
    ...
    public function init()
    {
        ...
        if((getAddons('areabonus', $this->website_id) || getAddons('teambonus', $this->website_id) || getAddons('globalbonus', $this->website_id)) && !file_exists('is_change_bonus') && !request()->isAjax()){
            file_put_contents('is_change_bonus', json_encode(['time' => time(), 'content' => '遷移分紅數據'], true));
            $this->setShFile();
            exec('sh '.ROOT_PATH.'taskRunnerBonus.sh > /dev/null 2>&1 &');
        }
        ...
        if ($check_auth) {
            ...
            if (! request()->isAjax()) {
                ...
                $this->checkHasAddons();
                $user_info = $this->user->getUserInfo('', 'user_tel');
                ...
                $this->assign("username", $user_info['user_tel']);
                ...
                $groupInfo = $this->user->getAdminUserInfo(['uid'=>$this->uid]);
    ...
    public function checkHasAddons(){
        ...
        $systemformExist = getAddons('systemform', $this->website_id, 0, true);
        $this->assign('systemformExist', $systemformExist);
        $scheduleExist = getAddons('schedule', $this->website_id, 0, true);
    ...
    private function setShFile(){
        ...
        $web_info = $this->website->getWebSiteInfo();
        $realm_ip = $http.$web_info['realm_ip'];
        $contents = file_get_contents('taskRunnerBonus.sh');
        $contents = str_replace('$domain',$realm_ip, $contents);
        $contents = str_replace('$path',ROOT_PATH, $contents);
        file_put_contents('taskRunnerBonus.sh', $contents);
    }
}
