...
use addons\systemform\model\VslSystemFormModel;
...
//            }
            ...
            foreach ($result['data'] as $k => $v) {
                ...
                $result['data'][$k]['goods_name'] = htmlspecialchars(htmlspecialchars_decode($v['goods_name']));
...
//        $userGroupRes = $userGroup->getSystemUserGroupList(1, 0, ['website_id' => $this->website_id, 'instance_id' => $this->instance_id]);
        ...
        //判断是否有计时/次商品
        $caltimegoods_module_id = $addons_model->Query(['name' => 'caltimegoods'],'module_id')[0];
        if(empty($caltimegoods_module_id)) {
            $this->assign('have_caltimegoods', 0);
        }else{

                $this->assign('have_caltimegoods', 1);
            
        }
        //判断是否有电子卡密应用
        if(getAddons('electroncard',$this->website_id)) {
            $this->assign('electroncard', 1);
            //获取所有的卡密库
            ...
            $this->assign('electroncard', 0);
        }
        //获取分红等级列表
		if(getAddons('bonus',$this->website_id)){
			$agent_level = new VslAgentLevelModel();
			$agent_cond['from_type'] = 3;
			$agent_cond['website_id'] = $this->website_id;
			$agent_level_list = $agent_level->getQuery($agent_cond, '', '');
        ...
        //此处获取自定义表单列表
        $systemform_list = [];
        if(getAddons('systemform',$this->website_id)){ 
            //获取所有的自定义表单--》即区别于旧的自定义表单的新表单
            $systemform_mdl = new VslSystemFormModel();
            $systemform_list = $systemform_mdl->getQuery(['website_id' => $this->website_id],'id,name','id asc');
            $this->assign('systemform_list', $systemform_list);
        }
        //编辑
        if ($goodsId > 0) {
            $this->assign("goodsid", $goodsId);
            ...
            $coupon_type_id_array = [];
            $gift_voucher_id_array = [];
            $package_voucher_id_array = [];
            $coupon_type_id_array_num = [];
            $gift_voucher_id_array_num = [];
            $package_voucher_id_array_num = [];
            //列表信息
            $coupon_type_id_array_list = [];
            $gift_type_id_array_list = [];
            $package_type_id_array_list = [];
            #coupon_type  gift_voucher   package_voucher
            if($goods_info['coupon_type_id_array']){
                $coupon_type_id_array = unserialize($goods_info['coupon_type_id_array']);
            }
            if($goods_info['gift_voucher_id_array']){
                $gift_voucher_id_array = unserialize($goods_info['gift_voucher_id_array']);
            }
            if($goods_info['package_voucher_id_array']){
                $package_voucher_id_array = unserialize($goods_info['package_voucher_id_array']);
            }
            if($goods_info['coupon_type_id_array_num']){
                $coupon_type_id_array_num = unserialize($goods_info['coupon_type_id_array_num']);
                
                $couponTypeModel = new \addons\coupontype\model\VslCouponTypeModel();
                foreach ($coupon_type_id_array_num as $k1 => $v1) {
                    foreach ($v1 as $k2 => $v2) {
                        $coupon_info = $couponTypeModel->getInfo(['coupon_type_id'=>$k2],'coupon_name');
                        $coupon_arr = [
                            'id' => $k2,
                            'num' => $v2,
                            'type' => 'coupon_type',
                            'name' => $coupon_info ? $coupon_info['coupon_name'] : ''
                        ];
                        array_push($coupon_type_id_array_list,$coupon_arr);
                    }
                }
            }
            ...
            if($goods_info['gift_voucher_id_array_num']){
                ...
                $couponTypeModel = new \addons\giftvoucher\model\VslGiftVoucherModel();
                foreach ($gift_voucher_id_array_num as $k1 => $v1) {
                    foreach ($v1 as $k2 => $v2) {
                        $coupon_info = $couponTypeModel->getInfo(['gift_voucher_id'=>$k2],'giftvoucher_name');
                        $gift_arr = [
                            'id' => $k2,
                            'num' => $v2,
                            'type' => 'gift_voucher',
                            'name' => $coupon_info ? $coupon_info['giftvoucher_name'] : ''
                        ];
                        array_push($gift_type_id_array_list,$gift_arr);
                    }
            ...
            if($goods_info['package_voucher_id_array_num']){
                ...
                $couponTypeModel = new \addons\voucherpackage\model\VslVoucherPackageModel();
                foreach ($package_voucher_id_array_num as $k1 => $v1) {
                    foreach ($v1 as $k2 => $v2) {
                        $coupon_info = $couponTypeModel->getInfo(['voucher_package_id'=>$k2],'voucher_package_name');
                        $gift_arr = [
                            'id' => $k2,
                            'num' => $v2,
                            'type' => 'gift_voucher',
                            'name' => $coupon_info ? $coupon_info['voucher_package_name'] : ''
                        ];
                        array_push($package_type_id_array_list,$gift_arr);
                    }
                }
            }
            
            $this->assign("coupon_type_id_array_list", json_encode($coupon_type_id_array_list, JSON_UNESCAPED_UNICODE));
            $this->assign("gift_type_id_array_list", json_encode($gift_type_id_array_list, JSON_UNESCAPED_UNICODE));
            $this->assign("package_type_id_array_list", json_encode($package_type_id_array_list, JSON_UNESCAPED_UNICODE));
           
            $this->assign("coupon_type_id_array", json_encode($coupon_type_id_array, JSON_UNESCAPED_UNICODE));
            $this->assign("gift_voucher_id_array", json_encode($gift_voucher_id_array, JSON_UNESCAPED_UNICODE));
            $this->assign("package_voucher_id_array", json_encode($package_voucher_id_array, JSON_UNESCAPED_UNICODE));
            // var_dump(json_encode($coupon_type_id_array_num));exit;
            $this->assign("coupon_type_id_array_num", json_encode($coupon_type_id_array_num, JSON_UNESCAPED_UNICODE));
            $this->assign("gift_voucher_id_array_num", json_encode($gift_voucher_id_array_num, JSON_UNESCAPED_UNICODE));
            $this->assign("package_voucher_id_array_num", json_encode($package_voucher_id_array_num, JSON_UNESCAPED_UNICODE));
            ...
            if ($good_discount['user_obj']['u_level_data'] && $memberLevelRes['data']) {
                ...
                foreach ($memberLevelRes['data'] as $v) {
                    $member_temp[$v->level_id] = [
                        'name' => $v['level_name'],
                        'val' => $good_discount['user_obj']['u_level_data'][$v->level_id]['val']
                    ];
                }
                //变更 需更新分销等级列表信息
            ...
            //供应商结算方式
            $supplier_pay_type = $goods->getSupplierPaymentTypeInfoByGoodsId($goodsId);
            $this->assign("good_discount", $good_discount);
            $this->assign("goods_info", $goods_info);
            $this->assign("goods_info2", json_encode($goods_info));
            $this->assign("supplier_pay_type", $supplier_pay_type['payment_type'] ?:0);
...
//        return json($res);
    ...
    public function goodsCreateOrUpdate()
    {
        ...
        //自定义表单id
       $form_base_id = $product['form_base_id'] ? : 0;
        //组装独立全球分红 区域分红 
        ...
        $goodservice = new GoodsService();
        $goods_data = [
            'goods_id'                      => $product["goodsId"], // 商品Id
            'goods_name'                    => $product["title"], // 商品标题
            'shopid'                        => $shopId,
            'category_id'                   => $product["categoryId"], // 商品类目
            ...
            'evaluate_give_point'           => $product["evaluate_give_point"],
            ...
            'discount_bonus'                => $product["discount_bonus"],
	    'is_commission'            => $product['is_commission'],
            'cash_method'            => $product['cash_method'],
            'write_off_method'            => $product['write_off_method'],
            'cash_reward'            => $product['cash_reward'],
            'write_off_reward'            => $product['write_off_reward'],
            'coupon_type_id_array'                => $product["coupon_type_id_array"],
            'gift_voucher_id_array'                => $product["gift_voucher_id_array"],
            'package_voucher_id_array'                => $product["package_voucher_id_array"],
            'coupon_type_id_array_num'                => $product["coupon_type_id_array_num"],
            'gift_voucher_id_array_num'                => $product["gift_voucher_id_array_num"],
            'package_voucher_id_array_num'                => $product["package_voucher_id_array_num"],
    ...
    public function ModifyGoodsOutline()
    {
        ...
        $condition['is_supplier'] = request()->post('is_supplier',0); // 是否供应商商品
        $goods_detail = new GoodsService();
    ...
    public function goodscomment()
    {
        if (request()->isAjax()) {
            ...
            if (!empty($member_name)) {
                $condition["nm.member_name|nm.user_name_default|su.user_name|su.nick_name|su.user_tel"] = array(
                    "like",
                    "%" . $member_name . "%"
                );
            }
    ...
    public function getGoodsCount()
    {
        ...
        //全部
        $goods_count_array['all'] = $goods_count->getGoodsCount(['website_id' => $this->website_id, 'shop_id' => $this->instance_id, 'supplier_id'=> $this->supplier_id]);
        //出售中
        $goods_count_array['sale'] = $goods_count->getGoodsCount(['website_id' => $this->website_id, 'state' => 1, 'shop_id' => $this->instance_id, 'supplier_id'=> $this->supplier_id]);
        //仓库中
        $goods_count_array['shelf'] = $goods_count->getGoodsCount(['website_id' => $this->website_id, 'state' => 0, 'shop_id' => $this->instance_id, 'supplier_id'=> $this->supplier_id]);
        //已售罄
        $goods_count_array['soldout'] = $goods_count->getGoodsCount(['website_id' => $this->website_id, 'shop_id' =>
            $this->instance_id, 'supplier_id'=> $this->supplier_id,   'stock' => array("<=", "0")]);
        //库存预警
        $goods_count_array['alarm'] = $goods_count->getGoodsCount(['website_id' => $this->website_id, 'state' => 1, 'shop_id' => $this->instance_id, 'supplier_id'=> $this->supplier_id, 'min_stock_alarm' => array("neq", 0), 'stock' => array("exp", "<= min_stock_alarm")]);
     ...
     * 转为自营商品
     */
    public function changeToSelfGoods()
    {
        if (request()->isAjax()) {
            $goods_id = request()->post('goods_id',0);
            $goods = new GoodsService();
            $res = $goods->changeToSelfGoods($goods_id);
            return AjaxReturn($res);
        }
    }
    /*
     * 限时抢购 选择分类
     */
    public function selectDisCategory()
    {
        return view($this->style . 'Goods/selectDisCategory');
    }
    /**
     * 显示抢购 检测分类重复
     */
    public function checkActiveByCategory(){
        $start_time = request()->post('start_time','');
        $end_time = request()->post('end_time','');
        $category_extend_id = request()->post('category_extend_id','');
        if(empty($category_extend_id)){
            return ['code' => 1];
        }
        $activeListServer = new ActiveList();
        $result = $activeListServer->checkActiveByCategory($category_extend_id,$start_time,$end_time);
        return ['code' => $result];
    }
}