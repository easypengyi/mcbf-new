...
class Express extends BaseController
{
    ...
    public function freightTemplateList()
    {
        if (request()->isAjax()) {
            ...
            $template_lists = $express->getShippingFeeList($page_index, $page_size, $condition, 'is_default desc,create_time desc');
    ...
    public function freightTemplateEdit()
    {
        ...
        if (request()->isAjax()) {
            ...
            if($shipping_fee_id){
                $this->addUserLogByParam('运费模板管理-添加', $shipping_fee_id);
            }
    ...
    public function setDefaultShippingFee()
    {
        ...
        if ($shipping_fee_id) {
            $express = new ExpressService();
            $result = $express->defaultShippingFee($shipping_fee_id);
        }
        if($result){
            $this->addUserLogByParam('设置默认运费模板', $result);
        }
    ...
    public function freightTemplateDelete()
    {
        ...
        if($retval){
            $this->addUserLogByParam('运费模板删除', $retval);
        }
...
//            $condition['nr.website_id'] = $this->website_id;
    ...
    public function setUse()
    {
        ...
        if($result){
            $this->addUserLogByParam('启用该物流公司', $co_id);
        }
    ...
    public function setUnused()
    {
        ...
        if($result){
            $this->addUserLogByParam('禁用该物流公司', $co_id);
        }
        return AjaxReturn($result);
    }
}