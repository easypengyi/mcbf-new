...
class Wchat extends BaseController
{
    ...
    public function menu()
    {
        ...
        $miniSer = new miniProgramService();
            $this->assign('isV3', 1);
        }
    ...
    public function updateMenuToWeixin()
    {
        ...
        $config = new Config();
        $auth_info = $config->getConfig($this->instance_id, 'SHOPWCHAT', $this->website_id);
            ...
            if ($tag_all) {
                ...
                if ($tag_info) {
                    foreach ($tag_info as $k => $v) {
                        ...
                        if($group_id){
                            $fan_group->save(['group_name' => urldecode($v['name'])],['website_id' => $this->website_id, 'group_id' => $v['id']]);
                        }else{
                            if($v['id']==2) {
                                $fan_group->save(['website_id' => $this->website_id, 'group_id' => $v['id'], 'group_name' => urldecode($v['name'])]);
                            }else{
                                $fan_group->save(['website_id' => $this->website_id, 'group_id' => $v['id'],'from'=>2, 'group_name' => urldecode($v['name'])]);
                            }
                        }
                    }
                }
