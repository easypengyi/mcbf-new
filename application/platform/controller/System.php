...
use addons\delivery\service\Delivery as DeliveryServer;
...
class System extends BaseController
{
    ...
    public function albumList()
    {
        if (request()->isAjax()) {
            ...
            $retval = $album->getAlbumClassList($page_index, $page_size, $condition, 'sort desc');
            return $retval;
        } else {
            $album = new Album();
        ...
        if (request()->isAjax()) {
            ...
            if ($list['data']) {
                foreach ($list['data'] as $key => $val) {
                    ...
                    if($val['uid']){
                        $userInfo = $this->user->getUserInfo($val['uid'], 'user_name,user_tel,nick_name');
                        $list['data'][$key]['user_name'] = $userInfo['user_name']?: $userInfo['user_tel']?:$userInfo['nick_name'];
                    }else{
                        $list['data'][$key]['user_name'] = '';
                    }
    ...
    public function setStorageConfig() {
        ...
        debugLog($res, '==setStorageConfig-1<==');
        if ($res && $type==1) {
            return AjaxReturn(1);
        }
        if($Accesskey && $Secretkey){
            ...
            debugLog($buckets, '==setStorageConfig-2<==');
            ...
            debugLog($result, '==setStorageConfig-3<==');
   ...
   */
    ...
    public function getWapConfig()
    {
        ...
        if($web_info['wap_status']==1){
            ...
            if($web_info['realm_ip']){
                $data['wap_url'] = $this->http.$web_info['realm_ip'].'/wap/pages/mall/index';
            }else{
                $ip = top_domain($_SERVER['HTTP_HOST']);
                $web_info['realm_two_ip'] = $web_info['realm_two_ip'].'.'.$ip;
                $data['wap_url'] = $this->http.$web_info['realm_two_ip'].'/wap/pages/mall/index';
            }
        }else{
            $data['wap_status'] = $web_info['wap_status'];
        }
    ...
    public function getCoinConfig(){
        ...
        $blockchain_info = $blockchain['value'];
    ...
    public function planTask()
    {
        if (request()->isAjax()) {
            ...
            //获取发货助手，导表发货数据
            if (getAddons('delivery', $this->website_id)) {
                $delivery = new DeliveryServer();
                $delivery_files = $delivery->getDeliveryFileList($page_index, $page_size, $condition, 'create_time desc');
                if ($delivery_files['data']) {
                    foreach($delivery_files['data'] as $key => $val){
                        $return_data[] = [
                            'type' => 4,
                            'id' => $val['id'],
                            'task_type' => '导表发货',
            ...
            if (getAddons('electroncard', $this->website_id)) {
                ...
                $electroncard_files = $electroncard_server->getElectroncardFileList($page_index, $page_size, $condition, 'create_time desc');
                if ($electroncard_files['data']) {
                    foreach($electroncard_files['data'] as $key => $val){
                        $return_data[] = [
                            'type' => 5,
                            'id' => $val['id'],
                            'task_type' => '电子卡密数据批量导入',
                            'status' => $val['status'],
                            'log' => $val['error_info'] ?: '',
                            'excel_name' => $val['old_excel_name'] ?: '',
                            'zip_name' => '',
            ...
            $excelslist = $excelsModel->pageQuery($page_index, $page_size, $condition, 'addtime desc', '*');
            if($excelslist){
                foreach ($excelslist['data'] as $key => $value) {
                    ...
                    $return_data[] = [
                        'type' => 3,
                        'id' => $value['id'],
                        'task_type' => $value['exname'].'导出',
                        'status' => $value['status'],
                        'log' => $value['msg'] ?: '',
                        'excel_name' => $string ?: $value['exname'],
                        'zip_name' => '',
                        'create_time' => $create_time,
                    ];
                }
            ...
            $data = [
                'data' => $return_data,
                'total_count' => ($goodsHelpList['total_count'] ?: 0) + ($invoice_files['total_count'] ?: 0) + ($excelslist['total_count'] ?: 0) + ($delivery_files['total_count'] ?: 0)+ ($electroncard_files['total_count'] ?: 0),
                'page_count' => ($goodsHelpList['page_count'] ?: 0) + ($invoice_files['page_count'] ?:0) + ($excelslist['page_count'] ?:0) + ($delivery_files['page_count'] ?: 0)+ ($electroncard_files['page_count'] ?: 0)
            ];
        ...
        //清空未读信息
        $excelsModel = new VslExcelsModel();
        $excelsModel->save(['is_view'=>0],['website_id'=>$this->website_id]);
    ...
    public function delTask()
    {
        if (request()->isAjax()) {
            ...
            try {
                ...
                }else if ($type == 2&& getAddons('invoice', $this->website_id,$this->instance_id)) {//发票助手
                ...
                }else if ($type == 4 && getAddons('delivery', $this->website_id)) {//导表发货
                    $deliveryFile = new VslDeliveryFileModel();
                    $state = $deliveryFile::get($id)->status;
                    if ($state != 3) {
                        return ['code' => -1, 'message' => '状态已改变'];
                    }
                    $res = $deliveryFile->where(['id' => $id])->delete();
                }else if ($type == 5 && getAddons('electroncard', $this->website_id)) {//电子卡密批量导入
                    $electroncardFile = new VslElectroncardDataFileModel();
                    $state = $electroncardFile::get($id)->status;
                    if ($state != 3) {
                        return ['code' => -1, 'message' => '状态已改变'];
                    }
                    $res = $electroncardFile->where(['id' => $id])->delete();
                }

                Db::commit();
                $type = $type == 1 ? '商品导入' : ($type == 2 ? '发票导入' : ($type == 4 ? '导表发货' : '电子卡密数据批量导入'));
    ...
    {
        $id = request()->post('id');
        $type = request()->post('type');//1： 商品助手任务  2：发票助手任务  4：导表发货任务  5：电子卡密批量导入

        $condition['website_id'] = $this->website_id;
        $condition['shop_id'] = $this->instance_id;
        $condition['status'] = 5;//部分成功

        $error_excel_path = '';
        if ($type == 1 && getAddons('goodhelper', $this->website_id)) {
        ...
        if ($type == 2 && getAddons('invoice', $this->website_id)) {
            //下载发票助手
            $condition[ 'id'] = $id;
            $invoiceFile = new VslInvoiceFileModel();
        ...
        if ($type == 3) {
            ...
            $excelsModel = new VslExcelsModel();
            $error_excel_path = $excelsModel->getInfo($condition2, 'path')['path'];
        }
        if ($type == 4 && getAddons('delivery', $this->website_id)) {
            //导表发货
            $condition[ 'id'] = $id;
            $deliveryFile = new VslDeliveryFileModel();
            $error_excel_path = $deliveryFile->getInfo($condition, 'error_excel_path')['error_excel_path'];
        }
        if ($type == 5 && getAddons('electroncard', $this->website_id)) {
            //电子卡密批量导入
            $condition[ 'id'] = $id;
            $electroncardFile = new VslElectroncardDataFileModel();
            $error_excel_path = $electroncardFile->getInfo($condition, 'error_excel_path')['error_excel_path'];
        }
        if ($error_excel_path) {
