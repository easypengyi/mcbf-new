...
//        return $this->ajaxFileReturn();
     ...
     * 功能说明：文件(图片)上传(存入相册)
     */
    public function uploadSvgaFile() {
        $data = array();
        // 重新设置文件路径
        $this->resetFilePath();
        // 检测文件夹是否存在，不存在则创建文件夹
        ...
        if ($ok["code"]) {
            @unlink($_FILES['file_upload']);
            $res['path'] = $this->reset_file_path . $newfile;
            if (is_file($this->reset_file_path . $newfile)) {
                $data['file_name'] = $ok["path"];
                $data['origin_file_name'] = $this->file_name;
                $data['file_path'] = $this->reset_file_path . $newfile;
                $data['state'] = '1';
...
//        $this->resetFilePath(1);//不需要重置
     ...
     * 功能说明：支付宝证书上传
     */
    public function uploadFileAlipay() {
        ...
        $this->file_path = 'upload/' . Session::get(request()->module() . 'website_id') . '/'. Session::get(request()->module() . 'instance_id') . '/'.$path.'/';
        ...
        if (!$this->validationFile(6)) {
            return $this->ajaxFileReturn();
        }
        $data = array();
        $appCertPublicKey = request()->get("appCertPublicKey", "");
        $alipayCertPublicKey = request()->get("alipayCertPublicKey", "");
        $alipayRootCert = request()->get("alipayRootCert", "");
        $type = request()->get("certtype", "");
        if ($type == 1) {
            $data['certtype'] = 1;
        }
        ...
        if (!empty($appCertPublicKey)) {
            if (legalFile($appCertPublicKey, $this->allow_file_ext['crt'])) {
                @unlink($_SERVER['DOCUMENT_ROOT'] . $appCertPublicKey);
            }
        }
        if (!empty($alipayCertPublicKey)) {
            if (legalFile($alipayCertPublicKey, $this->allow_file_ext['crt'])) {
                @unlink($_SERVER['DOCUMENT_ROOT'] . $alipayCertPublicKey);
            }
            $data['type'] = 2;
        }
        if (!empty($alipayRootCert)) {
            if (legalFile($alipayRootCert, $this->allow_file_ext['crt'])) {
                @unlink($_SERVER['DOCUMENT_ROOT'] . $alipayRootCert);
            }
            $data['type'] = 3;
        }
        if ($this->file_path == "") {
...
//            @unlink($_FILES['file_upload']);
//            if($type == 1){
//                shell_exec("openssl x509 -inform DER -in ".getcwd()."/".$this->file_path . $this->file_name."  -out ".getcwd()."/".$this->file_path . $file_names.".pem");
//                @unlink(getcwd()."/".$this->file_path . $this->file_name);
//            }
//            if($type == 2){
//                shell_exec("openssl pkcs12  -in  ".getcwd()."/".$this->file_path . $this->file_name." -passin pass:".$tl_password."  -out ".getcwd()."/".$this->file_path . $file_names.".pem -nodes");
//                @unlink(getcwd()."/".$this->file_path . $this->file_name);
//            }
            $data['filename'] = $file_names;
            $data['filesrc'] = $this->file_path . $file_names.".pem";
            $data['state'] = 1;
            $data['message'] = "文件上传成功";
        } else {
            $data['state'] = '0';
    ...
    public function uploadSsl() {
        ...
        if($ip == '172.16.243.84'){
            $this->file_path = '/www/wwwroot/cert/';
        }
        if ($this->file_path == "") {
...
//                     $data['file_name'] = $picture_info["pic_cover_mid"];
    ...
    public function uploadApp() {
        ...
        $fileSize = round($this->file_size/1024/1024,2);//四舍五入获取文件大小，单位M
        $unit = 'MB';
        if($this->file_size > 1073741824){
            $unit = 'GB';
            $fileSize = round($this->file_size/1024/1024/1024,2);//四舍五入获取文件大小，单位G
        }
        $ok = $this->moveUploadFile($_FILES["file_upload"]["tmp_name"], $this->reset_file_path . $this->file_name);
        if ($ok["code"]) {
            @unlink($_FILES['file_upload']);
            $data['filename'] = $this->file_name;
            $data['filesrc'] = __URLS($ok["path"]);
