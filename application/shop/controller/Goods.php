...
class Goods extends BaseController {
    ...
    public function goodsinfo($goodsid) {
        ...
        $goods_id = $goodsSer->getGoodsDetailById($goodsid, 'goods_id,price,goods_name,evaluates');
        if (!empty($goodsid) && $goods_id) {
            ...
            if ($default_client == "shop") {
                ...
                $redirect = __URL(__URL__ . "/wap/packages/goods/detail?goods_id=" . $goodsid);//新的
            ...
            // 商品详情
            $goods_info = $this->goods->getGoodsDetail($goodsid, 1);
...
//                    //判断是全部折扣还是部分
                    ...
                    if($discount_info['discount_type'] == 2){
                        ...
                        if($discount_info['discount_type'] == 1){
                            $goods_info['promote_price'] = round($discount->getDiscountPrice($goods_info['price'], $discount_info['discount_num']));
                        }else{
                            $goods_info['promote_price'] = $discount->getDiscountPrice($goods_info['price'], $discount_info['discount_num']);
                        }
                    }
            ...
            if(is_array($goods_attribute_list)){
            ...
            $goods_info["shipping_fee_name"] = '';
            if ($user_location['status'] == 1 || $user_location["province"] == '北京市') {
                // 定位成功，查询当前城市的运费
            ...
            if($this->is_qlkefu){
                ...
                if($qlkefu['ql_domain'] && $qlkefu['seller_code'] && $qlkefu['is_use']==1){
                    $seller_domain = $qlkefu['ql_domain'].'/index/index/chatBoxJs/u/'.$qlkefu['seller_code'];
                }
    ...
    public function goodsList() {
        ...
        if($order == 'ng.sales'){
            $orderby = 'ng.sales + ng.real_sales'. " " . $sort;
        }
...
//        print_r($goods_list['data'][0]);die;
    ...
    public function addCart() {
        ...
        $uid = $this->uid;
        $cart_detail = request()->post('cart_detail','');//$_POST['cart_detail'];
        $seckill_id = request()->post('seckill_id', 0);
        if (is_string($cart_detail)) {
            $cart_detail = json_decode(htmlspecialchars_decode($cart_detail), true);
        }
    ...
    public function againCart() {
        ...
        foreach ($cart_detail  as $k=>$v){
            ...
            $goodsSer = new \data\service\Goods();
            $goods_info = $goodsSer->getGoodsDetailById($goods_id, 'goods_id,price,goods_name,state');
            if(!$goods_info || ($goods_info && $goods_info['state'] != 1)){
                return -2;
            }
    ...
    public function getDescription(){
        $goods_id = request()->post('goods_id', '');
        $goodsSer = new \data\service\Goods();
        $goodsDes = $goodsSer->getGoodsDetailById($goods_id, 'goods_id,price,goods_name,description')['description'];
        return $goodsDes;
    }
