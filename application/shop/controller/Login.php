...
class Login extends Controller
{
    ...
    public function init()
    {
        ...
        // 是否开启qq跟微信
        $instance_id = 0;
        $qq_info = $config->getConfig(0, 'QQLOGIN', $this->website_id);
        $Wchat_info = $config->getConfig(0, 'WCHAT', $this->website_id);
        $this->assign("qq_info", $qq_info);
        $this->assign("Wchat_info", $Wchat_info);
        // 是否开启验证码
        $this->login_verify_code = $config->getConfig(0,'LOGINVERIFYCODE', $this->website_id);
        $this->assign("login_verify_code", $this->login_verify_code["value"]);
        // 是否开启通知
        $noticeMobile = $config->getConfig($instance_id, 'MOBILEMESSAGE', $this->website_id);
        $noticeEmail = $config->getConfig($instance_id, 'EMAILMESSAGE', $this->website_id);
        $this->notice['noticeEmail'] = $noticeEmail['is_use'];
        $this->notice['noticeMobile'] = $noticeMobile['is_use'];
    ...
    public function index()
    {
        ...
        $tencent = new Config();
        $qq_info = $tencent->getConfig(0, 'QQLOGIN', $this->website_id);
        $Wchat_info = $tencent->getConfig(0, 'WCHAT', $this->website_id);
        $this->assign("qq_info", $qq_info);
        $this->assign("Wchat_info", $Wchat_info);
    ...
    public function register()
    {
        ...
        $config = new Config();
        $instanceid = 0;
        $phone_info = $config->getConfig(0,'MOBILEMESSAGE', $this->website_id);
        ...
        $email_info = $config->getConfig(0,'EMAILMESSAGE', $this->website_id);
    ...
    public function oauthLogin()
    {
        ...
        if ($type == "WCHAT") {
            $config_info = $config->getConfig(0, 'WCHAT', $this->website_id);
