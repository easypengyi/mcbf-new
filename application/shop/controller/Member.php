...
class Member extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // 是否开启通知
        $noticeEmail = $web_config->getConfig(0, 'EMAILMESSAGE', $this->website_id);
    ...
    public function paymentOrder()
    {
        ...
        foreach ($list as $shop_id => $shop) {
            ...
            foreach ($shop['sku'] as $sku_id => $sku) {
                ...
                $store_list = $goodsSer->getGoodsDetailById($sku['goods_id'], 'goods_id,price,goods_name,store_list')['store_list'];
                if (empty($store_list)) {
                    $shop_has_store = 0;
...
//        p($list);die;
    ...
    {
        $config = new Config();
        $upload_type = $config->getConfigMaster(0, 'UPLOAD_TYPE', 0, 1) ? : 1;
        if ($upload_type == 1) {
    ...
    public function balanceList()
    {
        ...
        $config = new Config();
        $balanceConfig = $config->getConfig(0, 'WITHDRAW_BALANCE');
    ...
    public function balanceWithdrawals()
    {
        if (request()->isAjax()) {
            ...
            if ((md5($password) != $pay_password) && $is_check) {
                $result['code'] = -2;
            ...
            if ($withdraw_message) {
                ...
                $info1 = $config->getConfig(0, 'ALIPAY', $this->website_id);
                $info2 = $config->getConfig(0, 'WPAY', $this->website_id);
                $info3 = $config->getConfig(0, 'TLPAY', $this->website_id);
                if ($info1['is_use'] == 0) {
    ...
    public function updatePayPassword()
    {
        if (request()->isAjax()) {
            ...
            $password = request()->post("payment_password", '');
            $member = new MemberService();
            $res = $member->updateUserPaymentPassword($uid, $password);
            $config = new ConfigSer();
            ...
            if ($block) {
                $block = new Block();
                $block->updatePass($this->uid, $password, $password);
            }
            return AjaxReturn($res);
        } else {
            return view($this->style . "Member/updatePayPassword");
    ...
    */
    ...
    public function bonusIndex()
    {
        if ($this->globalStatus || $this->areaStatus || $this->teamStatus) {
            ...
            $config = new Config();
            $bonus_set = $config->getConfig(0,"BONUSCOPYWRITING",$this->website_id, 1);
            if ($bonus_set) {
