...
// 存放身份证图片
define('UPLOAD_IDCARD', 'upload/' . Session::get('wapapiwebsite_id') . '/idcard/');//
define('UPLOAD_LIVE', 'upload/' . Session::get('wapapiwebsite_id') . '/live_cover/');
//存放直播举报图片
define('UPLOAD_LIVE_REPORT', 'upload/' . Session::get('wapapiwebsite_id') . '/live_report/');
...
class Upload extends Controller
{
    ...
    public function uploadImage()
    {
        ...
        switch ($type) {
            ...
            case 'customform':
                ...
                break;
            case 'idcard':
                $this->file_path = UPLOAD_IDCARD;
                break;
            case 'live':
                $this->file_path = UPLOAD_LIVE;
                break;
            case 'live_report'://
            $this->file_path = UPLOAD_LIVE_REPORT;
            break;
            default:
    ...
    private function validationFile()
    {
        $flag = true;
        if (strstr($this->file_path, 'goods/')) {
            if ($this->file_type != 'image/gif' && $this->file_type != 'image/png' && $this->file_type != 'image/jpeg' && $this->file_size > 10000000) {
