...
class Wchat extends Controller
{
    ...
    protected $error_log_url = 'public/ErrorLog/wchat/wchatmessage_err.txt';
...
//            webClose($web_info['close_reason'],$web_info['logo']);
    ...
    public function relateWeixin()
    {
        ...
        if (isset($sign)) {
            ...
            $config = new Config();
            $wchat_config = $config->getConfig(0, 'SHOPWCHAT', $this->website_id);
    ...
    public function getMessage()
    {
        ...
        if(strstr($from_xml, 'Encrypt')){
            $ticket_xml = '';
            $errCode = $this->decryptMsg($from_xml,  $ticket_xml);
        }else{
            $ticket_xml = $from_xml;
        }
...
//                                $this->wchat->event_key_text($postObj, $content);
                ...
                $user = new User();
                $userInfo = $user->getUserFirstData(['website_id'=>$this->website_id, 'wx_openid'=>$data['openid']],'uid');
                if ($userInfo) {
                ...
                if($info['openid']){
                    $user_cond2['wx_openid'] = $info['openid'];
                }
                if($info['unionid']){
                    $user_cond2['wx_unionid'] = $info['unionid'];
                }
                $user_info = $user_mdl->where($user_cond)->where(function($query) use ($user_cond2){
                    $query->where(['wx_openid' => $user_cond2['wx_openid']])->whereOr(['wx_unionid' => $user_cond2['wx_unionid']]);
                })->find();
                if(!$user_info){
            ...
            case "TEMPLATESENDJOBFINISH":
                ...
                $templateSer->userAcceptStatue($data);
                break;
            default:
                break;
        }
