...
class Upload extends Controller {
    ...
    public function uploadFile() {
        ...
        $year = date('Y',time());
      	...
      	$day = date('d',time());
      	$hour = date('H',time());
        $this->file_path = 'upload/' . Session::get(request()->module() . 'website_id') . '/' . Session::get(request()->module() . 'instance_id') . '/' . Session::get(request()->module() . 'supplier_id') . '/'.$year.'/'.$month.'/'.$day.'/'.$hour.'/';
        if ($this->file_path == "") {
