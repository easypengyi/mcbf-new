...
//        $auth = $auth_group->get($admin_info['group_id_array']);
    ...
    public function addAlbumClass() {
        ...
        $album = new Album();
        $retval = $album->addAlbumClass($album_name, $sort, 0, '', 0, $this->instance_id, $this->supplier_id);
        if ($retval) {
    ...
    public function albumPictureList() {
        if (request()->isAjax()) {
            ...
            $is_use = request()->post("is_use", 0);//0全部 1未使用
    ...
    public function operationLog() {
        if (request()->isAjax()) {
            ...
            $condition = ['website_id' => $this->website_id, 'supplier_id' => $this->supplier_id];
            ...
            if ($list['data']) {
                foreach ($list['data'] as $key => $val) {
                    ...
                    if($val['uid']){
                        $userInfo = $userService->getUserInfo($val['uid'], 'user_name,user_tel,nick_name');
    ...
    public function planTask()
    {
        if (request()->isAjax()) {
            ...
            $data = [
                'data' => $return_data,
                'total_count' => ($goodsHelpList['total_count'] ?: 0) + ($delivery_files['total_count'] ?: 0),
                'page_count' => ($goodsHelpList['page_count'] ?: 0) + ($delivery_files['page_count'] ?: 0)
            ];
