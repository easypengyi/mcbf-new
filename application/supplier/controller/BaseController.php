...
class BaseController extends Controller {
    ...
    public function init() {
        ...
        $this->instance_id = $this->user->getSessionInstanceId()?:Session::get($model . 'instance_id');
        $this->supplier_id = $this->user->getSessionSupplierId() ?:Session::get($model.'supplier_id');
        ...
        if (empty($this->supplier_id)) {
            $this->redirect(__URL("SUPPLIER_MAIN/login?website_id=").$this->website_id);
        }
        //获取是否拥有分销分红权限
...
//        }
        ...
        if (strpos($this->action, 'menu_') !== false) {
            ...
            $redirect = __URL(__URL__ . '/' . SUPPLIER_MODULE . "/Menu/addonmenu", $params);
        ...
        if ($check_auth) {
            ...
            $this->assign("instance_id", $this->instance_id);//TODO 供应商
            if (!request()->isAjax()) {
...
//                    $this->error('商城已过期，请联系商家',__URL("ADMIN_MAIN/login?website_id=").$this->website_id);
//                }
                /* 店铺导航 */
                $shop = new Shop();
                // 用户信息
                $user_info = $this->user->getUserInfo();
                $shop_info = $shop->getShopDetail($user_info['instance_id']);//TODO 供应商
                ...
                //供应商信息
                $supplier_model = new VslSupplierModel();
                $supplier_info = $supplier_model->getInfo(['supplier_id' => $this->supplier_id]);
    ...
    public function addUserLog($operation, $target = '') {
        $this->user->addUserLog(0, 1, $this->controller, $this->action, \think\Request::instance()->ip(), $operation . ':' . $target, $operation, $this->supplier_id);
    }
    public function getOrderCount(){
        $order = new OrderService();
        $order_count_array = array();
        $order_count_array['daifahuo'] = $order->getOrderCount(['order_status'=>1,'supplier_id'=>$this->supplier_id,'website_id'=>$this->website_id]);//待发货
        $order_count_array['tuikuanzhong'] = $order->getOrderCount(['order_status'=>-1,'supplier_id'=>$this->supplier_id,'website_id'=>$this->website_id]);//退款中
    ...
    public function businessCenter(){
        ...
        $auth = new User();
        $userInfo = $auth->getUserInfo($uid, 'is_system,user_tel,user_password');
