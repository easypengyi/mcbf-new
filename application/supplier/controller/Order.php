...
class Order extends BaseController {
    ...
    public function orderList() {
        ...
        if (request()->isAjax()) {
            ...
            if ($express_no) {
                $condition['express_no'] = ['LIKE', '%' . trim($express_no) . '%'];
            }
            if ($goods_name) {
                $condition['goods_name'] = ['LIKE', '%' . trim($goods_name) . '%'];
            }
            if ($order_type) {
            ...
            $this->assign('supplier_module', $this->module == 'supplier' ?1:0);
    ...
    public function orderReceiverList()
    {
        ...
        $condition['website_id'] = $this->website_id;
//        $condition['shop_id'] = $this->instance_id;//供应商订单不带shop_id
    ...
    public function orderDetail() {
        ...
        foreach ($detail['order_goods'] as $k => $v) {
            if($v['supplier_id'] != $this->supplier_id) {
                unset($detail['order_goods'][$k]);
            }
        }
        foreach ($detail['order_goods_no_delive'] as $k => $v) {
            if($v['supplier_id'] != $this->supplier_id) {
                unset($detail['order_goods_no_delive'][$k]);
            }
            //供应商端详情
            if($this->module=='supplier'){
                $detail['order_goods_no_delive'][$k]['real_money'] = $v['supplier_money'] + $v['shipping_fee'];
                $detail['order_goods_no_delive'][$k]['supplier_money'] = roundLengthNumber(($v['supplier_money']- $v['shipping_fee'])/$v['num']);
            }
        }
//        p($detail['goods_packet_list']);
        foreach ($detail['goods_packet_list'] as $k => $v) {
            foreach ($v['order_goods_list'] as $k1 => $v1) {
                if($v1['supplier_id'] != $this->supplier_id) {
                    unset($v['order_goods_list'][$k1]);
                }
            }
            //供应商端详情
//            if($this->module=='supplier'){
...
//        }
    ...
    public function orderDeliveryData() {
        ...
        $data['goods_type'] = intval($goods_type);
    ...
    public function orderDelivery() {
        ...
        if ($shipping_type == 1) {
            //需要物流
            $retval = $order_service->orderDelivery($order_id, $order_goods_id_array, $express_name, $shipping_type, $express_company_id, $express_no, $delivery_num);
            if ($retval) {
                $this->addUserLog(' 订单发货需要物流', '订单编号(' . $order_info['order_no'] . '),订单商品id(' . $order_goods_id_array . '),物流信息(' . $express_no . ')');
            }
        } else {
            //无需物流
            $retval = $order_service->orderGoodsDelivery($order_id, $order_goods_id_array, $delivery_num);
            if ($retval) {
    ...
    public function getOrderGoodsCount()
    {
        ...
        $condition = [
            'no.website_id' => $this->website_id,
            'no.is_deleted' => 0,
            'nog.supplier_id' => $this->supplier_id
        ];
//        $sql1 = "instr(CONCAT( ',', no.supplier_id, ',' ), '," . $this->supplier_id . ",' ) ";
//        $condition[] = ['exp',$sql1];

        $condition['nog.refund_status'] = ['IN', [2, -3]];
        $order_count_array['waiting_buyer_operation'] = $orderGoods->getOrderGoodsCount($condition); //待买家操作

        $condition['nog.refund_status'] = ['IN', [1, 3, 4]];
        $order_count_array['waiting_seller_operation'] = $orderGoods->getOrderGoodsCount($condition); //待卖家操作

        $condition['nog.refund_status'] = 5;
        $order_count_array['have_refunded'] = $orderGoods->getOrderGoodsCount($condition); //已退款

        $condition['nog.refund_status'] = ['IN', [-1, -3]];
        $order_count_array['have_refused'] = $orderGoods->getOrderGoodsCount($condition); //已拒绝
        ...
        $condition['nog.refund_status'] = ['IN', [-1, -3, 1, 2, 3, 4, 5]];
        $order_count_array['all'] = $orderGoods->getOrderGoodsCount($condition); //全部
    ...
    public function updateOrdersAddress()
    {
        ...
        $order_service = new OrderService();
        $condition['website_id'] = $this->website_id;
//        $condition['shop_id'] = $this->instance_id;//供应商订单不用shop_id
        $condition['supplier_id'] = $this->supplier_id;
        $condition['order_id'] = ['IN', $order_id_array];
    ...
    public function editExcelTemplate()
    {
        ...
        if($template_id>0){
            ...
            $input['supplier_id'] = $this->supplier_id;
    ...
    public function getOrderCount()
    {
        ...
        if ($buyer_id) {
            $order_count_array['daifukuan'] = $order->getOrderCount(['order_status' => 0, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //代付款
            $order_count_array['daifahuo'] = $order->getOrderCount(['order_status' => 1, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //代发货
            $order_count_array['yifahuo'] = $order->getOrderCount(['order_status' => 2, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //已发货
            $order_count_array['yishouhuo'] = $order->getOrderCount(['order_status' => 3, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //已收货
            $order_count_array['yiwancheng'] = $order->getOrderCount(['order_status' => 4, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //已完成
            $order_count_array['yiguanbi'] = $order->getOrderCount(['order_status' => 5, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //已关闭
            $order_count_array['tuikuanzhong'] = $order->getOrderCount(['order_status' => -1, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //退款中
            $order_count_array['yituikuan'] = $order->getOrderCount(['order_status' => -2, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //已退款
            $order_count_array['all'] = $order->getOrderCount(['website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //全部
            $order_count_array['chuli'] = $order->getOrderCount(['order_status' => 6, 'website_id' => $this->website_id, 'supplier_id' => $this->supplier_id, 'buyer_id' => $buyer_id, 'is_deleted' => 0]); //链上处理中
        } else {
            $condition = [
            ...
            $order_count_array['daifukuan'] = $order->getOrderCount($condition); //代付款

            $condition['order_status'] = [['neq',4],['neq',5],['neq',-1]];
            $condition['shipping_status'] = 0;
            $condition['pay_status'] = 2;
            $order_count_array['daifahuo'] = $order->getOrderCount($condition); //代发货

            unset($condition['shipping_status'],$condition['pay_status']);
            $condition['order_status'] = 2;
            $order_count_array['yifahuo'] = $order->getOrderCount($condition); //已发货

            $condition['order_status'] = 3;
            $order_count_array['yishouhuo'] = $order->getOrderCount($condition); //已收货

            $condition['order_status'] = 4;
            $order_count_array['yiwancheng'] = $order->getOrderCount($condition); //已完成

            $condition['order_status'] = 5;
            $order_count_array['yiguanbi'] = $order->getOrderCount($condition); //已关闭

            $condition['order_status'] = -1;
            $order_count_array['tuikuanzhong'] = $order->getOrderCount($condition); //退款中

            $condition['order_status'] = -2;
            $order_count_array['yituikuan'] = $order->getOrderCount($condition); //已退款

            $condition['order_status'] = 6;
            $order_count_array['chuli'] = $order->getOrderCount($condition); //链上处理中

            unset($condition['order_status']);
            $order_count_array['all'] = $order->getOrderCount($condition); //全部
        }
