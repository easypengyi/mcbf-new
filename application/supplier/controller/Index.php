...
use addons\supplier\model\VslSupplierWithdrawModel;
...
class Index extends BaseController {
    ...
    public function __construct() {
        ...
        $shop = new VslSupplierModel();
        
        $base_info = $shop->getInfo(['shop_id' => $this->instance_id, 'website_id' => $this->website_id,'supplier_id' => $this->supplier_id]);
        if($base_info['shop_logo']){
            $this->assign("basic_set",1);
        }
        $express_company_shop_relation_model = new VslExpressCompanyShopRelationModel();
        $list['express_company_set'] = $express_company_shop_relation_model->getInfo(['shop_id' => $this->instance_id,'website_id'=>$this->website_id,'supplier_id' => $this->supplier_id]);
        if($list['express_company_set']){
            $this->assign("express_company_set",1);
        }
        $vsl_order_shipping_fee = new VslOrderShippingFeeModel();
        $list['shipping_fee_set'] = $vsl_order_shipping_fee->getInfo(['shop_id' => $this->instance_id,'website_id'=>$this->website_id,'supplier_id' => $this->supplier_id,'is_enabled'=>1]);
        if($list['shipping_fee_set']){
            $this->assign("shipping_fee_set",1);
        }
        $goodsSer = new GoodsService();
        $list['goods_set'] = $goodsSer->getGoodsDetailByCondition(['shop_id' => $this->instance_id,'website_id'=>$this->website_id,'supplier_id' => $this->supplier_id,'state'=>1]);
        if($list['goods_set']){
    ...
    public function preview()
    {
        ...
        if($web_info['realm_ip']){
            $text = $this->http.$web_info['realm_ip'].'/wap/shop/home/'.$this->instance_id;
                ...
                $text = $this->http.$web_info['realm_two_ip'].'/wap/shop/home/'.$this->instance_id;
            }
    ...
    public function getGoodsCount(){
        ...
        //仓库中
        $goods_count_array['store'] = $goods_count->getGoodsCount(['website_id'=>$this->website_id,'state'=>0,'shop_id'=>$this->instance_id,'supplier_id'=>$this->supplier_id]);
        //出售中
        ...
        //库存预警
        $goods_count_array['warning'] = $goods_count->getGoodsCount(['website_id'=>$this->website_id,'shop_id'=>$this->instance_id,'supplier_id'=>$this->supplier_id,'min_stock_alarm' => array("neq", 0), 'stock' => array("exp", "<= min_stock_alarm")]);
        return $goods_count_array;
    }
    /**
     * 获取 订单数量       待发货     退款中
...
//        $order_count_array['daifahuo'] = $order->getOrderCount(['order_status'=>1,'website_id'=>$this->website_id,'supplier_id'=>$this->supplier_id]);//待发货
        $daifahuo_condition = [
            'nm.is_deleted' => 0,
            'nm.shipping_status' => 0,
            'nm.pay_status' => 2,
            'nm.order_status' => ['not in','-1,4,5'],
            'nm.website_id' => $this->website_id,
        ];
        $sql1 = "instr(CONCAT( ',', nm.supplier_id, ',' ), '," . $this->supplier_id . ",' ) ";
        $daifahuo_condition[] = ['exp',$sql1];
        $orderModel = new VslOrderModel();
        $order_count_array['daifahuo'] = $orderModel->getViewCount2($daifahuo_condition);;//待发货
        $order_count_array['tuikuanzhong'] = $order->getOrderCount(['order_status'=>-1,'website_id'=>$this->website_id,'supplier_id'=>$this->supplier_id]);//退款中
        $order_count_array['total_tips'] =  $order_count_array['daifahuo']+$order_count_array['tuikuanzhong'];
        $order_count_array['complete'] = $order->getOrderCount(['create_time' => [[">",$start_date],["<",$end_date]],'order_status'=>4,'supplier_id'=>$this->supplier_id,'website_id'=>$this->website_id]);//本月成交
        $order_count_array['complete1'] = $order->getOrderCount(['create_time' => [[">",$start_date1],["<",$end_date1]],'order_status'=>4,'supplier_id'=>$this->supplier_id,'website_id'=>$this->website_id]);//上月成交
    ...
    public function getWithdrawCount(){
        $Withdraw = new VslSupplierWithdrawModel();
        $Withdraw_count_array = array();
        $Withdraw_count_array['withdraw'] = $Withdraw->getCount(['status'=>['in',[1,2]],'shop_id'=>$this->instance_id,'website_id'=>$this->website_id,'supplier_id' => $this->supplier_id]);//提现中
    ...
    public function getIndexCount(){
        ...
        $order= new OrderService();
        //今日和昨日销售额
        $condition1 = ['create_time' => [[">",$start_date],["<",$end_date]],'website_id'=>$this->website_id,'supplier_id' => $this->supplier_id,'order_status'=>[['>',0],['<',5]]];
        $condition2 = ['create_time' => [[">",$start_date1],["<",$end_date1]],'website_id'=>$this->website_id,'supplier_id' => $this->supplier_id,'order_status'=>[['>',0],['<',5]]];
        ...
        //供应商端
        $supplier_freezing_money = $supplier_total_money = 0;
        if($this->supplier_id){
            $supplierSer = new SupplierSer();
            $supplierAccount = $supplierSer->getSupplierAccount($this->supplier_id);
            $supplier_total_money = $supplierAccount['supplier_total_money'];
            $supplier_freezing_money = $supplierAccount['supplier_freezing_money'];
        }
        ...
        $result = array(
            ...
            "supplier_total_money"=>$supplier_total_money,//可提现金额
            "supplier_freezing_money"=>$supplier_freezing_money,//待结算金额
        );
        return $result;
    }
    /**
    ...
    public function getOrdermovementsChartCount() {
        ...
        for ($i = 0; $i < count($orderType); $i++) {
            ...
            for ($j = 0; $j < ($end + 1 - $start) / 86400; $j++) {
                ...
                if ($status) {
                    $count = $order->getOrderCount(['website_id' => $this->website_id,'shop_id'=>$this->instance_id,'supplier_id' => $this->supplier_id, 'create_time' => ['between', [$date_start, $date_end]], 'order_status' => $status]);
                } else {
                    $count = $order->getOrderCount(['website_id' => $this->website_id, 'shop_id'=>$this->instance_id,'supplier_id' => $this->supplier_id,'create_time' => ['between', [$date_start, $date_end]]]);
                }
                $aCount[$j] = $count;
...
//                );
    ...
    public function goodsAnalysis(){
        if (request()->isAjax()) {
            ...
            $condition["no.supplier_id"] = $this->supplier_id;
...
//        }
        ...
        $order = new OrderService();
        //下单买家数和下单金额
        $order_num = $order->getMemberOrderCount(['create_time' => ['between',[$start_date,$end_date]],'shop_id'=>$this->instance_id,'website_id' => $this->website_id,'supplier_id' => $this->supplier_id]);
        $order_money = $order->getOrderMoneySum(['create_time' => ['between',[$start_date,$end_date]],'shop_id'=>$this->instance_id,'website_id' => $this->website_id,'supplier_id' => $this->supplier_id],'order_money');
        //支付买家数和支付金额
        $pay_num = $order->getMemberOrderCount(['create_time' => ['between',[$start_date,$end_date]],'shop_id'=>$this->instance_id,'website_id' => $this->website_id,'supplier_id' => $this->supplier_id,'order_status'=>['>',0]]);
        $user_platform_money = $order->getOrderMoneySum(['create_time' => ['between',[$start_date,$end_date]],'shop_id'=>$this->instance_id,'website_id' => $this->website_id,'supplier_id' => $this->supplier_id,'order_status'=>['>',0]],'user_platform_money');
        $user_pay_money = $order->getOrderMoneySum(['create_time' => ['between',[$start_date,$end_date]],'shop_id'=>$this->instance_id,'website_id' => $this->website_id,'supplier_id' => $this->supplier_id,'order_status'=>['>',0]],'pay_money');
