...
class Seckill extends BaseService
{
    ...
    public function addSecKill(array $input, $check_method)
    {
        ...
        try {
            ...
            $condition = [
                ...
                'seckill_vrit_num' => $input['seckill_vrit_num'],
            ];
            ...
            $act_data = array(
                ...
                'type'=>2,
                'act_id'=>$seckill_id,
                'stime'=>$seckill_now_time,
                'etime'=>strtotime("+$hours hours",$seckill_now_time),
            ...
            foreach($input['goods_info'] as $sku_id=>$goods){
                ...
                $sec_goods[$i]['seckill_vrit_num'] = $goods['seckill_vrit_num'] ? : 0;
                $sec_goods[$i]['check_status'] = $check_status;
                $sec_goods[$i]['del_status'] = 1;
                $sec_goods[$i]['create_time'] = time();
                $sec_goods[$i]['max_buy_time'] = time();
                $i++;
                //更新redis里面的库存
                $redis_goods_sku_seckill_key = 'seckill_' . $seckill_id . '_' . $input['goods_id'] . '_' . $sku_id;//每个活动的库存都不一样
                $is_index = $redis->get($redis_goods_sku_seckill_key);
                if (!$is_index) {
                    $redis->set($redis_goods_sku_seckill_key, $goods['seckill_num']);
                }
            }
    ...
    {
        $survive_time = getSeckillSurviveTime($condition['website_id']);
...
//            'ns.shop_id' => $condition['shop_id'],
        ...
        $checked_seckill_arr = $seckill_mdl->getSeckillGoodsCount($condition5);
        //将所有的场次查出来
        $addonsConfigSer = new \data\service\AddonsConfig();
        $value_arr = $addonsConfigSer->getAddonsConfig('seckill', $condition['website_id'], 0, 1);
...
//        echo '<pre>';print_r($sk_quantum_arr);exit;
        ...
        foreach($sk_quantum_arr as $k=>$v){
            ...
            foreach($checked_seckill_arr as $k4=>$v4){
                if($v == $v4['seckill_name']){
                    $total_goods[$v]['seckill_name'] =  $v4['seckill_name'];
                    $total_goods[$v]['checked_total'] =  $v4['g_total'];
                    $total_goods[$v]['seckill_id'] =  $v4['seckill_id'];
                }
            }
        }
        ...
        if($diff_seckill_arr){
            foreach($diff_seckill_arr as $k5=>$v5){
                ...
                $total_goods[$v5]['checked_total'] =  0;
    ...
    public function getSeckillGoodsList($page_index=1, $page_size, $condition, $order_by)
    {
        ...
        $today_seckill_goods_list = array_values($today_seckill_goods_list);
        //页数
...
//        var_dump($start_date);exit;
    ...
    public function getCanApplyDate()
    {
        $website_id = $this->website_id;
        $addonsConfigSer = new \data\service\AddonsConfig();
        $value_arr = $addonsConfigSer->getAddonsConfig('seckill', $website_id, 0, 1);
...
//        echo $seckill_mdl->getLastSql();exit;
    ...
    {
        $goodsSer = new Goods();
        $goods_spec_format = $goodsSer->getGoodsDetailById($sku_arr[0]['goods_id'], 'goods_id,goods_name,goods_spec_format')['goods_spec_format'];
...
//            var_dump($show_value_str);
    ...
    public function delSeckillGoods($condition)
    {
        ...
        //活动列表变更为结束
        $activeListServer = new ActiveList();
        $activeListServer->changeActive($condition['seckill_id'],2,2,$this->website_id);
        $activeListServer->delActive($condition['seckill_id'],2);
        return $bool;
    }
    /*
     * 判断是否有该商品其它的未过期档
    ...
    public function updateSeckillGoodsDelStatus($seckill_id, $check_method, array $input)
    {
        ...
        $sec_data['least_buy'] = $input['least_buy'];
        $sec_data['seckill_vrit_num'] = $input['seckill_vrit_num'];
        ...
        $act_data = array(
            ...
            'type'=>2,
            'act_id'=>$seckill_id,
            'stime'=>$sec_data['seckill_now_time'],
            'etime'=>strtotime("+$hours hours",$sec_data['seckill_now_time']),
            'goods_id'=> $input['goods_id'],
            'category_extend_id'=> 0
        );
        $activeListServer = new ActiveList();
        ...
        foreach($input['goods_info'] as $sku_id=>$goods){
            ...
            $secRes =  $sec_goods_obj->getInfo($condition,'seckill_limit_buy,max_buy_time');
            ...
            $sec_goods['max_buy_time'] = $secRes['seckill_limit_buy'] != $goods['seckill_limit_buy'] ? time(): $secRes['max_buy_time'];
    ...
    public function getStatusGoodsCount($condition)
    {
        ...
        //已结束
        //获取当前的时间点和今天日期
        $condition4['ns.seckill_now_time'] = ['<', time() - $survive_time*3600];
    ...
    public function saveSeckConfig($is_open,$value)
    {
        ...
        if (!empty($seckill_info)) {
            $res = $ConfigService->updateAddonsConfig($value, '秒杀设置', $is_open, 'seckill');
        } else {
    ...
    public function isSeckillGoods($condition)
    {
        ...
        if(!empty($seckill_list['goods_id'])){
            ...
            if($now_time < $seckill_list['seckill_now_time']){
                $is_start = false;//未开始
            }elseif($now_time>=$seckill_list['seckill_now_time'] && $now_time<=$seckill_list['seckill_now_time']+$survive_time*3600){
                $is_start = true;//开始
            }else{
                $is_start = false;//结束
            }
            if($is_start){
                $seckill_list['seckill_end_time'] = $seckill_list['seckill_now_time']+$survive_time*3600;
                return $seckill_list;
            }else{
                return false;
            }
     ...
     * 判断当前商品是开始、已过期、未开始
     * **/
    public function getSeckillStatus($condition)
    {
        $is_addons = getAddons('seckill',$this->website_id);
        if(!$is_addons){
            return false;
    ...
    public function isSkuStartSeckill($condition)
    {
        ...
        try{
            ...
            $max_time = time();
            $min_time = time()-$survive_time*3600;
...
//        $seckill_info = $seckill_mdl->alias('s')->where($condition)->join('vsl_seckill_goods sg', 's.seckill_id = sg.seckill_id','LEFT')->select();
        $seckill_info = $seckill_mdl->alias('s')->where($condition1)->where(function($query) use ($survive_time, $seckill_date){
            //传过来的时间往前推
            $seckill_end_time1 = strtotime($seckill_date);
            $seckill_start_time1 = $seckill_end_time1-$survive_time*3600;
            //传过来的时间往后推
            $seckill_start_time2 = strtotime($seckill_date);
            $seckill_end_time2 = $seckill_start_time2+$survive_time*3600;
    ...
    public function subRedisSeckillGoodsStock($seckill_id, $goods_id, $sku_id)
    {
        $redis = connectRedis();
        $redis_goods_key = 'seckill_' . $seckill_id . '_' . $goods_id . '_' . $sku_id;
        $redis_goods_store = (int)$redis->get($redis_goods_key);
        $seck_goods_mdl = new VslSeckGoodsModel();
        $data['remain_num'] = $redis_goods_store;
        $res = $seck_goods_mdl->save($data, ['seckill_id'=>$seckill_id, 'sku_id'=>$sku_id]);
        return $res;
    }
    /*
     * 取消订单加剩余库存
