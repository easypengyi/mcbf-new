...
class VslSeckillModel extends BaseModel
{
    ...
    {
        return $this->hasMany('VslSeckGoodsModel','seckill_id','seckill_id');
    }
...
//        echo $this->getLastSql();exit;
    ...
    public function getSeckillConGoodsList($page_index=1, $page_size, $condition, $order_by){
        $today_seckill_goods_list = $this->alias('ns')
            ...
            ->field('ns.seckill_id, g.goods_name, g.price, g.shop_id, sap.pic_cover_big, nsg.seckill_price, nsg.seckill_num, nsg.remain_num, nsg.seckill_limit_buy, nsg.sku_id, gs.sku_name, g.goods_id, s.shop_name, ns.seckill_name, ns.seckill_time, ns.shop_id,ns.least_buy,ns.seckill_vrit_num,g.activity_pic')
            ->order($order_by)
            ->where($condition)
