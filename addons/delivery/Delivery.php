...
class Delivery extends Addons
{
    ...
    {
        parent::__construct();
        if($this->module=='platform' || $this->module == 'admin' || $this->module == 'supplier'){
    ...
    public function goodsShortName()
    {
        if ($this->module == 'platform') {
            ...
            $this->assign('categoryUrl', __URL('SUPPLIER_MAIN/goods/category'));
            $this->assign('goodsListUrl', __URL('SUPPLIER_MAIN/goods/goodslist'));
            $this->assign('ajaxEditGoodsUrl', __URL('SUPPLIER_MAIN/goods/ajaxEditGoodsDetail'));
        }
    ...
    public function printSetting()
    {
        ...
        $info = $configService->getConfig($this->instance_id, 'DELIVERY_ASSISTANT', $this->website_id, 1, $this->supplier_id);
    ...
    {
        $configService = new Config();
        $delivery_info = $configService->getConfig($this->instance_id, 'DELIVERY_ASSISTANT', $this->website_id, 1, $this->supplier_id);
        $this->assign('portUrl', $delivery_info['port']);
        $this->assign('areaUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/area')));
        $this->assign('deliveryTemplateDetailUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/deliveryTemplateDetail')));
        $this->assign('senderTemplateDetailUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/senderTemplateDetail')));
        $this->assign('expressTemplateDetailUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/expressTemplateDetail')));
    ...
    {
        $this->assign('orderListUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/orderList')));
        $this->assign('orderDataExcelUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/orderDataExcel')));
        $this->assign('adminOrderDataExcelUrl', __URL(addons_url('delivery://delivery/adminOrderDataExcel')));
        $this->assign('uploadFileUrl', __URL(call_user_func('addons_url_' . $this->module, 'delivery://delivery/uploadFile')));
        $this->fetch('template/' . $this->module . '/importDelivery');
    }
    /**
     * 发货助手一键发货
     */
