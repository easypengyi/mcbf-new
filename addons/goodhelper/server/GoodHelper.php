...
//        curl_setopt($curl,CURLOPT_PROXY,'http://140.227.212.179:3128');185.90.210.146	61468
     ...
     */
    public function addGoodsByXls($file, $zip, $add_type, $file_type,$website_id, $shop_id, $supplier_id=0) {
        set_time_limit(0);
        ...
        try {
            ...
            if ($shop_id) {
                $img_path = 'upload/' . $website_id . '/shop/' . $shop_id . '/goodhelper/common/';
            }
            //供应商
            if ($supplier_id){
                $img_path = 'upload/' . $website_id . '/supplier/' . $supplier_id . '/goodhelper/common/';
            }
            $zip_error_info = [];
            if ($zip) {
                $zip_error_info =  $this->get_zip_originalsize($zip, $img_path); //处理zip文件 bcnmnbnm
            ...
            if ($add_type == 1) {//淘宝csv去掉已读取的第二行中文字段名
                ...
                foreach ($excel_array as $k => $v) {
                    ...
                    if (count($right_arr[$k]) == $sheetCount) {
                        ...
                        $check = $goods->getInfo(['goods_name' => $excelData['title'], 'shop_id' => $shop_id, 'website_id' => $website_id,'supplier_id' => $supplier_id], 'goods_id');
                        ...
                        $goodsImg = $this->getGoodsImg($excelData['picture'],$website_id, $shop_id, $supplier_id);
                        $data['picture'] = $goodsImg['picture'];
                        $data['img_id_array'] = $goodsImg['img_id_array'];
                        //供应商
                        if($supplier_id){
                            $data['payment_type'] = 1;//成本价
                        }
                        if ($check) {
                        ...
                        $checkSkus = $goodsSku->getQuery(['goods_id' => $check['goods_id']], 'sku_id');
                        if ($checkSkus && $check['goods_id']) {
                            foreach ($checkSkus as $checkSku){
                                $goodsSku->save(['price' => $data['price'], 'stock' => $data['stock'], 'code' => $data['code'], 'update_date' => time()], ['sku_id' => $checkSku['sku_id']]);
                            }
                        } else {
                            //规格导入暂未处理
                            $goods_sku_id = $goodsSku->save(['goods_id' => $result, 'price' => $data['price'], 'stock' => $data['stock'], 'code' => $data['code'], 'update_date' => time(), 'create_date' => time()]);
                            //针对供应商商品‘original_sku_ids’记录新建商品时候，商品的sku_ids
                            $goods->save(['original_sku_ids' => $goods_sku_id], ['goods_id' => $result]);
                        }
                        $i++;
                    } else {
                        $error_arr[] = $v;
                    ...
                    if (!array_intersect($v_img_arr, $repeat_arr)) {//与重复数组没有交集,可以往下判断是否在zip解压成功中
                        ...
                        if (count($right) == count($v_img_arr)) {//该行所有图片也全在zip解压成功的集合中
                            ...
                            foreach ($v_img_arr as $v_img) {
                                ...
                                $img_name = $v_img;
                                $album = new Album();
                                $album_id = $album->getDefaultAlbum($website_id, $shop_id, $supplier_id)['album_id'];
                                $res = $this->photoCreateForGoodsHelper($file_path . '/', $file_name, $ext, 0, $img_name, $album_id, $image_size[0], $image_size[1], $img_name, 0, $result["domain"], $result["bucket"], $result['path'] ?: $file_name, $website_id, $shop_id,$supplier_id);
                                $t++;
                                //foeach 结束
                            }
                            ...
                            if (count($v_img_arr) == $t) {
                                ...
                                $goods = new VslGoodsModel();
                                $check = $goods->getInfo(['goods_name' => $excelData['商品名称'], 'shop_id' => $shop_id, 'website_id' => $website_id,'supplier_id' => $supplier_id], 'goods_id');
                                $data = [
                                ...
                                if ($excelData['运费模板']) {
                                    ...
                                    $shippingFee = $shippingModel->getInfo(['shipping_fee_name' => $excelData['运费模板'], 'website_id' => $website_id, 'shop_id' => $shop_id, 'supplier_id' => $supplier_id], 'shipping_fee_id');
                                    if ($shippingFee) {
                                        $data['shipping_fee_type'] = 2;
                                        $data['shipping_fee_id'] = $shippingFee['shipping_fee_id'];
                                    }
                                }
                                if ($check && $check['goods_id']) {
                                    $result = $goods->save($data, ['goods_id' => $check['goods_id']]);
                                ...
                                $otherData = $this->setOtherData($goods_id, $excelData['商品分类'], $excelData['商品品类'], $excelData['商品规格'], $excelData['规格值'], $excelData['商品属性'], $excelData['商品品牌'], $website_id, $shop_id,$supplier_id);
                                ...
                                $res = $goods->save($update, ['goods_id' => $goods_id]);
                                $i++;
                            } else {
            ...
            if ($i == 0) {
                $res['code'] = 0;
                $res['message'] = '错误:'.$excel_error_message;
                Db::rollback();
            } elseif ($i == $j) {
                $res['code'] = 1;
            ...
            Db::rollback();
            $msg = '行号:'.$e->getFile().' 解压错误错误信息:'.$e->getMessage();
     ...
     * @return array
     */
    public function addGoodsImportQueue($help_id)
    {
        if(config('is_high_powered')) {
            $data['help_id'] = $help_id;
            $exchange_name = config('rabbit_goods_import.exchange_name');
            $queue_name = config('rabbit_goods_import.queue_name');
            $routing_key = config('rabbit_goods_import.routing_key');
            $url = config('rabbit_interface_url.url');
            $url = $url.'/rabbitTask/goodsImport';
            $request_data = json_encode($data);
            $push_data = [
                "customType" => "goods_import",//标识什么业务场景
     ...
     * @param number $type            
     */
    public function photoCreateForGoodsHelper($upFilePath, $photoPath, $ext, $type, $pic_name, $album_id, $width, $height, $pic_tag, $pic_id, $domain, $bucket, $upload_img, $website_id, $shop_id,$supplier_id) {
        $width1 = 0.6 * $width;
        $width2 = 0.4 * $width;
        $width3 = 0.2 * $width;
        $width4 = 0.1 * $width;
...
//            if (stristr($type, $v['type'])) {
        ...
        $album = new Album();
        if ($pic_id == "") {
            $retval = $album->addPicture($pic_name, $pic_tag, $album_id, $upload_img, $width . "," . $height, $width . "," . $height, $photoArray["bigPath"]["path"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["middlePath"]["path"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["smallPath"]["path"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["littlePath"]["path"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $shop_id, $this->upload_type, $domain, $bucket, $website_id,$supplier_id);
        } else {
            $retval = $album->ModifyAlbumPicture($pic_id, $upload_img, $width . "," . $height, $width . "," . $height, $photoArray["bigPath"]["path"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["middlePath"]["path"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["smallPath"]["path"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["littlePath"]["path"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $shop_id, $this->upload_type, $domain, $bucket,$website_id,$supplier_id);
//            $retval = $pic_id;
        }
        return $retval;
    }
    ...
    public function getGoodsImg($picture, $website_id =0,$shop_id = 0,$supplier_id=0) {
        $img = [
            'picture' => 0,
            'img_id_array' => ''
        ];
        ...
        foreach ($allPics as $key => $imgurl) {
            ...
            $pic = $albumPic->getFirstData(['pic_name' => ['like','%'.$picDetail[0].'%'],'website_id' => $website_id, 'shop_id' => $shop_id, 'supplier_id' => $supplier_id],'upload_time desc'); //根据图片名称查找已导入的图片数据
    ...
    public function dealTaoImg($imgArray,$platForm = '0') {
        ...
        foreach ($imgArray as $key => $val) {
            ...
            $filename = $val;
            $albumPic = new AlbumPictureModel();
            $checkPic = $albumPic->getInfo(['pic_name' => $filename, 'website_id' => $this->website_id, 'shop_id' => $this->instance_id,'supplier_id'=>$this->supplier_id], 'pic_id');
    ...
    public function setOtherData($goods_id = 0, $category_name = '', $attr_name = '', $spec_name = '', $spec_value = '', $attr_value = '', $brand_name = '', $website_id = 0, $shop_id = 0, $supplier_id=0) {
        ...
        try {
            ...
            $data['goods_spec_format'] = $this->setSpecAndSku($spec_name, $spec_value, $goods_id, $data['attr_id'], $website_id, $shop_id,$supplier_id);

            $data['attr_value'] = $this->setAttrValue($attr_value, $goods_id, $data['attr_id'], $website_id, $shop_id,$supplier_id);

            $attrModel->commit();
            return $data;
        } catch (\Exception $e) {
            $msg = '行号:'.$e->getFile().' 添加商品信息错误信息:'.$e->getMessage();
            debugFile($msg,'','public/goodshelper.txt');
            $attrModel->rollback();
            return false;
        }
    ...
    public function setAttrValue($attr_value = '', $goods_id = 0, $attr_id = 0, $website_id = 0, $shop_id = 0, $supplier_id=0) {
        ...
        foreach ($attr_value_arr as $k_av => $v_av) {
            ...
            $attr_condition = [
                'attr_value_name' => $attr_value_one[0],
                'attr_id' => $attr_id,
                'type' => 1,
                'website_id' => $website_id,
                'shop_id' => array(['eq', 0], ['eq', $shop_id], 'or')
            ];
            if ($supplier_id){
                $attr_condition[] = ['exp','supplier_id ='.$supplier_id.' OR (supplier_id=0 AND shop_id=0)'];
            }
            $attr_value_id = $attrValueModel->getInfo($attr_condition, 'attr_value_id')['attr_value_id'];
            if (!$attr_value_id) {
                $attr_data = [
                    'attr_value_name' => $attr_value_one[0],
                    'attr_id' => $attr_id,
                ...
                $attr_value_id = $attrValueModel->save($attr_data);
            }
    ...
    public function setSpecAndSku($spec_name = '', $spec_value = '', $goods_id = 0, $attr_id = 0, $website_id = 0, $shop_id = 0,$supplier_id=0) {
        ...
        try {
            ...
            foreach ($spec_name_arr as $ks => $vs) {
                ...
                if ($supplier_id){
                    $spec_condition[] = ['exp','supplier_id ='.$supplier_id.' OR (supplier_id=0 AND shop_id=0)'];
                }
                $check_spec = $specModel->getInfo($spec_condition, 'spec_id,goods_attr_id');
                ...
                if (!$check_spec) {
                    ...
                    $specModel = new VslGoodsSpecModel();
                    $spec_id = $specModel->save($spec_data);
                } else {
    ...
    public function addGoodsByArray($data) {
        ...
        $goods = new VslGoodsModel();
        $check = $goods->getInfo(['taobao_id' => $data['taoId'], 'shop_id' => $data['instance_id'], 'website_id' => $data['website_id'],'supplier_id'=>$data['supplier_id']], 'goods_id');
        $insertData = [
            'goods_name' => $data['taoTitle'],
            'price' => $data['taoPrice'],
    ...
    public function getDetail($detail_list, $platform){
        $detail = '<p>';
        ...
        if(isset($detail_list['imgUrlMap']) && $platform != '1688'){
            $detail .= '<div style="width:100%; text-align:center;">';
            foreach($detail_list['imgUrlMap'] as $img){
             ...
             unset($img);
            $detail .= '</div>';
        }elseif($platform == '1688'){
            $detail .= $detail_list['imgUrlMap'];
        }
        $detail .= '</p>';
        return $detail;
    }
    ...
    public function login($usert_tel, $password = '') {
        ...
        if ($port[1] == 'supplier'){
            unset($condition['port']);
            $condition['supplier_id'] = ['>',0];
        }
        $userInfo = $userModel->getInfo($condition, $field = 'uid,user_tel,user_status,user_name,user_headimg,is_system,instance_id,is_member,current_login_ip, current_login_time, current_login_type,website_id,port,supplier_id');
    
        if (!empty($userInfo)) {
            if ($userInfo['user_status'] == 0) {
                return USER_LOCK;
            } else {
                $user = new \data\service\User();
                $user->initLoginInfo($userInfo);
                //登录成功后增加用户的登录次数
                $set_inc_condition['uid'] = $userInfo['uid'];
                $userModel->save(['login_num' => $userInfo['login_num'] + 1, 'user_token' => encryptPass($userInfo['uid'])], $set_inc_condition);
                return $userInfo;
            }
        } else {
            return USER_ERROR;
    ...
    {
        if(!array_key_exists('supplier_id',$condition)){
