...
class Taskcenter extends Addo
{
    ...
    public $menu_info = array(
		...
		[
            ...
            'icon_class' => '',//字体图标class（一般为空）
            'is_control_auth' => 1,//是否有控制权限
            'hook_name' => 'generalTaskDialogGoodsList',
            'module' => 'platform'
        ],
    ); // 钩子名称（需要该钩子调用的页面）
...
//        var_dump(__URL(addons_url_platform('taskcenter://Taskcenter/isRepeatKeyword')));exit;
        if ($this->module == 'platform') {
            ...
            $this->assign('generalTaskDialogGoodsListUrl', __URL(addons_url_platform('taskcenter://Taskcenter/generalTaskDialogGoodsList')));//关键字检查是否重复
        }
    }
...
//        p($general_poster_info);exit;
        ...
        if($goods_id){
            ...
            $goods_name = $goodsSer->getGoodsDetailById($goods_id)['goods_name'];
