...
class FestivalCare extends BaseService
{
    ...
    public function getFestivalCareDetail($condition,$type=0)
    {
        ...
        if($type==1 && $info){
            ...
            if($info['prize_type']==5){
                $goods = [];
                $vslgoods = $goodsSer->getGoodsDetailById($info['prize_type_id'], 'goods_id,goods_name,price,picture', 1);
    ...
    public function getPrizeName($prize_type_id, $prize_type)
    {
        ...
        if($prize_type==5){
            $vsl_goods = new \data\service\Goods();
            $goods = $vsl_goods->getGoodsDetailById($prize_type_id, 'goods_name');
    ...
    public function saveConfig($is_festivalcare)
    {
        ...
        if (!empty($info)) {
            $res = $AddonsConfig->updateAddonsConfig('', "节日关怀设置", $is_festivalcare, "festivalcare");
        } else {
        ...
        if($info){
            ...
            if ($res) {
                ...
                if(config('project') == 'shopvslai'){
                    $weixin_msg = str_replace("[奖品链接]", '<a href="'.__URLS('/wap/packages/festivalcare','',['website_id'=>$info['website_id']]).'/index?festivalcare_id='.$res.'">点击领取</a>', $weixin_msg);
                }else{
                    //获取小程序的appid
                    $weixin_auth = new WeixinAuthModel();
                    $mp_cond['website_id'] = $info['website_id'];
                    $mp_cond['shop_id'] = $this->instance_id?:0;
                    $app_id = $weixin_auth->getInfo($mp_cond, 'authorizer_appid')['authorizer_appid'];
                    $weixin_msg = str_replace("[奖品链接]",'<a href="http://www.qq.com" data-miniprogram-appid="'. $app_id .'" data-miniprogram-path="packages/festivalcare/index?festivalcare_id='. $res .'">点击领取</a>',$weixin_msg);
                }
                ...
                if($detail['member_prize_id']==0 && $detail['state']==2){
                    ...
                    if($record_prize>0){
                        ...
                        if($result){
                            ...
                            //加入延时队列处理奖品过期修改状态
                            $delay_time1 = $detail['expire_time'] - time();
                            if($delay_time1 > 0){
