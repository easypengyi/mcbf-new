    ...
    public function updateOrderMicroShop($params)
    {
        ...
        if(in_array($params['order_id'],$orderids)) {
            ...
            if ($member_info['isshopkeeper']==2|| $order_info['shopkeeper_id']) {//购买的微店商品或者自购
                ...
                $data['profitC'] = 0;
                $data['profit'] = 0;
                $order_profitA = $order_profit->Query(['order_id' => $params['order_id'],'return_status' => 0], 'profitA');
                $order_profitA_id = $order_profit->getInfo(['order_id' => $params['order_id']], 'profitA_id');
                $order_profitB = $order_profit->Query(['order_id' => $params['order_id'],'return_status' => 0], 'profitB');
                $order_profitB_id = $order_profit->getInfo(['order_id' => $params['order_id']], 'profitB_id');
                $order_profitC = $order_profit->Query(['order_id' => $params['order_id'],'return_status' => 0], 'profitC');
                ...
                if ($order_profitC_id['profitC_id'] && $list['microshop_pattern'] >= 3) {
                    ...
                    if($params['order_goods_id'] && $params['status'] == 2){
                        $order_commission = new VslOrderMicroShopProfitModel();
                        $re_data = array(
                            'return_status' => 1,
                        );
