...
class Microshop extends BaseService
{
    ...
    public function getMicroShopCount($website_id)
    {
        ...
        $data['profit_total'] = round(array_sum($profit_total),2);
        $withdrawals_total = $MicroShopAccount->Query(['website_id'=>$website_id],'withdrawals');
        $data['withdrawals_total'] = round(array_sum($withdrawals_total),2);
        return $data;
    }
    ...
    public function getShopKeeperLevelInfo($id)
    {
        ...
        if($level_info['goods_id']){
            ...
            $goods_info = $goodsSer->getGoodsListByCondition(['goods_id'=>['in',$level_info['goods_id']]], 'goods_id,picture,goods_name');
            foreach($goods_info as $k=>$v){
                $v = objToArr($v);
                $level_info['goods_info'][$k]['pic'] = $v['album_picture']['pic_cover_mid'];
                $level_info['goods_info'][$k]['goods_name'] = $v['goods_name'];
    ...
    public function getShopKeeperInfo($uid)
    {
        ...
        $result['total_profit'] = number_format($Profit_info['profit']+$Profit_info['withdrawals']+$result['freezing_profit'],2);//累积收益
        $user = new UserModel();
        $user_info = $user->getInfo(['uid'=>$uid],'user_headimg,user_name,user_tel,nick_name');
    ...
    public function getShopKeeperLevelLists($uid='',$page_index = 1, $page_size = 0, $where = '', $order = '')
    {
        ...
        if($list['data']){
            foreach($list['data'] as $k=>$v){
                ...
                $v['goods_info'] = array();
                if($v['goods_id']){
                    $goods_info = $goodsSer->getGoodsListByCondition(['goods_id'=>['in',$v['goods_id']]], 'goods_id,picture,goods_name,price');
                    if(!$goods_info){
                        continue;
                    }
                    $goods_info = array_column($goods_info, null, 'goods_id');
                    foreach ($goods_info as $ks=>$vs){
                        $v['goods_info'][$ks]['pic'] = $vs['album_picture']['pic_cover_mid'];
                        $v['goods_info'][$ks]['goods_name'] =$vs['goods_name'];
                        $v['goods_info'][$ks]['goods_id'] =$vs['goods_id'];
                        $v['goods_info'][$ks]['price'] = $vs['price'];
                    }
                    $v['goods_info'] = array_values($v['goods_info']);
                }
    ...
    public function getShopKeeperLevelInfos($uid)
    {
        ...
        $level_info['goods_info'] = array();
        $goodsSer = new \data\service\Goods();
        if($level_info['goods_id']){
            $goods_info = $goodsSer->getGoodsListByCondition(['goods_id'=>['in',$level_info['goods_id']]], 'goods_id,picture,goods_name,price');
            if(!$goods_info){
                return $level_info;
            }
            $goods_info = array_column($goods_info, null, 'goods_id');
            foreach ($goods_info as $ks=>$vs){
                 $level_info['goods_info'][$ks]['pic'] = $vs['album_picture']['pic_cover_mid'];
                 $level_info['goods_info'][$ks]['goods_name'] =$vs['goods_name'];
                 $level_info['goods_info'][$ks]['goods_id'] =$vs['goods_id'];
                 $level_info['goods_info'][$ks]['price'] = $vs['price'];
             }
             $level_info['goods_info'] = array_values($level_info['goods_info']);
        }
        return $level_info;
    }
    /**
    ...
    public function myGoodsList($uid){
        $member = new VslMemberModel();
        $microshop_goods = $member->getInfo(['uid'=>$uid],'microshop_goods')['microshop_goods'];
        $query = 'select goods_id, goods_name, price, max_price, create_time,sales,real_sales, album_picture.pic_cover from '.config('database.database').'_goods where state = 1 and for_store = 0 ';
        if($microshop_goods){
        ...
        $query.= " and shop_state = 1 and website_id =" .$this->website_id;
        ...
        $query .= ' order by create_time desc, goods_id asc';
        if(config('ecs.host')){
            $goodsSer = new GoodsService();
            $list = $goodsSer->getWapGoodsEs(0, $query, 0, ['create_time', 'goods_id']);
        }else{
    ...
    public function myCategoryList($uid,$condition=[])
    {
        ...
        $category = '';
        if($microshop_goods){
            $category = $goodsSer->getGoodsListByCondition(['website_id'=>$this->website_id,'goods_id'=>['in',$microshop_goods]], 'category_id_1,category_id_2,category_id_3');
            if($category){
                $category = array_unique(array_reduce($category, function ($result, $value) {
    ...
    public function setMicroShopSite($microshop_status,$microshop_pattern, $shopKeeper_check, $goods_id,$pro_types)
    {
        ...
        if (! empty($MicroShop_info)) {
            $res = $ConfigService->updateAddonsConfig($value, "微店设置", $microshop_status, "microshop");
        } else {
    ...
    public function getMicroShopSite($website_id){
        if($website_id){
            ...
            $MicroShop_info = $MicroShop['value'];
            if($MicroShop_info['goods_id']){
                $MicroShop_info['goodsid'] = explode(',',$MicroShop_info['goods_id']);
                $goodsSer = new \data\service\Goods();
                $goods_info = $goodsSer->getGoodsListByCondition(['goods_id'=>['in',$MicroShop_info['goods_id']]], 'goods_id,picture,goods_name,price');
            }else{
                $goods_info = [];
            }
            if($goods_info){
                $goods_info = array_column($goods_info, null, 'goods_id');
                foreach ($goods_info as $k=>$v){
                    $MicroShop_info['goods_info'][$k]['pic'] = $v['album_picture']['pic_cover_mid'];
    ...
    public function getMicroShopSettlementSite($website_id=null){
        ...
        $MicroShopInfo = $config->getConfig(0,"SETMICROSHOPTLEMENT",$this->website_id, 1);
    ...
    public function getMicroShopAgreementSite($website_id=null){
        $ConfigService = new ConfigService();
        $MicroShop_info = $ConfigService ->getConfig(0,"AGREEMICROSHOPMENT",$this->website_id, 1);
        return $MicroShop_info;
    }
    /**
     * 获得近七天的微店订单金额
    ...
    public function orderMicroShopProfit($params)
    {
        ...
        $set_info = $ConfigService ->getConfig(0,"SETMICROSHOPTLEMENT",$order_goods_info['website_id'], 1);
    ...
    public function becomeShopKeeper($uid,$order_id){
        ...
        $base_info = $MicroShop_info['value'];
        $order = new VslOrderModel();
    ...
    public function becomeShopKeeperProfit($uid,$goods_id,$order_id){
        ...
        if($MicroShop['referee_id']){
            ...
            if($MicroShops['isshopkeeper']==2){
                $goodsSer = new \data\service\Goods();
                $goods_price = $goodsSer->getGoodsDetailById($goods_id, 'goods_id,promotion_price')['promotion_price'];
    ...
    public function getProfitWithdrawConfig($uid){
        ...
        $list = $config->getConfig(0,"SETMICROSHOPTLEMENT",$account_info['website_id'], 1);
    ...
    public function addMicroShopProfitWithdraw($withdraw_no,$uid,$account_id,$cash){
        ...
        $Profit_withdraw_set = $config->getConfig(0,'SETMICROSHOPTLEMENT',$member_info['website_id'], 1);
        // 判断是否提现设置是否为空 是否启用
    ...
    public function profitWithdrawAudit($id,$status,$memo)
    {
        ...
        $res =0;
        $config = new ConfigService();
        $profit_withdraw_set = $config->getConfig(0,'SETMICROSHOPTLEMENT',$Profit_info['website_id'], 1);
        $make_money = $profit_withdraw_set['make_money'];
        ...
        if($Profit_info  && $status == 2 && $make_money==1) { // 平台手动审核通过提现自动打款，更新提现状态
            ...
            if($Profit_info['type']==3){//支付宝
                ...
                $params['website_id'] = $Profit_info['website_id'];
                $alipay_pay = new AliPay();
                $retval = $alipay_pay->aliPayTransferNew($Profit_info['withdraw_no'],$Profit_info['account_number'],abs($Profit_info['income_tax']),$Profit_info['realname']);
                if($retval['is_success']==1){
                    runhook('Notify', 'withdrawalSuccessBySms', $params);
                    $data_Profit['status'] =11;
                    $data_Profit['text'] = '收益提现成功到支付宝';//支付宝
            ...
            foreach ($list['data'] as $k => $v) {
                if($v['from_type']!=1 && $v['from_type']!=2 && $v['from_type']!=3 && $v['from_type']!=22){
                    ...
                    if($list['data'][$k]['balance'] == 0){
                        $list['data'][$k]['change_money'] = (-1)*abs($list['data'][$k]['profit']);
                        $list['data'][$k]['real_cash'] =  $list['data'][$k]['profit'];
                    }else{
                        $list['data'][$k]['real_cash'] =  $list['data'][$k]['profit'] + $list['data'][$k]['tax'];
                        $list['data'][$k]['profit'] =  (-1)*(abs($list['data'][$k]['profit'])+abs($list['data'][$k]['tax']));
                    }
                }else{
                    $list['data'][$k]['type'] = 0;
                    if($v['from_type']==1){
     ...
     * 收益提现列表导出Excel
     */
    public function getProfitWithdrawListToExcel($page_index, $page_size, $condition, $order = '', $field = '*')
    {
        $Profit_withdraw = new VslMicroShopProfitWithdrawModel();
        $member = new Member();
        $data = $Profit_withdraw->getViewList($page_index, $page_size, $condition, 'nmar.ask_for_date desc');
        $list['data'] = [];
        if($data['page_count'] >= 1) {
            for ($i = 1; $i <= $data['page_count']; $i++) {
                $res['data'] = [];
                $res['data'] = $Profit_withdraw->getViewList($page_index, $page_size, $condition, 'nmar.ask_for_date desc')['data'];
                if (!empty($res['data'])) {
                    foreach ($res['data'] as $k => $v) {
                        ...
                        $res['data'][$k]["user_info"] = $member->removeEmoji($res['data'][$k]["user_info"]);
    ...
    public function getAccountListToExcel($page_index, $page_size, $condition, $order = '', $field = '*')
    {
        $Profit_account = new VslMicroShopAccountRecordsViewModel();
        $member = new Member();
        $data = $Profit_account->getViewList($page_index, $page_size, $condition, 'nmar.create_time desc');
        $list['data'] = [];
        if($data['page_count'] >= 1) {
            for ($i = 1; $i <= $data['page_count']; $i++) {
                $res['data'] = [];
                $res['data'] = $Profit_account->getViewList($i, $page_size, $condition, 'nmar.create_time desc')['data'];
                if (!empty($res['data'])) {
                    foreach ($res['data'] as $k => $v) {
                        if($v['from_type']!=1 && $v['from_type']!=2 && $v['from_type']!=3 && $v['from_type']!=22){
                            if($v["from_type"]==10){
                            ...
                            $res['data'][$k]['from_type'] = $status_name;
                            $res['data'][$k]['profit'] = (-1)*(abs($res['data'][$k]['profit'])+abs($res['data'][$k]['tax']));
                        }else{
                            if($v['from_type']==1){
                                $res['data'][$k]['from_type'] = '订单完成';
                                $res['data'][$k]['profit'] = '+'.$res['data'][$k]['profit'];
                            }
                            if($v['from_type']==2){
                                $res['data'][$k]['from_type'] = '订单退款完成';
                                $res['data'][$k]['profit'] = '-'.$res['data'][$k]['profit'];
                            }
                            if($v['from_type']==3){
                                $res['data'][$k]['from_type'] = '订单支付完成';
                                $res['data'][$k]['profit'] = '+'.$res['data'][$k]['profit'];
                            }
                            if($v['from_type']==22){
                                $res['data'][$k]['from_type'] = '下级开店返利';
                                $res['data'][$k]['profit'] = '+'.$res['data'][$k]['profit'];
                            }
                        }
