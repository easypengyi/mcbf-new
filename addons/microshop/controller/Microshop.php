...
class Microshop extends baseMicroShop
{
    ...
    public function profitWithdrawListDataExcel()
    {
        ...
        $insert_data = array(
            ...
            'status' => 0,
            'exname' => $xlsName,
            'website_id' => isset($_GET['website_id']) ? $_GET['website_id'] : $this->website_id,
            'addtime' => time(),
    ...
    public function previewMicroShop()
    {
        ...
        if ($list) {
            foreach ($list['data'] as $k => $v) {
                $v['goods_price'] = $v['goods_price'] ? : $v['price'];
                $v['market_price'] = $v['max_price'] ? : $v['market_price'];
                if ($is_seckill) {
                    //判断如果是秒杀商品，则取最低秒杀价
                    $goods_id = $v['goods_id'];
                ...
                $goods_list[$k]['logo'] = $v['pic_cover'] ? getApiSrc($v['pic_cover']) : ($v['album_picture'] ? getApiSrc($v['album_picture']['pic_cover']) : '');
            }
        } else {
            $goods_list = [];
        }
     ...
     * 收益提现列表
     */
    public function profitWithdrawListBiz()
    {
        ...
        $list = $profit->getprofitWithdrawList($page_index, $page_size, $condition, 'ask_for_date desc');
        $list['withdraw_list'] = [];
        if($list['data']){
     ...
     * 收益提现详情
     */
    public function profitWithdrawInfoBiz()
    {
        $profit = new MicroShopService();
