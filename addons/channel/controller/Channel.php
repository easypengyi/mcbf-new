...
//        var_dump($condition);exit;
    ...
    public function updateChannelGrade()
    {
        ...
        //平二级奖
        $flat_second = request()->post('flat_second', 0);
        if(!empty($flat_second)){
            $post_data['flat_second'] = $flat_second/100;
        }

        //平三级奖
        $flat_third = request()->post('flat_third', 0);
        if(!empty($flat_third)){
...
//        echo '<Pre>';print_r($channel_grade_list['data']);exit;
            foreach($channel_grade_list['data'] as $k => $v){
                ...
                if($auto_upgrade == '1'){
                    ...
                    if(!empty($upgrade_val_arr['goods_id'])){
                        ...
                        $goods_arr = $goods_server->getGoodsDetailById($upgrade_val_arr['goods_id'],'goods_name');
    ...
    public function getChannelGradeGoods(){
        ...
        $condition['g.goods_type'] = ['not in', [0, 3, 4, 5,6]];//0-计时计次 3-虚拟商品 4-知识付费 5-电子卡密 6预约
...
//        echo $cart_mdl->getLastSql();exit;
    ...
    public function channelCartAdjustNum()
    {
        $buy_type = request()->post('buy_type','');
        $sku_id = request()->post('sku_id',0);
        $num = request()->post('num',0) <= 0 ? 1 : request()->post('num',0);
        $channel_info = request()->post('channel_info',0);
        if (!$buy_type || !$sku_id || !$channel_info) {
            return json(AjaxReturn(LACK_OF_PARAMETER));
        }
...
//            $order_info['channel_info'] = $k;
        ...
        $shop_name = $websiteService->getWebMallName($this->website_id);
        ...
        if($order_id>0){
            ...
            //加入延时队列处理采购订单超时未支付关闭
            $config_info = $channel_server->getChannelConfig($website_id);
            if (!empty($config_info)) {
                $close_time = $config_info['channel_order_close_time'];
            } else {
                $close_time = 1; //默认1分钟
            }
            $this->createChannelOrderCloseDelay($order_id, $close_time);
            //支付订单号qd拼接，用于区分是渠道订单
            return [
                'code' => 0,
                'message' => "提交成功",
     ...
     * @param $close_time
     */
    public function createChannelOrderCloseDelay($order_id, $close_time)
    {
        if(config('is_high_powered')){
            ...
            $back_url = $url.'/rabbitTask/channelOrdersClose';
            $custom_type = 'channel_order_close';
...
//        echo json_encode($result);exit;
    ...
    public function applayChannelForm()
    {
        ...
        $info = $base_config->getConfig(0,'CHANNEL', $this->website_id, 1);
