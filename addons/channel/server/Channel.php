...
class Channel extends BaseService
{
    ...
    public function addChannelConfig($post_data, $is_use)
    {
        ...
        //看设置是否存在数据中
        $ConfigService = new AddonsConfigSer();
        $is_channel_setting = $ConfigService->getAddonsConfig('channel', $website_id);
        if($is_channel_setting){
            $retval = $ConfigService->updateAddonsConfig($post_data, "渠道商设置", $is_use, "channel");
        }else{
            $retval = $ConfigService->addAddonsConfig($post_data, "渠道商设置", $is_use, "channel");
        }
        return $retval;
    ...
    public function getChannelConfig($website_id = 0)
    {
        ...
        $channel_setting_arr = $addonsConfigSer->getAddonsConfig('channel', $website_id);
        $return_data = $channel_setting_arr['value'];
        $return_data['is_use'] = $channel_setting_arr['is_use'];
        $goods_info = $this->goods_ser->getGoodsDetailById($return_data['condition']['goods_id'], 'goods_name,picture', 1);
        $return_data['condition']['pic'] = $goods_info['album_picture']['pic_cover_mid'];
    ...
    {
        $goods_info = $this->goods_ser->getGoodsDetailById($goods_id, 'goods_name,picture', 1);
        $pic_url = $goods_info['album_picture']['pic_cover_mid'];
        return [
    ...
    public function getAgreementSite($website_id){
        ...
        $globalbonus_info = $config->getConfig(0,"CHANNEL", $website_id, 1);
    ...
    public function getPurchaseGoodsSkuInfo($page_index=1, $page_size, $condition, $order, $purchase_discount = 1){
        $website_id = $this->website_id;
        $goods_mdl = new VslGoodsSkuModel();
        ...
        foreach($goods_sku_arr as $k=>$gs){
            $sku_id = $gs['sku_id'];
            $goods_list = $goods_mdl->getInfo(['sku_id' => $sku_id], 'stock, price');
    ...
    public function getChannelDetail($condition)
    {
        ...
        if (getAddons('customform', $this->website_id)) {
            ...
            if ($addinfo['value']){
                ...
                $info = $customSer->getCustomData(1,10,7,'',['nm.channel_id'=>$channel_info['channel_id']]);
                $channel_info['custom_list'] = $info;
            }
        }
        return $channel_info;
    }
...
//        p($member_info);
        ...
        if($channel_peers>=2 && !empty($my_down_channel['two_grade'])){
            foreach($my_down_channel['two_grade'] as $k=>$user_id){
                ...
                //判断和上一级是否等级连续, 不连续不算平二级 A-B-C-D B、C的等级要相等。
                $last_channel_list = $channel_mdl->alias('c')->field('c.uid, cl.weight')->join('vsl_channel_level cl', 'c.channel_grade_id=cl.channel_grade_id')->where(['uid' => $my_down_channel['one_grade'][0]['referee_id']])->find();
                if($last_channel_list && $last_channel_list['weight'] != $channel_list['weight']){//只有上两级的等级相等并且连续才可以分平二级奖
                    $channel_grade = $channel_list['weight'];
                    if($my_grade_weight==$channel_grade){
            ...
            if($channel_peers == 3 && !empty($my_down_channel['three_grade'])){
                foreach($my_down_channel['three_grade'] as $k=>$user_id){
                    ...
                    $condition_channel['c.website_id'] = $website_id;
                    $channel_list = $channel_mdl->alias('c')->field('c.uid, cl.weight')->join('vsl_channel_level cl', 'c.channel_grade_id=cl.channel_grade_id')->where($condition_channel)->find();
                    //判断和上二级是否等级连续, 不连续不算平三级  A-B-C-D  A、B、C的等级要相等。
                    $last_channel_list_one = $channel_mdl->alias('c')->field('c.uid, cl.weight')->join('vsl_channel_level cl', 'c.channel_grade_id=cl.channel_grade_id')->where(['uid' => $my_down_channel['one_grade'][0]['referee_id']])->find();
                    $last_channel_list_two = $channel_mdl->alias('c')->field('c.uid, cl.weight')->join('vsl_channel_level cl', 'c.channel_grade_id=cl.channel_grade_id')->where(['uid' => $my_down_channel['two_grade'][0]['referee_id']])->find();
                    if($last_channel_list_one && $last_channel_list_two && $last_channel_list_one['weight'] == $last_channel_list_two['weight'] && $last_channel_list_two['weight'] == $channel_list['weight']){
                        $channel_grade = $channel_list['weight'];
                        if($my_grade_weight==$channel_grade){
...
//                ->join('sys_user u','m.referee_id=u.uid','LEFT')
    ...
    public function getChannelSkuStore($sku_id, $channel_info, $website_id = 0)
    {
        ...
        if($channel_info == 'platform'){
            $goods_mdl = new VslGoodsSkuModel();
            $sku_list = $goods_mdl->getInfo(['sku_id' => $sku_id], 'stock, price');
        }else{
...
//            echo $goods_mdl->getLastSql();exit;
    ...
    public function isChannelgPlatform($goods_id, $sku_id, $channel_info){
        $goods_list = $this->goods_ser->getGoodsDetailById($goods_id, 'max_buy');
//        echo $goods_mdl->getLastSql();exit;
        ...
        try {
            ...
            foreach ($cart_lists as $channnel_info=> $cart) {
                if($cart['channel_info'] == 'platform'){
                    $goods_info = $goodsSer->getGoodsDetailById($cart['goods_id']);
                }else{
...
//                        var_dump($channel_goods_list);var_dump($channel_goods_sku_list);exit;
                        ...
                        if($og['channel_info'] == 'platform'){
                            $goods_sku_mdl = new VslGoodsSkuModel();
                            $who_goods_list = $this->goods_ser->getGoodsDetailById($og['goods_id']);
                        ...
                        $platform_sku_stock = $sku_mdl->getInfo(['sku_id' => $og['sku_id']], 'stock')['stock'] ? : 0;
                        //插入goods_sku表
                        if($channel_goods_sku_list){
                            ...
                            //插入渠道商redis库存
                            $channel_key = 'channel_'.$channel_info['channel_id'].'_'.$og['sku_id'];
                            $only_channel_key = 'only_channel_'.$channel_info['channel_id'].'_'.$og['sku_id'];
                            $only_platform_key = 'only_platform_'.$channel_info['channel_id'].'_'.$og['sku_id'];
                            $redis->set($channel_key, $channel_goods_sku_list['stock'] + $og['num'] + $platform_sku_stock);//这里加上平台库存，是因为零售是：上级渠道商不够扣平台的。
                            $redis->set($only_channel_key, $channel_goods_sku_list['stock'] + $og['num']);
                            $redis->set($only_platform_key, $platform_sku_stock);
...
//                        echo '<pre>';print_r($goods_sku_arr);exit;
                            ...
                            //插入渠道商redis库存
                            $channel_key = 'channel_'.$channel_info['channel_id'].'_'.$og['sku_id'];
                            $only_channel_key = 'only_channel_'.$channel_info['channel_id'].'_'.$og['sku_id'];//用于当零售的商品库存同时有渠道商的和上级的时候，购买商品库存更新区分
                            $only_platform_key = 'only_platform_'.$channel_info['channel_id'].'_'.$og['sku_id'];
                            $redis->set($channel_key, $og['num'] + $platform_sku_stock);
                            $redis->set($only_channel_key, $og['num']);
                            $redis->set($only_platform_key, $platform_sku_stock);
                        }
                    }
                    //加渠道商的sku销量
...
//        echo '<Pre>';print_r($to_channel_condition);exit;
    ...
    public function addChannelInfo($website_id,$uid,$post_data, $mobile)
    {
        ...
        if($info['channel_condition'] != 'none'){
            return -1; //未开启主动申请
        }
    ...
    * **/
    ...
    public function purchaseOrderList($page_index, $page_size, $condition, $condition1, $condition2, $order)
    {
        ...
        if($condition1['goods_code']) {
            ...
            $goods_list = $this->goods_ser->getGoodsListByCondition($goods_cdt, 'goods_id');
            if($goods_list) {
                $goods_ids =  '';
     ...
     * 更新渠道商的等级
     * **/
    public function pay_updateChannelLevel($buyer_id,$level_id)
    {
        $channel_mdl = new VslChannelModel();
        $channel_level = new VslChannelLevelModel();
        $website_id = $this->website_id;
        $up_grade_condition_list = $channel_level->getInfo(['weight' => $level_id, 'website_id' => $website_id],'channel_grade_id,upgrade_condition,upgrade_val,auto_upgrade');
        $channel_mdl->save(['channel_grade_id' => $up_grade_condition_list['channel_grade_id'], 'upgrade_time' => time(), 'down_upgrade_time' => ''], ['uid' => $buyer_id]);
