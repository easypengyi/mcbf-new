...
class VoucherPackage extends BaseService
{
    ...
    public function getVoucherPackageDetail(array $condition, array $with = [])
    {
        ...
        $data['coupon_type_list'] = $data['gift_voucher_list'] = $data['gift_voucher_id_array'] = $data['coupon_type_id_array'] = $data['coupon_type_id_array_num'] = $data['gift_voucher_id_array_num'] = [];
        if (isset($data['voucher_relation'])) {
            ...
            foreach ($data['voucher_relation'] as $k => $v) {
                if ($v['relation_type'] == 'coupon_type') {
                    ...
                    $data['coupon_type_list'][$v['relation_id']]['num'] = $v['num'];
                    ...
                    $data['gift_voucher_id_array_num'][] = [
                        $v['relation_id'] => $v['num']
                    ];
                }
            }
    ...
    public function addVoucherPackage(array $input)
    {
        ...
        try {
            ...
            $coupon_type_id_array_num = $input['coupon_type_id_array_num'];
            $gift_voucher_id_array_num = $input['gift_voucher_id_array_num'];
            $gift_num = [];
            $coupon_num = [];
            ...
            foreach ($coupon_type_id_array_num as $k3 => $v3) {
                foreach ($v3 as $k4 => $v4) {
                    $coupon_num[$k4] = $v4;
                }
            }
            unset($input['gift_voucher_id_array'], $input['coupon_type_id_array'], $input['coupon_type_id_array_num'], $input['gift_voucher_id_array_num']);
     ...
     */
    public function userAchieveVoucherPackage($uid, $voucher_package_id,$order_id='')
    {
        ...
        if ($voucher_package_info) {
            ...
            foreach ($voucher_package_info->voucher_relation as $v) {
                if($v->relation_type == 'coupon_type' && $is_coupon_type){
                    if($v->num <= 0){
                        continue;
                    }
                    for ($i=0; $i < $v->num; $i++) { 
                        if($coupon_service->isCouponTypeReceivable($v->relation_id, $uid)){
                            $coupon_service->userAchieveCoupon($uid, $v->relation_id, 7);
                            $received_coupon_type_id[] = $v->relation_id;
                            debugLog($voucher_package_id,'订单完成赠送卷-4-1-1');
                        }
                    }
                    continue;
                }
                if($v->relation_type == 'gift_voucher' && $is_gift_voucher){
                    if($v->num <= 0){
                        continue;
                    }
                    for ($i=0; $i < $v->num; $i++) { 
                        if($coupon_service->isCouponTypeReceivable($v->relation_id, $uid)){
                            $gift_voucher_service->getUserReceive($uid, $v->relation_id, 3,1,$order_id);
                            $received_gift_voucher_id[] = $v->relation_id;
                            debugLog($voucher_package_id,'订单完成赠送卷-4-1-2');
                        }
                    }
                    continue;
                }
    ...
    public function saveVoucherPackageConfig($is_addons)
    {
        ...
        if (!empty($addons_info)) {
            $res = $config_service->updateAddonsConfig('', '券包设置', $is_addons, 'voucherpackage');
        } else {
