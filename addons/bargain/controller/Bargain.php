...
class Bargain extends baseBargain
{
    ...
    public function addBargainConfig()
    {
        ...
        try {
            // $post_data = request()->post();////独立规则暂不需要转义检验
            $post_data = $_POST;
            ...
            $ConfigService = new \data\service\AddonsConfig();
            $bargain_info = $ConfigService->getAddonsConfig('bargain', $this->website_id);
            if (!empty($bargain_info)) {
                $res = $ConfigService->updateAddonsConfig($post_data, "砍价设置", $is_bargain, "bargain");
            } else {
                $res = $ConfigService->addAddonsConfig($post_data, "砍价设置", $is_bargain, "bargain");
            }
            if($res){
...
//        var_dump($_POST);exit;
    ...
    public function bargainClose()
    {
        ...
        if($res){
            //去掉加入redis的库存
            $bargain_key = 'bargain_'.$bargain_id.'*';
            $redis->del($bargain_key);
            $this->addUserLog('关闭砍价活动',$bargain_id);
        }
        return ajaxReturn($res);
    }
    /*
    ...
    public function myActionBargain()
    {
        ...
        if($bargain_uid == $uid){
            ...
            $is_help_condition['br.bargain_status'] = ['neq', 2];//必须是未支付的
            $is_help_condition['br.order_id'] = ['eq', 0];//未提交订单
...
//                        $my_bargain_price = $bargain_list['first_bargain_money'] > $my_bargain_price ? $my_bargain_price : $bargain_list['first_bargain_money'];
            ...
            $detail_condition['br.bargain_status'] = ['neq', 2];
            $detail_condition['br.order_id'] = ['eq', 0];
...
//            p($bargain_detail_list);exit;
            ...
            foreach ($goods_data['sku_list'] as $k => $sku) {
                ...
                $temp_sku['max_buy'] = $limit_buy?:0;
                if($limit_buy != 0){
                    $max_buy = $limit_buy - $buy_num;
                    $temp_sku['stock_num'] = $max_buy > 0 ? ($max_buy > $bargain_stock ? $bargain_stock : $max_buy) : 0;
                }else{
                    $temp_sku['stock_num'] = $bargain_stock ? :0;
                }
            ...
            if(!$is_help_bargain){
                ...
                if($this->uid != $bargain_record_list['uid']){//登录的uid和参加活动的uid不能相同
                    ...
                    if (!$is_other_bargain && !$is_my_bargain && $first_bargain_money > 0) {//是否设置了第一刀价格 并且没有设置自己砍价 并且没有人砍过，就砍设置的价格
                        ...
                        $help_bargain_arr1['helper_ip'] = $_SERVER['HTTP_X_FORWARDED_FOR'] ? : $_SERVER['REMOTE_ADDR'];
...
//                            $bargain_money = round(rand($bargain_record_list['rand_lowest_money'], $bargain_record_list['rand_highest_money']), 2);
                        ...
                        $help_bargain_arr['helper_ip'] = $_SERVER['HTTP_X_FORWARDED_FOR'] ? : $_SERVER['REMOTE_ADDR'];
