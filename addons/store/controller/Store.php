...
//        }
        ...
        $img = '';
        $data['is_commission'] = request()->post('is_commission', 0);
        $data['cash_method'] = request()->post('cash_method', 0);
        $data['write_off_method'] = request()->post('write_off_method', 0);
        $data['cash_reward'] = request()->post('cash_reward', 0);
        $data['write_off_reward'] = request()->post('write_off_reward', 0);
        if ($data['imageArray']) {
    ...
    public function getAllStoreListForWap()
    {
        ...
        $count = count($list['store_list']);
        $data = [];
        $page_count = ceil($count/$page_size);
        $offset = ($page_index-1)*$page_size;
        for ($i = $offset; $i < $offset + $page_size; $i++) {
            if ($list['store_list'][$i]) {
                $data[] = $list['store_list'][$i];
            }
        }
        $list['store_list'] = $data;
    ...
    public function getStoreGoodsCategoryList()
    {
        ...
        $storeServer = new storeServer();
        $stock_type = (int)$storeServer->getStoreSet($shop_id)['stock_type'] ?: 1;
    ...
    public function getStoreGoods()
    {
        ...
        $storeServer = new storeServer();
        $goods_info = $storeServer->getStoreGoods($page_index, $page_size, $condition,0,$store_id,$shop_id);
...
//                $full_cut_condition['end_time'] = ['>=', time()];
            ...
            if ($is_group && !$mic_goods) {
                ...
                $group_list['regiment_count'] = !empty($regimentCount) ? $regimentCount['now_num'] : 0;
            }
        ...
        if (!empty($cartlist['cart_list'])) {
            foreach ($cartlist['cart_list'] as $i => $v) {
                ...
                if ($this->is_discount && !empty($discount_info) && $cartlist['cart_list'][$i]['promotion_type'] == 5) {
                    ...
                    $discount_info = $discount_server->getBestDiscount($v['shop_id']);
                    $cartlist['cart_list'][$i]['discount_info'] = !empty($discount_info) ? $discount_info : (object)array();
            ...
            foreach ($cartlist['cart_list'] as $k => $v) {
                ...
                if ($v['discount_info'] && is_array($v['discount_info'])) {
                    if ($v['discount_info']['range'] == 2) {
                        $is_active_goods = $discount_server->checkIsDiscountProduct($v['goods_id'], $v['shop_id']);
    ...
    public function orderInfo()
    {
        ...
        if ($order_tag == 'cart') {
            ...
            foreach ($cart_info as $v) {
                ...
                //查询出当前商品是否在活动中
                $goods_id = $v['goods_id'];
                $goods_info = $goodsSer->getGoodsDetailById($goods_id, 'goods_id,goods_name,price,promotion_type');
...
//                }
            ...
            foreach ($shop_info['goods_list'] as $key => $val) {
                $store_list = $goodsSer->getGoodsDetailById($val['goods_id'], 'goods_id,goods_name,price,store_list');
                if(!$store_list){
                    $has_store=0;
                    break;
                }
...
//        return $return_data;
    ...
    public function getWithdrawCount()
    {
        ...
        $order_count_array['make'] = $order->getMemberWithdrawalCount(['status' => 4, 'website_id' => $this->website_id]);
        $order_count_array['makefail'] = $order->getMemberWithdrawalCount(['status' => -3, 'website_id' => $this->website_id]);
        $order_count_array['nomake'] = $order->getMemberWithdrawalCount(['status' => -2, 'website_id' => $this->website_id]);
    ...
    public function commissionWithdrawList()
    {
        ...
        $commission = new storeServer();
        $condition = array('website_id' => $website_id);
        $search_text = request()->post('search_text', '');
        $condition['assistant_name'] = [
            'like',
            '%' . $search_text . '%'
        ];
        if ($status != '' && $status != 9) {
        ...
        if ($withdraw_no != '') {
            $condition['withdraw_no'] = $withdraw_no;
        }
        ...
        $condition["add_time"] = [[">", $start_date], ["<", $end_date]];
