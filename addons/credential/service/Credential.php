    ...
    public function getCredImg($uid, $wchat_name, $cred_info)
    {
        try{
            ...
            if(config('project') == 'shopvslai'){
                $qr_url = $domain_name.'/wap/packages/credential/result?cred_no='.$cred_no;
            }else{
            ...
            if(!empty($data_arr['bg'])){
                ...
                if ($image->width() != $default_cred['default_width'] || $image->height() != $default_cred['default_height']) {
                    ...
                    $new_height = ($scale > ($width_scale + 0.1)) ? $image->height() * $default_cred['default_height'] / $image->width() : $default_cred['default_height'];
            ...
            if($cred_info['cred_type'] == 1){//分销商
                ...
                $channel_info = $channel->alias('c')->where(['uid'=>$uid])->field('channel_grade_name')->join('vsl_channel_level cl', 'c.channel_grade_id = cl.channel_grade_id', 'left')->find();
                    ...
                    switch($v['type']){
                        ...
                        case 'qr':
                            if(config('project') == 'shopvslai'){
                                ...
                                $mp_result = $mpController->getUnLimitMpCode($params);
                                if ($mp_result['code'] > 0) {
