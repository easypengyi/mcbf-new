...
class Gift extends BaseService {
    ...
    public function saveGiftConfig($is_gift)
    {
        ...
        if (!empty($giftInfo)) {
            $res = $ConfigService->updateAddonsConfig('', "赠品设置", $is_gift, "gift");
        } else {
     ...
     * 赠品记录
     */
    public function ordergiftRecord($page_index = 1, $page_size = 0, array $condition = []){
        $giftModel = new VslGiftVoucherRecordsModel();
        $list = $giftModel->pageQuery($page_index, $page_size, $condition,'fetch_time desc', '*');
        if($list['data']){
            $orderModel = new \data\model\VslOrderModel();
            $userModel = new \data\model\UserModel();
            foreach ($list['data'] as $k => $v) {
                if ($v['order_id']) {
                    $order_info = $orderModel->getInfo(['order_id'=>$v['order_id']], 'order_no');
                    $list['data'][$k]['no'] = $order_info ? $order_info['order_no'] : '';
                }
                $user_info = $userModel->getInfo(['uid' => $v['uid']], 'nick_name,user_name,user_tel,user_headimg');
                if ($user_info['user_name']) {
                    $list['data'][$k]['user_tel'] = $user_info['user_name'];
                } elseif ($user_info['nick_name']) {
                    $list['data'][$k]['user_tel'] = $user_info['nick_name'];
                } elseif ($user_info['user_tel']) {
                    $list['data'][$k]['user_tel'] = $user_info['user_tel'];
                }
                $list['data'][$k]['num'] = 1;
                $list['data'][$k]['create_time'] = date('Y-m-d H:i:s',$v['fetch_time']);
            }
        }
        return $list;
    }
