...
class Gift extends Addons {
    ...
    public function ordergiftRecord() {
        $giftId = request()->get('gift_id');
        $giftServer = new giftServer();
        $giftDetail = $giftServer->giftDetail($giftId);
        $this->assign('giftId', $giftId);
        $this->assign('giftDetail', $giftDetail);
        $this->assign('ordergiftRecordUrl', __URL(call_user_func('addons_url_' . $this->module, 'gift://Gift/ordergiftRecord')));
        $this->fetch('template/' . $this->module . '/ordergiftRecord');
    }
    /*
     * 赠品设置
     */
    public function giftSetting()
    {
        ...
        $giftInfo = $addonsConfigSer->getAddonsConfig('gift', $this->website_id, 0);
