...
class Gift extends baseGift {
    ...
    public function modalGiftGoodsList()
    {
        ...
        $goodsSer = new Goods();
        $list = $goodsSer->getPageGoodsList($index, PAGESIZE, $condition, 'create_time desc', 'goods_id,goods_name,description,price,img_id_array,picture');
        if( !empty($list['data']) ){
...
//            'vmg.shop_id' => $this->instance_id,
    ...
    public function ordergiftRecord() {
        ...
        $condition = array(
            ...
            'gift_voucher_id' => $gift_id,
        );
        $list = $giftServer->ordergiftRecord($page_index, $page_size, $condition);
        return $list;
    }
}
