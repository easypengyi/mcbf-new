...
class Paygift extends basePaygift
{
        ...
        if($list['data']){
            foreach($list['data'] as $k => $v){
                ...
                if($v['modes']==2){
                    ...
                    $goods = $goodsSer->getGoodsDetailById($v['modes_id'], 'goods_id,goods_name');
    ...
    public function addPaygift()
    {
        ...
        if($res){
            ...
            if($delay_time1 > 0){
                $this->smallPromotionOpenOrClose('pay_gift_id', $res, 'pay_gift', $delay_time1);
            }
            $delay_time2 = $input['end_time'] - time();
            if($delay_time2 > 0){
                $this->smallPromotionOpenOrClose('pay_gift_id', $res, 'pay_gift', $delay_time2);
            }
