...
//        $this->website_id = request()->post('website_id', 0);
    ...
    public function shopList()
    {
        ...
        foreach ($list['data'] as $k => $v) {
            $user_name = $username->getUserInfo($v['uid'],'user_tel');
...
//        try {
            ...
            $order_sort_es = 'sales+real_sales desc';
            unset($condition);
            $levelCondition = [];
            $supplierCondition = [];
            $levelQueryEs = '';
            if($this->uid) {
                ...
                if (!empty($userLevle)) {
                    ...
                    $levelQuery .= ') ';
                    if ($level_flag){
                        $levelCondition = ['exp', $sql1 . $sql2];
                    }
                    if($levelQuery !== '()'){
                        $levelQueryEs = ' and ' . $levelQuery;
                    }
                }
            ...
            if (getAddons('supplier',$this->website_id)){
                $supplierCondition = ['exp', ' if(ng.supplier_info >0, s.status =1, 1)'];//供应商必须为开启
                $levelQueryEs .= ' and (supplier_info = 0 or (supplier_info>0 and supplier_status=1))';
            }
            foreach ($list['data'] as &$v) {
               ...
               $query = 'select goods_id, goods_name, price, max_price, create_time,sales,real_sales, album_picture.pic_cover from '.config('database.database').'_goods where state=1 and for_store=0 and goods_id>0 and shop_id='.$v['shop_id'].' and website_id='.$this->website_id;
                // 获取该用户的权限
                if($levelCondition){
                ...
                if(config('ecs.host')){
                    ...
                    $goods_list = $goodsSer->getWapGoodsEs(4, $query, 0, ['sales+real_sales','goods_id']);
                }else{
                ...
                if($goods_list['data']){
                    foreach ($goods_list['data'] as $goods){
                        ...
                        $temp_goods['logo'] = isset($goods['pic_cover']) ? getApiSrc($goods['pic_cover']) : ($goods['album_picture'] ? getApiSrc($goods['album_picture']['pic_cover']) : '');
                        $temp_goods['price'] = isset($goods['goods_price']) ? $goods['goods_price'] : $goods['price'];
...
//        }
    ...
    public function ajax_shopVerify()
    {
        ...
        $shop = new ShopService();
        $retval = $shop->dealwithShopApply($shop_apply_id, $type, $shop_platform_commission_rate, $margin, $shop_audit, $refuse_reason, $shop_type);
        return AjaxReturn($retval);
    }
     ...
     * 店铺账户列表导出
     */
    public function shopAccountListDataExcel() 
    {
        $xlsName = "店铺账户列表";
        $xlsCell = array(
            array(
    ...
    public function shopAccountWithdrawAudit()
    {
        ...
        if (isset($retval['code'])){
            $this->addUserLog(self::$port_name.'提现审核不通过', $retval['message']);
            return $retval;
        }
        if($retval){
            if($status==-1){
        ...
        if (isset($retval['code'])){
            $this->addUserLog(self::$port_name.'提现审核不通过', $retval);
            return $retval;
        }
        if($is_biz){
    ...
    public function addPlatformShop()
    {
        ...
        $shopInfo['merchants_uid'] = request()->post('merchants_uid', 0);//招商员id
        if($shop_id==='0'){
            $shopInfo['shop_qq'] = request()->post('shop_qq', 0);
            $shopInfo['shop_phone'] = request()->post('shop_phone', 0);
            $shopInfo['shop_logo'] = request()->post('shop_logo', 0);
            $shopInfo['shop_hide'] = request()->post('shop_hide', 0);
        }
        ...
        $shopApplyInfo['joinpay_machid'] = request()->post('joinpay_machid', '');
        $shopInfo['joinpay_machid'] = request()->post('joinpay_machid', '');
        $shop = new ShopService();
        
        if ($shop_id || $shop_id==='0') {/*修改店铺*/
            $add_type = 1;//更新
            ...
            //获取会员信息
            $user = new User();
            $result = $user->getUserInfo($uid, 'user_tel,user_password');
            if($shopInfo['user_account']){
                $check = $user->getUserCount(['user_tel' => $shopInfo['user_account'], 'website_id' => $this->website_id, 'uid' => ['<>', $uid]]);
                if($check){
                    return json(['code' => -1,'message' => '手机号已存在,请重新输入']);
                }
            }
        ...
        if($res > 0){
            //新增店铺装修初始化
            if ($res >0) {
                $shop->addShopAndCreateCustomTemplate($res, $shopInfo['shop_type'],$add_type);
            }
    ...
    public function applyForWap(){
        ...
        $merchants_uid= input('post.referee_uid') ? : 0;//招商员uid
        $uid = $this->user->getSessionUid();
        $retval = $shop->addShopApply($apply_type, $uid, $company_name, $company_province_id, $company_city_id, $company_district_id, $company_address_detail, $company_phone, $company_type, $company_employee_count, $company_registered_capital, $contacts_name, $contacts_phone, $contacts_email, $contacts_card_no, $contacts_card_electronic_1, $contacts_card_electronic_2, $contacts_card_electronic_3, $business_licence_number, $business_sphere, $business_licence_number_electronic,  $shop_name, $apply_state, $apply_message, $apply_year, $shop_group_name, $shop_group_id, $paying_money_certificate, $paying_money_certificate_explain, $paying_amount, 0, $post_data, $merchants_uid);
        return json(AjaxReturn($retval));
    }
    /*
     * 移动端获取店铺协议
...
//        $join['key'] = '入驻协议';
     ...
     * 移动商家端店铺提现详情
     * **/
    public function shopAccountWithdrawInfoBiz()
    {
        $shop = new ShopService;
        ...
        $retval = $shop->shopAccountWithdrawDetail($id);
        unset($retval['modify_date'],$retval['service_charge'],$retval['platform_money'],$retval['remark'],$retval['shop_id'],$retval['uid']);
        return json(['code' => 1, 'message' => '获取成功', 'data' => (object)$retval]);
    }
    /**
     * 移动商家端店铺提现列表
     * @return Ambigous <\think\response\View, \think\response\$this, \think\response\View>
     */
    public function shopAccountWithdrawListBiz()
    {
        ...
        $list = $shop->getShopAccountWithdrawList($page_index, $page_size, $condition, $shop_name, 'ask_for_date desc');
        $list['withdraw_list'] = [];
        if($list['data']){
            foreach($list['data'] as $key => $val){
                $list['withdraw_list'][$key]['id'] = $val['id'];
                $list['withdraw_list'][$key]['withdraw_no'] = $val['withdraw_no'];
                $list['withdraw_list'][$key]['type'] = $val['type'];
                $list['withdraw_list'][$key]['shop_id'] = $val['shop_id'];
                $list['withdraw_list'][$key]['status'] = $val['status'];
                $list['withdraw_list'][$key]['shop_name'] = $val['shop_name'];
     ...
     * 获取分账列表 
     */
    public function shopSeparateList(){
        $page_index = request()->post('page_index', '');
        $page_size = request()->post('page_size', '');
        $website_id = request()->post('website_id', '');
        $search_text = request()->post('search_text', '');
        $shop = new ShopService();
        $retval = $shop->getShopSeparateCountList($page_index, $page_size, ['website_id' => $website_id], '', $search_text);
        return $retval;
    }
    /**
     * 确认分账 手动发起分账请求
     */
    public function shopSeparateSend(){
        $id = request()->post('id', '');
        if(empty($id)){
            return json([
                'code' => -1,
                'message' => '操作失败，分账订单不能为空'
            ]);
        }
        //需要添加并发锁，防止重复操作 --  

        $shop = new ShopService();
        $retval = $shop->shopSeparateSend($id);
        return json($retval);
    }
}
