...
class ShopAccount extends BaseService
{
     ...
     * @param unknown $shop_id
     */
    public function addShopAccountData($shop_id, $number, $data_id,$is_examine='',$make_money='',$wx_openid='',$withdraw_no='',$type='',$account_number='',$service_charge = 0,$charge = 0,$platform_money = 0, $real_name = '')
    {
        # $is_examine 提现自动审核：1自动 2人工
        # $make_money 提现自动打款：1自动 2人工
        # $type 提现方式：1银行卡(自动提现) 2微信 3支付宝 4银行卡(手动提现)
     ...
     * @param unknown $shop_id
     */
    public function addAuditShopAccountData($platform_money,$charge,$service_charge,$data_id,$shop_id,$number,$uid,$withdraw_no,$type,$account_number='',$real_name = '')
    {
     ...
     * @param        $platform_money        [平台抽取]
     * @param        $charge                [手续费]
     * @param        $service_charge        [实际到账金额]
     * @param        $number                [流水金额]
     * @param        $data_id               [提现记录id]
     * @param        $shop_id               [店铺id]
     * @param        $status                [-1审核不通过 0自动审核，待打款 1待审核 2支付宝打款成功 3微信打款成功 4拒绝打款 5打款失败 6银行卡打款成功 7手动审核通过，待打款]
     * @param        $text                  [描述]
     * @param string $msg
    ...
    public function addShopAccountRecords($platform_money,$charge,$service_charge,$number,$data_id,$shop_id,$status,$text,$msg=''){
        ...
        if($status==7){//手动审核通过，待打款
            ...
            $retval = $balance_withdraw->save($withdraw_data,['id'=> $data_id,'website_id' => $this->website_id]);
            return $retval;
        }
        $shop_account = new VslShopAccountModel();
        $all_info = $shop_account->getInfo(['shop_id'=> $shop_id,'website_id'=>$this->website_id], '*');
...
//        p($withdraw_info,'$withdraw_info');die;
        $params['shop_id'] = 0;
        $params['takeoutmoney'] = abs($number);
        ...
        if($withdraw_info['type']==3){
            $alipay_pay = new AliPay();
            $retval = $alipay_pay->aliPayTransferNew($withdraw_info['withdraw_no'],$withdraw_info['account_number'],abs($service_charge),$withdraw_info['realname']);
        }
        if($withdraw_info['type']==2){
            $weixin_pay = new WeiXinPay();
            $search = '/^0?1[3|4|5|6|7|8|9][0-9]\d{8}$/';
            if(preg_match( $search, $withdraw_info['account_number'])){
                $user = new UserModel();
                $member_info = $user->getInfo(['user_tel'=>$withdraw_info['account_number'],'is_member'=>1,'website_id'=>$this->website_id],'wx_openid');
                $wx_openid = $member_info['wx_openid'];
            }else{
                $wx_openid = $withdraw_info['account_number'];
            }
            
            $retval = $weixin_pay->EnterprisePayment($wx_openid,$withdraw_info['withdraw_no'],'',abs($service_charge),'店铺余额微信提现,手动打款',$this->website_id);
            
        }
        if($withdraw_info['type']==1|| $withdraw_info['type']==4){/*其实这里只能是4，因为C端提现申请只有提现到银行（4），没有提现到余额（1）*/
            $bank = new VslShopBankAccountModel();
            ...
            if (isset($retval['code'])&&$retval['code']<0){
        ...
        if($retval['is_success']==1){//打款成功
            ...
            return AjaxReturn(FAIL,[],$retval['msg']);
        }
    }
    /*
     * 同意打款（手动打款）
    ...
    public function addAgreeShopAccountDatas($service_charge,$data_id,$shop_id,$number,$text){
        ...
        if($retval['is_success']==1){//打款成功
            ...
            $retval['msg'] = '手动打款失败';
            if($withdraw_info['type']==1 || $withdraw_info['type']==4) {
                $addRes = $this->addShopAccountRecords($withdraw_info['platform_money'],$withdraw_info['charge'],$service_charge,$number, $data_id, $shop_id, 5, '店铺余额银行卡提现，手动打款失败', $retval['msg']);
            }
            if($withdraw_info['type']==2) {
                $addRes = $this->addShopAccountRecords($withdraw_info['platform_money'],$withdraw_info['charge'],$service_charge,$number, $data_id, $shop_id, 5, '店铺余额微信提现，手动打款失败', $retval['msg']);
            }
            if($withdraw_info['type']==3) {
                $addRes = $this->addShopAccountRecords($withdraw_info['platform_money'],$withdraw_info['charge'],$service_charge,$number, $data_id, $shop_id, 5, '店铺余额支付宝提现，手动打款失败', $retval['msg']);
            }
            if (strlen($addRes) > 2){
                $retval['msg'] = $addRes;
            }
    ...
    public function getShopAccount($shop_id)
    {
        ...
        // 店铺提现中
        $withdraw = new VslShopWithdrawModel();//TODO... 优化查询
        ...
        // 店铺已提现总额
        $shop_withdraw = $withdraw->getSum(['shop_id'=>$shop_id,'status'=>3,'website_id'=>$this->website_id],'cash');
        $account_obj["shop_withdraw"] = abs($shop_withdraw);
        // 店铺可用总额 （店铺入账金额 -  店铺已提现总额 - 平台抽成 - 退款处理中的退款金额 - 店铺提现中的金额 ）
