...
class Shop extends Addo
{
    ...
    public $menu_info = array(
        [
            ...
            'icon_class' => 'icon-shops', // 字体图标class（一般为空）
        ...
        [
            ...
            'is_menu' => 1, // 是否是菜单
            'is_dev' => 0, // 是否是开发模式可见
            'desc' => '入驻店铺分账订单信息。', // 菜单描述
            'module_picture' => '', // 图片（一般为空）
            'icon_class' => '', // 字体图标class（一般为空）
            'is_control_auth' => 1, // 是否有控制权限
            'hook_name' => 'shopAccountSeparateList'
        ],
        [
            'module_name' => '店铺列表',
    ...
    ;
     public function __construct(){
        ...
        $this->assign('shopAccountistDataExcelUrl', __URL(addons_url_platform('shop://shop/shopAccountListDataExcel')));
        ...
        $pc_info = $ConfigService->getAddonsConfig('pcport',$this->website_id, 0, 1);
    ...
    public function updateShopLevel(){
        ...
        $module_list_two=$web_site->getSystemModuleList(1,0,['module'=>'admin','level'=>2,'module_id'=>['in',$first_list]],'sort');
        $module_list_three=$web_site->getSystemModuleList(1,0,['module'=>'admin','level'=>3,'module_id'=>['in',$first_list]],'sort');
        $module_list_four   = $web_site->getSystemModuleList(1,0,['module'=>'admin','level'=>4,'module_id'=>['in',$first_list]],'sort');
        $this->assign('module_list_one',$module_list_one['data']);
        $this->assign('module_list_two',$module_list_two['data']);
        $this->assign('module_list_three',$module_list_three['data']);
        $this->assign('module_list_four',$module_list_four['data']);
    ...
    public function updateShop(){
        ...
        $user = new User();
        $user_info = $user->getUserInfo($info['base_info']['uid'],'user_tel');
    ...
    public function addPlatformShop(){
            $shop = new ShopService();
            $merchants_uid = $_GET['merchants_uid']?:0;//招商员id
            $merchants_from = $_GET['merchants_from']?:0;//1:招商员列表点击添加店铺 2：招商部列表点击添加店铺
            $group_list = $shop->getShopGroup(1, 0,['website_id'=> $this->website_id], 'group_sort ASC');
            $this->assign('group_list', $group_list['data']);
            $type_list = $shop->getShopTypeList(1, 0,['website_id'=> $this->website_id]);
            $this->assign('type_list', $type_list['data']);
            $this->assign('merchants_uid', $merchants_uid);
            $this->assign('merchants_from', $merchants_from);
            $this->fetch('template/platform/addShop');
    }
    /**
     * 店铺账目列表
    ...
    public function applyIndex($param = array())
    {
        ...
        $user_info = $member->getUserInfo('', 'user_name');
    ...
    public function shopStreet($param=array())
    {
        ...
        $shop_name = isset($param['shop_name']) ? addslashes($param['shop_name']) : ''; // 店铺名称
        $shop_group_id = isset($param['shop_group_id']) ? (int)$param['shop_group_id'] : 0; // 店铺分类
        $order_type = isset($param['order_type']) ? (int)$param['order_type'] : 0; // 排序类型 为1销售排行2信誉排行
        $sort = isset($param['sort']) ? $param['sort'] : 'asc'; // 倒排正排
        if(strlen($sort) > 4) {
            //防sql注入
            $sort = 'asc';
        }
        $page = isset($param['page']) ? (int)$param['page'] : '1'; // pageindex
    ...
    public function shopIndex($param=array())
    {
        ...
        $shop_id = intval($param['shop_id']) >= 0 ? intval($param['shop_id']) : 0; //店铺id
    ...
    public function shopGoodList($param=array())
    {
        ...
        $order_type = isset($param['order_type']) ? (int)$param['order_type'] : ''; // 1销量2价钱3评论
        $sort = isset($param['sort']) ? $param['sort'] : 'asc'; // 倒排正排
        $shop_id = intval($param['shop_id']) >= 0 ? intval($param['shop_id']) : 0; //店铺id
        if(strlen($sort) > 4) {
            //防sql注入
        ...
        if(getAddons('discount', $this->website_id)){
            //首页限时折扣限时
            $discountSer = new \addons\discount\server\Discount();
            $bestDiscount = $discountSer->getBestDiscount($shop_id);
            $discount_num = $bestDiscount['discount_num'];  //折扣
            foreach ($good_list['data'] as $k=>$v){
                //重新赋值活动价格
                if(!empty($discountSer->checkIsDiscountProduct($v['goods_id'],$param['shop_id']))){
                    $good_list['data'][$k]['promotion_price'] = $discountSer->getDiscountPrice($v['price'],$discount_num);
                }
            }
        }
    ...
    public function modalShopList()
    {
        ...
        return $this->fetch('template/platform/shopDialog');
     ...
     * 分账信息
     */
    public function shopAccountSeparateList(){
        $this->fetch('template/platform/shopSeparateList');
    }
}