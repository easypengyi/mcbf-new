...
class Integral extends baseIntegral
{
    ...
    public function addIntegralSetting()
    {
        try {
            ...
            $bargain_info = $AddonsConfig->getAddonsConfig('integral', $this->website_id, 0);
            if (!empty($bargain_info)) {
                $res = $AddonsConfig->updateAddonsConfig('', "积分商城设置", $is_integral_open, "integral");
            } else {
                $res = $AddonsConfig->addAddonsConfig('', "积分商城设置", $is_integral_open, "integral");
            }
            if($res){
...
//        var_dump($res);exit;
    ...
    public function selfIntegralgoodsList()
    {
        if (request()->isAjax()) {
            ...
            $goods_mdl = new VslIntegralGoodsModel();
            // 'ng.sort' => 'desc',
            // 根据商品分组id，查询标签名称
    ...
    public function modifyIntegralGoodsOnline()
    {
        ...
        if(!strstr($condition, ',')){
            $goods_id = $condition;
            $integral_goods_res = $integral_goods->getInfo(['goods_id' => $goods_id], 'goods_exchange_type, gift_voucher_id');
            if($integral_goods_res){
                $integral_goods_arr = objToArr($integral_goods_res);
            }
            $now_time = time();
            if($integral_goods_arr['goods_exchange_type'] == 1){//优惠券
                //判断时间是否过期
                $coupon = new VslCouponTypeModel();
                $coupon_info = $coupon->getInfo(['coupon_type_id' => $integral_goods_arr['coupon_type_id']], 'end_time');
                if ($now_time > $coupon_info['end_time']) {//过期了
                    return ['code'=>-1, 'message' => '当前优惠券已过期'];
                }
            }elseif ($integral_goods_arr['goods_exchange_type'] == 2){//礼品券
                //判断时间是否过期
                $giftvoucher = new VslGiftVoucherModel();
                $giftvoucher_info = $giftvoucher->getInfo(['gift_voucher_id' => $integral_goods_arr['gift_voucher_id']], 'end_time');
                if ($now_time > $giftvoucher_info['end_time']) {//过期了
                    return ['code'=>-1, 'message' => '当前礼品券已过期'];
                }
            }
        }
...
//                $product["point_exchange_type"],
    ...
    public function getMemberBalancePoint()
    {
        ...
        if($account_info){
            ...
            if ($payPassRes) {
                $account_info['cpp'] = $payPassRes['value'] ? 1 : 0;//1关闭
            }
    ...
    public function integralPay()
    {
        ...
        if ($point_exchange_type == 1) {//只用积分
            ...
            if($order_id>0){
                ...
                $integral_order_info = $order_model->getInfo(['order_id' => $order_id], 'order_id, buyer_id');
            ...
            switch ($pay_type) {
                ...
                case '2'://支付宝
                    ...
                    $return_url = $this->realm_ip. "/wap/packages/pay/result?out_trade_no=".$out_trade_no;;
                    if($type==2) {
                        $pay = new UnifyPay();
