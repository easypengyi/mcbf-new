...
use think\helper\Time;
        ...
        public function globalAgentList(){
            ...
            $list = $agent->getagentList($page_index, PAGESIZE, $condition,'become_global_agent_time desc');
            return $list;
        }
        ...
        public function globalBasicSetting()
        {
            ...
            if (request()->isPost()) {
                ...
                $up_team_money = request()->post('up_team_money', ''); // 团队订单金额
                $globalbonus_admin_status = request()->post('globalbonus_admin_status', 0); // 是否开启店铺分红
                $retval = $config->setGlobalBonusSite($globalbonus_status,$agent_condition, $agent_conditions, $pay_money,$number,$one_number,$two_number,$three_number, $order_money,$agent_check, $agent_grade, $goods_id,$agent_data,$up_team_money,$globalbonus_admin_status);
                if($retval){
