...
/*-------------------------------------------------------------------前端钩子开始-----------------------------------------------------------------------*/
    ...
    public function orderGlobalPayCalculate($params)
    {
        ...
        if($bonusLog['global_pay_status'] == 1){
            return true;
        }
        $bonusLog['status'] = 3;
        $global_service->addGlobalBonus($bonusLog);
    }
    /**
     * 订单退款成功后需要重新计算订单的分红
     */
    public function updateGlobalBonusMoney($params)
    {
        ...
        if($bonusLog['global_return_status'] == 1){
            return true;
        }
        $bonusLog['status'] = 2;
        $global_service->addGlobalBonus($bonusLog);
    ...
    public function updateOrderGlobalBonus($params)
    {
        ...
        $bonusLog = $bonusLogModel->getQuery(['website_id'=>$params['website_id'],'order_id'=>$params['order_id'],'global_return_status'=>0, 'global_cal_status' => 0]);
       
        if($bonusLog){
            $newLog = ['status' => 1];
            $global_bonus_arr = []; //合并多个订单商品的分红明细为一条操作
            foreach($bonusLog as $val){
                $newLog['order_id'] = $val['order_id'];
                $newLog['website_id'] = $val['website_id'];
                $newLog['global_bonus'] += $val['global_bonus'];
                $global_bonus_details = json_decode(htmlspecialchars_decode($val['global_bonus_details']),true);
                if(!$global_bonus_details || !is_array($global_bonus_details)){
                    continue;
                }
                foreach($global_bonus_details as $global_bonus_key => $global_bonus_val){
                    if(!isset($global_bonus_arr[$global_bonus_key])){
                        $global_bonus_arr[$global_bonus_key]['bonus'] = $global_bonus_val['bonus'];
                    }else{
                        $global_bonus_arr[$global_bonus_key]['bonus'] += $global_bonus_val['bonus'];
                    }
                }
            ...
            unset($val);
            unset($global_bonus_val);
            // if(!$global_bonus_arr){
            //     return true;
            // }
            if($global_bonus_arr){
                $newLog['global_bonus_details'] = json_encode($global_bonus_arr,true);
                $global_service->addGlobalBonus($newLog);
            }
