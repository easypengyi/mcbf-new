...
class FollowGift extends BaseService
{
    ...
    public function getFollowGiftDetail($condition,$type=0)
    {
        ...
        if($type==1 && $info){
            ...
            if($info['prize_type']==5){
                ...
                $vslgoods = $vsl_goods->getGoodsDetailById($info['prize_type_id'], 'goods_id,goods_name,price,picture', 1);
    ...
    public function saveConfig($is_followgift)
    {
        ...
        if (!empty($info)) {
            $res = $AddonsConfig->updateAddonsConfig('', "关注有礼设置", $is_followgift, "followgift");
        } else {
                ...
                if($v['modes']==1 && $sign==0){
                    ...
                    if($res){
                        ...
                        if(config('project') == 'shopvslai'){
                            $weixin_msg = str_replace("[奖品链接]",'<a href="'.__URLS('/wap/packages/followgift/index').'?followgift_id='.$res.'">点击领取</a>',$weixin_msg);
                        }else{
                            //获取小程序的appid
                            $weixin_auth = new WeixinAuthModel();
                            $mp_cond['website_id'] = $website_id;
                            $mp_cond['shop_id'] = $this->instance_id?:0;
                            $app_id = $weixin_auth->getInfo($mp_cond, 'authorizer_appid')['authorizer_appid'];
                            $weixin_msg = str_replace("[奖品链接]",'<a href="http://www.qq.com" data-miniprogram-appid="'. $app_id .'" data-miniprogram-path="packages/followgift/index?followgift_id='. $res .'">点击领取</a>',$weixin_msg);
                        }
