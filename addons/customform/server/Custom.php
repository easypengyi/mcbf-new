...
class Custom extends BaseService
{
    ...
    public function getCustomFormList($page_index = 1, $page_size = 0, $condition = '', $order = '')
    {
        ...
        $set_data = $custom_setting->getAddonsConfig('customform',$condition['website_id'], 0, 1);
    ...
    {
         $custom_form = new VslCustomModel();
        $custom_data = $custom_form->getInfo(['name'=>$data['name'], 'website_id'=>$this->website_id]);
         if ($custom_data) {
             return -1;
         }
     ...
     * 条件删除数据
     */
    public function deleteCustomFormData ($condition)
    {
        $custom_form = new VslCustomModel();
        return $custom_form->delData($condition);
    }
    ...
    public function saveCustomFormSetting($all)
    {
        ...
        $res = $add_config->getAddonsConfig('customform', $this->website_id, 0, 1);
        $all['member_status']=0;
        $all['order_status']=0;
        ...
        $coupon_info = $add_config->getAddonsConfig('customform', $this->website_id);
        if (!empty($coupon_info)) {
            $res = $add_config->updateAddonsConfig($all, "模板设置", 1, "customform");
        } else {
    ...
    public function getCustomData($page_index,$page_size,$usage,$search_text='',$ext_condition =[]){
        //1:确认订单 2:会员资料 3:分销商申请资料 4:股东申请资料 5:区域代理申请资料 6:队长申请资料 7:渠道商申请资料
        if($usage==1){
            ...
            if ($search_text) {
                $condition['nm.custom_order'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
            $list = $order->getViewList($page_index,$page_size,$condition,'nm.create_time desc');
            if($list['data']){
        ...
        if($usage==2){
            ...
            if ($search_text) {
                $condition['custom_person'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
            $list = $user->pageQuery($page_index,$page_size,$condition,'reg_time desc','custom_person,uid,nick_name,user_name,user_headimg,user_tel');
            if($list['data']){
                $list['data'] = objToArr($list['data']);
                foreach ($list['data'] as $k => $v) {
        ...
        if($usage==5){
            ...
            if ($search_text) {
                $condition['nm.custom_area'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
            $list = $member->getViewList($page_index,$page_size,$condition,'nm.reg_time desc');
            if($list['data']){
                $list['data'] = objToArr($list['data']);
                foreach ($list['data'] as $k => $v) {
        ...
        if($usage==4){
            ...
            $condition = [
                'nm.website_id' => $this->website_id,
                'nm.custom_global'=>['neq','']
            ];
            if ($search_text) {
                $condition['nm.custom_global'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
        ...
        if($usage==6){
            ...
            if ($search_text) {
                $condition['nm.custom_team'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
        ...
        if($usage==3){
            ...
            $condition = [
                'nm.website_id' => $this->website_id,
                'nm.distributor_apply'=>['neq','']
            ];
            if ($search_text) {
                $condition['nm.distributor_apply'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
        ...
        if($usage==7) {
            ...
            $condition = [
                'nm.website_id' => $this->website_id,
                'nm.channel_custom'=>['neq','']
            ];
            if ($search_text) {
                $condition['nm.channel_custom'] = ['like','%'.$search_text.'%'];
            }
            if ($ext_condition) {
                $condition = array_merge($condition, $ext_condition);
            }
            $list = $channel->getViewList($page_index, $page_size, $condition, 'nm.create_time desc');
            if($list['data']){
                $list['data'] = objToArr($list['data']);
                foreach ($list['data'] as $k => $v) {
