...
class Customform extends Addons
{
    public $info = array(
        ...
        'title' => '系统表单',//插件中文名
        'description' => '收集订单/会员/分销商/代理等表单',//插件描述
        'status' => 1,//状态 1使用 0禁用
        'author' => 'vslaishop',// 作者
        'version' => '1.0',//版本号
    ...
    public $menu_info = array(
        ...
        [
            'module_name' => '模板列表',
            'parent_module_name' => '系统表单', //上级模块名称 确定上级目录
            'last_module_name' => '', // 上一个菜单名称 用来确定菜单排序
            'is_menu' => 1, // 是否为菜单
            'is_dev' => 0,  // 是否为开发者模式可见
        ...
        [
            'module_name' => '编辑表单',
            'parent_module_name' => '系统表单',//上级模块名称 用来确定上级目录
            'last_module_name' => '',//上一个菜单名称 用来确定菜单排序
            'is_menu' => 0,//是否为菜单
            'is_dev' => 0,//是否是开发模式可见
            'desc' => '编辑表单',//菜单描述
        ...
        [
            ...
            'is_menu' => 1,//是否为菜单
            'is_dev' => 0,//是否是开发模式可见
            'desc' => '支持设置系统表单的模块有确认订单、会员资料、分销商申请、股东申请、区域代理申请、队长申请、渠道商申请。',//菜单描述
            'module_picture' => '',//图片（一般为空）
            'icon_class' => '',//字体图标class（一般为空）
            'is_control_auth' => 1,//是否有控制权限
    ...
    public function customFormList()
    {
        ...
        $all = $custom_form->all(['vsl_custom_model.website_id'=>$this->website_id],['custom_tag']);
        $addonsConfigSer = new \data\service\AddonsConfig();
        $set_data = $addonsConfigSer->getAddonsConfig('customform', $this->website_id, 0, 1);
    ...
    {
        $addonsConfigSer = new \data\service\AddonsConfig();
        $custom_info = $addonsConfigSer->getAddonsConfig('customform', $this->website_id, 0, 1);
