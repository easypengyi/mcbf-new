...
class Customform extends baseCoupon
{
   public function __construct()
   {
    ...
    {
       $id = request()->post('id');
       $is_check = request()->post('is_check',false);//是否标签下有关联模板也删除
        try {
            Db::startTrans();
            //先查询下面是否有关联的模板，进行提示
            $couponServer = new CustomFormServer();
            $data= $couponServer->getCustomFormList(1,1, ['website_id'=>$this->website_id,'tagid'=>$id]);
            if($data['custom_forms']['data'] && $is_check){
                return AjaxReturn(-2,[],'该标签下面存在使用的模板，是否删除？');
            }
            $custom = new Custom();
            $custom->deleteCustomFormTag($id);
    
            $res = $couponServer->deleteCustomFormData(['website_id'=>$this->website_id,'tagid' => $id]);
            if($res){
    ...
    public function customDataExcel()
    {
        ...
        foreach ($list['data'] as $k => $v) {
            ...
            if(!empty($list['data'][$k]['custom_data'])){
            ...
            $list['data'][$k]["user_name"] = $list['data'][$k]["user_name"] ? $list['data'][$k]["user_name"] : $list['data'][$k]["nick_name"];
            $list['data'][$k]["user_name"] = strval($list['data'][$k]["user_name"]);
