...
class Wheelsurf extends BaseService
{
        ...
        if($type==1 && $info['prize']){
            foreach($info['prize'] as $k => $v){
                ...
                if($v['prize_type']==5){
                    ...
                    $vslgoods = $goodsSer->getGoodsDetailById($v['prize_type_id'], 'goods_id,price,goods_name,stock,picture', 1);
    ...
    public function getPrizeName($prize_type_id, $prize_type)
    {
        ...
        if($prize_type==5){
            $goodsSer = new \data\service\Goods();
            $goods = $goodsSer->getGoodsDetailById($prize_type_id, 'goods_id,price,goods_name,stock');
    ...
    public function saveConfig($is_wheelsurf)
    {
        ...
        if (!empty($info)) {
            $res = $AddonsConfig->updateAddonsConfig('', '大转盘设置', $is_wheelsurf, 'wheelsurf');
        } else {
