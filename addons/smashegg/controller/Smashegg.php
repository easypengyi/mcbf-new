...
class Smashegg extends baseSmashegg
{
    ...
    public function prizeType()
    {
        ...
        if($type==5){
            ...
            $list = $goodsSer->getPageGoodsList($page_index, $page_size, $condition, 'create_time desc', 'goods_id,goods_name,description,price,img_id_array,picture');
            $goods_list = [];
            if( !empty($list['data']) ){
    ...
    public function addSmashegg()
    {
        ...
        $input['point'] = input('post.point') ?: 0;
        ...
        if($res){
            ...
            //加入延时队列处理开启/关闭
            $delay_time1 = $input['start_time'] - time();
            if($delay_time1 > 0){
                $this->smallPromotionOpenOrClose('smash_egg_id', $res, 'smash_egg', $delay_time1);
            }
            $delay_time2 = $input['end_time'] - time();
            if($delay_time2 > 0){
                $this->smallPromotionOpenOrClose('smash_egg_id', $res, 'smash_egg', $delay_time2);
            }
     ...
     * @param $delay_time
     */
    public function createSmashEggDelay($smash_egg_id, $delay_time)
    {
