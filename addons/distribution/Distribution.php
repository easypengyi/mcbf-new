...
class Distribution extends Addo
{
    ...
    public $menu_info = array(
        [
            ...
            'icon_class' => 'icon-commission', // 字体图标class（一般为空）
        ...
        [
            ...
            'is_menu' => 1, // 是否是菜单
            'is_dev' => 0, // 是否是开发模式可见
            'desc' => '分销商下线购买商品、分销商自己购买商品都可产生美丽分记录，当账目有出入时可通过流水追溯原由。', // 菜单描述
            'module_picture' => '', // 图片（一般为空）
            'icon_class' => '', // 字体图标class（一般为空）
            'is_control_auth' => 1, // 是否有控制权限
            'hook_name' => 'beautifulPointRecordsList'
        ],
        [
            'module_name' => '佣金提现列表',
            'parent_module_name' => '财务', // 上级模块名称 用来确定上级目录
    ...
    public function commissionRecordsList(){
        if(getAddons('membercard',$this->website_id)) {
            $this->assign('membercard_status',1);
        }else{
            $this->assign('membercard_status',0);
        }
     ...
     * 美丽分流水
     */
    public function beautifulPointRecordsList(){
        $this->fetch('template/platform/pointBill');
    }
    /**
     * 安装方法
     */
...
/*-------------------------------------------------------------------前端钩子开始-----------------------------------------------------------------------*/
    ...
    public function orderPayCalculate($params)
    {
        ...
        }else if($member['isdistributor'] != 2){ //add 支付后如果没有上级的情况下，需要走成为流程
            $config->becomeDistributor($params['uid'], $params['order_id']);
        }
    ...
    public function updateOrderCommission($params)
    {
            ...
            if(in_array($params['order_id'],$orderids)) {
                ...
                $config = new DistributorService();
                    ...
                    $list = $config->getDistributionSite($params['website_id']);
                        ...
                        $order_beautiful_pointA = $order_commission->Query(['order_id' => $params['order_id'],'return_status' => 0], 'beautiful_pointA');
                        ...
                        $order_beautiful_pointB = $order_commission->Query(['order_id' => $params['order_id'],'return_status' => 0], 'beautiful_pointB');
                        ...
                        $order_beautiful_pointC = $order_commission->Query(['order_id' => $params['order_id'],'return_status' => 0], 'beautiful_pointC');
                        ...
                        if ($order_commissionB_id['commissionB_id'] && $list['distribution_pattern'] >= 2) {
                            ...
                            if ($order_status == 4 && !isset($params['status'])) {
                                ...
                                $data['beautiful_point'] = array_sum($order_beautiful_pointB);//二级积分
                                // 发放订单的二级分销佣金
                                $distributor->addCommissionDistribution($data);
                                // 更新当前用户的分销商等级
                                $distributor->updateDistributorLevelInfo($data['uid']);
                        ...
                        if ($order_commissionC_id['commissionC_id'] && $list['distribution_pattern'] >= 3) {
                            ...
                            if ($order_status == 4 && !isset($params['status'])) {
                                ...
                                $data['beautiful_point'] = array_sum($order_beautiful_pointC);//三级积分
                                // 发放订单的三级分销佣金
                                $distributor->addCommissionDistribution($data);
                                // 更新当前用户的分销商等级
                                $distributor->updateDistributorLevelInfo($data['uid']);
        ...
        // 当前用户不是分销商，满足成为分销商条件
        if (($order_status == 4 || $params['status'] == 3) && $member_info['isdistributor'] != 2 ) {
