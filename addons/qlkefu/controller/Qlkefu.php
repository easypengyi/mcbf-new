...
class Qlkefu extends baseQlkefu
{
    ...
    public function saveSetting()
    {
        ...
        if(empty($info)){
            ...
            $AddonsConfig->addAddonsConfig($input, "客服系统设置", $is_use, self::$addons_name);
        }else{
            $res = $qlkefu_server->updateQlkefu($input,['qlkefu_id'=>$info['qlkefu_id']]);
            $AddonsConfig->updateAddonsConfig($input, "客服系统设置", $is_use, self::$addons_name);
        }
        if($res){
    ...
    public function addQlkefu()
    {
        ...
        if($res){
            ...
            $is_use = $addonsConfigSer->getAddonsConfig('qlkefu', $this->website_id);
            if($is_use['is_use']!=1){
                //默认开启
    ...
    public function qlkefuInfo()
    {
        ...
        if($shops_id=='' && $goods_id==''){
            ...
            $goods_info = $goodsSer->getGoodsDetailById($goods_id, 'goods_id,website_id,shop_id');
            if($goods_info){
