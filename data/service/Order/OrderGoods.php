...
class OrderGoods extends BaseService
{
     ...
     * @param $order_types [string] 订单来源 代理agent或其他
     * @return int|string
     */
    public function addOrderGoodsNew($order_id, array $sku_lists, $adjust_money = 0, $uid = 0, $website_id = 0, $order_type = 1, $pay_money = 1, $order_from = 1, $store_id=0, $invoice_tax =0,$order_types ='')
    {
        $err = 0;
        ...
        try {
            ...
            foreach ($sku_lists as $sku_id => $sku_info) {
                ...
                $average_tax = $sku_info['tax'] ?:0;//前面算好了税费
                ...
                if ($sku_info['order_type'] == 10 && getAddons('integral', $this->website_id, $this->instance_id)) {
                    ...
                    $fields = 'goods_name,price,goods_type,picture,point_exchange_type,give_point,supplier_info';
                }
                ...
                $receive_order_goods_data = '';//string
                $receive_goods_code_deduct = 0;
                if (isset($sku_info['receive_goods_code_data'])){
                    if($sku_info['receive_order_goods_data']){
                        $receive_order_goods_data = json_encode($sku_info['receive_order_goods_data']);
                    }
                    $receive_goods_code_deduct = $sku_info['receive_order_goods_data']['money']?:0;
                }
                //领货码、积分抵扣、税费、会员卡抵扣。
                if(isset($sku_info['receive_goods_code_data']) || $sku_info['deduction_money'] || isset($sku_info['tax'])){
                    $actual_price =  $sku_info['real_money'];
                    $real_money = $sku_info['real_money']*$sku_info['num'];
                }
                if ($sku_info['membercard_deduction_money']){
                    $actual_price =  $sku_info['real_money'] - $sku_info['membercard_deduct_shipping_fee'];
                    $real_money =  bcsub($sku_info['real_money']*$sku_info['num'],$sku_info['membercard_deduct_shipping_fee'],2);
                }
                $anchor_id = 0;
                //判断商品是否属于这个直播间这个主播
                if(getAddons('liveshopping', $this->website_id)){
                    $live_goods=  new LiveGoodsModel();
                    $live = new LiveModel();
                    $anchor_cond['goods_id'] = $sku_info['goods_id'];
                    $anchor_cond['anchor_id'] = $sku_info['anchor_id'];
                    //                    $anchor_cond['is_recommend'] = 1;
                    $is_live_goods = $live_goods->getInfo($anchor_cond);
                    if($is_live_goods){
                        if($sku_info['anchor_id']){
                            $live_info = $live->getInfo(['anchor_id'=>$sku_info['anchor_id'], 'status'=>2], 'play_time');
                            if($live_info){
                                $anchor_id = $sku_info['anchor_id'];
                ...
                $data_order_sku = array(
                    ...
                    'remainder' => $sku_info['remainder'] ?:0,//商品应付总额的余数 即质数导致的差额
                    ...
                    'invoice_tax' => $sku_info['invoice_tax'],
                    'channel_stock' => $channel_stock,
                    'anchor_id' => $anchor_id ?:0,
                    'play_time' => $play_time ?:0,
                    'supplier_id' => $goods_info['supplier_info'],
                    //                    'receive_goods_code' => $sku_info['receive_goods_code'],
                    'receive_goods_code_deduct' => $receive_goods_code_deduct,
                    'receive_order_goods_data' => $receive_order_goods_data,
                    'membercard_deduction_money' => isset($sku_info['membercard_deduction_money'])?$sku_info['membercard_deduction_money']:0,
                    'membercard_return_point' => isset($sku_info['membercard_return_point'])?$sku_info['membercard_return_point']:0,
                    'membercard_deduct_shipping_fee' => isset($sku_info['membercard_deduct_shipping_fee'])?$sku_info['membercard_deduct_shipping_fee']:0,
                ); // 积分数量默认0
                //判断平台开启的是什么零售结算节点
                if($channel_stock) {
                ...
                $order_mdl = new VslOrderModel();
                $order_data = $order_mdl->getInfo(['order_id'=>$order_id], '*');
                if (($order_data['goods_money'] == 0 && getAddons('channel', $this->website_id) && $order_data['buy_type'] == 2) || (getAddons('channel', $this->website_id) && $order_data['buy_type'] == 0 && !empty($sku_info['channel_id']) && $channel_stock)) {//渠道商自提的并且金额为0或者 零售的订单（有减库存就有记录）
                    //将订单商品
                    $channel = new Channel();
                ...
                $order_goods_id = $order_goods->save($data_order_sku);
                $order_goods_id_arr[$sku_id]['order_goods_id'] = $order_goods_id;
                if ($sku_info['num'] == 0) {
                    $err = 1;
                }
                ...
                if (($channel_id && getAddons('channel', $this->website_id) && $order_data['buy_type'] == 2) || ($channel_id && getAddons('channel', $this->website_id) &&$channel_stock)) {//渠道商
                    ...
                    $goods_calculate->subRedisChannelGoodsStock($sku_info['goods_id'], $sku_info['sku_id'], $channel_stock?:$sku_info['num'], $channel_id);
                    //渠道商带了秒杀等活动
                    if (!empty($sku_info['seckill_id']) && $seckill_status) {
                        ...
                        $seckill_server->subRedisSeckillGoodsStock($sku_info['seckill_id'], $sku_info['goods_id'], $sku_info['sku_id']);
                        ...
                        $bargain_server->subRedisBargainGoodsStock($sku_info['bargain_id'], $sku_info['sku_id']);
                        //加销量
                        if ($pay_money == 0 && $order_from != 3) {//订单金额为0的情况，不需要支付，在支付那里加的销量，如此就走不到支付那里了
                            $goods_calculate = new GoodsCalculate();
                            $goods_calculate->addGoodsSales($sku_info['goods_id'], $sku_info['num']);
                ...
                if (($channel_id && $order_types == '' && getAddons('channel', $this->website_id) && $order_data['buy_type'] == 2) || ($channel_id && getAddons('channel', $this->website_id) &&$channel_stock)) {//渠道商
                    //如果是渠道商商品，则减的是渠道商的商品库存，并且加入member的账户余额中

                    $goods_calculate->subRedisChannelGoodsStock($sku_info['goods_id'], $sku_info['sku_id'], $channel_id);
                    //渠道商带了秒杀等活动
                    if (!empty($sku_info['seckill_id']) && $seckill_status) {
                        $seckill_server = new Seckill();
                ...
                if (!empty($sku_info['seckill_id']) && $seckill_status && empty($channel_stock)) {//秒杀
                    ...
                    $bargain_condition['bargain_id'] = $sku_info['bargain_id'];
                    $bargain_condition['uid'] = $order_data['buyer_id'];
                    ...
                    if($store_id && $stock_type == 1){
                        ...
                        $goods_calculate->storeRedisSubGoodsStock($sku_info['goods_id'], $sku_info['sku_id'], $store_id);
                    }else{
                        if($goods_info['supplier_info']) {
                            //供应商商品是减供应商的库存,最后再同步
                            $goods_calculate->subSupplierGoodsStock($sku_info['goods_id'], $sku_info['sku_id'], $sku_info['num'] - $channel_stock);
                        }else{
                            if($goods_info['goods_type'] != 5) {
                                //电子卡密商品不在这里减库存,支付成功分配卡密时减卡密库的库存
                                //减平台库存
                                $goods_calculate->subRedisGoodsStock($sku_info['goods_id'], $sku_info['sku_id']);
                            }
                        }
                    }
                    if ($pay_money == 0 && $order_from != 3) {//订单金额为0的情况，不需要支付，在支付那里加的销量，如此就走不到支付那里了
            ...
            //如果使用了会员卡抵扣就要减去会员卡余额
            if ($order_data['membercard_deduction_money'] != 0.00) {
                if (getAddons('membercard', $order_data['website_id'])) {
                    $membercardServer = new \addons\membercard\server\Membercard();
                    $membercardServer->adjustBalance($order_data['buyer_id'], $order_data['membercard_deduction_money'], '订单抵扣',75, $order_id);
                }
            }
            if ($err == 0) {
...
//                    $order_goods_info = $order_goods->getInfo(['sku_id' => $sku_info['sku_id'], 'order_id' => $order_id, 'goods_id' => $sku_info['goods_id']]);
                    ...
                    if ($order_type != 2 && $order_type != 3 && $order_type != 4 && $order_type != 10) {
                        ...
                        if ($microshop_status && $had_insert === false) {//微店计算
                            ...
                            $temp_data['order_goods_id'] = $order_goods_id_arr[$sku_id]['order_goods_id'];
            ...
            debugLog($e->getMessage(), 'getMessage');
            recordErrorLog($e);
            $this->order_goods->rollback();
            return $e->getMessage();
        }
    ...
    public function addOrderGoodsForStore($order_id, array $sku_lists, $adjust_money = 0, $uid = 0, $website_id = 0,$order_type=1, $pay_money = 1, $order_from=1, $invoice_tax =0, $store_id)
    {
        ...
        try {
            ...
            $err = 0;
            $temp_total_goods_price = 0;//店铺总商品价格
            foreach ($sku_lists as $k => $v) {
                $temp_total_goods_price += $v['discount_price'] * $v['num'];
            }
            unset($v);
            ...
            foreach ($sku_lists as $sku_id => $sku_info) {
                ...
                if($stock_type == 1) {
                    ...
                    $goods_sku_model = new VslGoodsSkuModel();
                    $goods_model = new VslGoodsModel();//单条查询
                    $fields = 'goods_name,price,picture';
                    $goods_sku_info = $goods_sku_model->getInfo([
                        'sku_id' => $sku_info['sku_id']
                    ], 'sku_id,goods_id,market_price,stock,sku_name,attr_value_items');
...
//                }
                //税费
//                if (isset($sku_info['tax'])){
//                    $real_money = $sku_info['real_money'];
//                }
                //领货码
//                if (isset($sku_info['receive_goods_code_deduct_type'])){
////                    $real_money -= $sku_info['receive_goods_code_deduct'];
//                    $real_money = $sku_info['real_money'];
//                }
                $receive_order_goods_data = '';//string
                $receive_goods_code_deduct = 0;
                if (isset($sku_info['receive_order_goods_data'])){
//                    $real_money = $sku_info['real_money']*$sku_info['num'];
//                    $actual_price = $sku_info['real_money'];
                    if($sku_info['receive_order_goods_data']){
                        $receive_order_goods_data = json_encode($sku_info['receive_order_goods_data'],JSON_UNESCAPED_SLASHES);
                    }
                    $receive_goods_code_deduct = $sku_info['receive_order_goods_data']['money']?:0;
                }
                //领货码、积分抵扣、税费、会员卡抵扣。
...
//                    $goods_calculate->storeSubGoodsStock($sku_info['goods_id'], $sku_info['sku_id'], $sku_info['num'],$store_id);
                    ...
                    //减平台库存
//                    $goods_calculate->subGoodsStock($sku_info['goods_id'], $sku_info['sku_id'], $sku_info['num']);
                    $goods_calculate->subRedisGoodsStock($sku_info['goods_id'], $sku_info['sku_id']);
                    //加平台销量
                    $goods_calculate->addGoodsSales($sku_info['goods_id'], $sku_info['num']);
                }
            ...
            //如果使用了会员卡抵扣就要减去会员卡余额
            $order_mdl = new VslOrderModel();
            $order_data = $order_mdl->getInfo(['order_id' => $order_id], 'buyer_id,membercard_deduction_money,website_id');
            if ($order_data['membercard_deduction_money'] != 0.00) {
                if (getAddons('membercard', $order_data['website_id'])) {
                    $membercardServer = new \addons\membercard\server\Membercard();
                    $membercardServer->adjustBalance($order_data['buyer_id'], $order_data['membercard_deduction_money'], '订单抵扣',75, $order_id);
...
//                            /*$task_path = 'http://'. $_SERVER["HTTP_HOST"].'/task/load_task_five_rabbitmq';
//                            $data = json_encode(['url'=> $task_path, 'order_id' => $order_id]);*/
...
//                    }
                    if(config('is_high_powered')){
                        ...
                        $routing_key = config('rabbit_discom.routing_key');
                        $url = config('rabbit_interface_url.url');
                        $order_create_url = $url.'/rabbitTask/rabbitmqActDistributionOrCommition';
                        $data['order_id'] = $order_id;
                        $data['action'] = 'order_create';
                        $request_data = json_encode($data);
                        $push_data = [
     ...
     *            ','隔开
     */
    public function orderGoodsDelivery($order_id, $order_goods_id_array, $delivery_num = 0)
    {
        $this->order_goods->startTrans();
        try {
            ...
            foreach ($order_goods_id_array as $k => $order_goods_id) {
                $order_goods = new VslOrderGoodsModel();
                $order_goods_info = $order_goods->getInfo(['order_goods_id' => $order_goods_id],'num,delivery_num');
                //剩余还需要发货的数量
                $can_delivery_num = $order_goods_info['num'] - $order_goods_info['delivery_num'] >= 0 ? $order_goods_info['num'] - $order_goods_info['delivery_num'] : 0;

                $new_delivery_num = 0;
                if($delivery_num[$k] > $can_delivery_num) {
                    $new_delivery_num = $can_delivery_num;
                }else{
                    $new_delivery_num = $delivery_num[$k];
                }
                //总共发货的数量 
                $total_delivery_num = 0;
                $total_delivery_num = $new_delivery_num + $order_goods_info['delivery_num'];
                if (($total_delivery_num >= $order_goods_info['num']) || empty($delivery_num[$k])) {
                    //全部发完了
                    $data = array(
                        'shipping_status' => 1,
                        'order_status' => 2,
                        'delivery_num' => $total_delivery_num,
                    );
                } else {
                    //还有未发的
                    $data = array(
...
//            $order_model->save(['refund_money' => $refund_require_money], ['order_id' => $order_id]);
    ...
    public function orderGoodsRefundAgree($order_id, $order_goods_id, $return_id = 0)
    {
        ...
        try {
            ...
            if($model == 'supplier') {
                ...
                unset($v);
                $this->uid = $this->supplier_id;
            }else{
                //如果是平台或店铺端，则不能处理供应商的商品
                foreach ($orderGoodsInfo as $k => $v) {
                    if($v['supplier_id']) {
                        return -3;
                    }
    ...
    public function orderGoodsRefuseForever($order_id, $order_goods_id, $reason = '')
    {
        ...
        try {
            $model = $this->getRequestModel();
            $order_goods_info = $this->order_goods->getInfo(['order_goods_id' => $order_goods_id],'supplier_id');
            if($model == 'supplier') {
                //如果是供应商端，则不能处理平台或店铺的商品
                if(empty($order_goods_info['supplier_id'])) {
                    return -2;
                }
                $this->uid = $this->supplier_id;
            }else{
                //如果是平台或店铺端，则不能处理供应商的商品
                if($order_goods_info['supplier_id']) {
                    return -3;
                }
            }
            $status_id = OrderStatus::getRefundStatus()[-1]['status_id'];
    ...
    public function orderGoodsRefuseOnce($order_id, $order_goods_id, $reason = '')
    {
        ...
        try {
            ...
            foreach ($order_goods_id as $v) {
                $order_goods_info = $this->order_goods->getInfo(['order_goods_id' => $v],'supplier_id');
                if($model == 'supplier') {
                    //如果是供应商端，则不能处理平台或店铺的商品
                    if(empty($order_goods_info['supplier_id'])) {
                        return -2;
    ...
    public function orderGoodsConfirmRefund($order_id, $order_goods_id, $refund_real_money, $refund_balance_money, $refund_trade_no, $refund_way, $refund_remark)
    {
        ...
        try {
            ...
            if ($order_goods['channel_info'] && $order_goods['channel_stock']) {
                $channel_key = 'channel_'.$order_goods['channel_info'].'_'.$order_goods['sku_id'];
                //将库存塞回去队列
                for($i=0; $i<$order_goods['channel_stock']; $i++){
            ...
            if ($order_seckill_list) {
                ...
                $activity_cond['sku_id'] = $order_goods['sku_id'];
                $activity_cond['buy_type'] = 1;
                $aosr_mdl->where($activity_cond)->delete();
                $redis_goods_sku_seckill_key = 'seckill_' . $seckill_id . '_' . $order_goods['goods_id'] . '_' . $order_goods['sku_id'];
                //将库存塞回去队列
                for($i=0; $i<$order_goods['num']; $i++){
                ...
                $bargain_key = 'bargain_'.$order_info['bargain_id'].'_'.$order_goods['sku_id'];
                //将库存塞回去队列
                for($i=0; $i<$order_goods['num']; $i++){
                ...
                $presell_key = 'presell_'.$order_goods['presell_id'].'_'.$order_goods['sku_id'];
                //将库存塞回去队列
                for($i=0; $i<$order_goods['num']; $i++){
                ...
                if($order_info['shipping_type'] == 1){
                    ...
                    $goods_sku_model = new VslGoodsSkuModel();
                    if($order_goods['supplier_id']) {
                        //供应商商品，库存退回给供应商
                        $supplier_sku_id = $goods_sku_model->Query(['sku_id' => $order_goods['sku_id']],'supplier_sku_id')[0];
                        $supplier_goods_info = $goods_sku_model->getInfo(['sku_id' => $supplier_sku_id],'goods_id,stock');
                        $data_goods_sku = array(
                            'stock' => $supplier_goods_info['stock'] + $order_goods['num'] - $order_goods['channel_stock']
                        );
                        $goods_sku_model->save($data_goods_sku, [
                            'sku_id' => $supplier_sku_id
                        ]);
                        $count = $goods_sku_model->getSum([
                            'goods_id' => $supplier_goods_info['goods_id']
                        ], 'stock');
                        //库存同步到所有上架了该商品的店铺
                        
                        $goods_server->updateGoods([
                            'goods_id' => $supplier_goods_info['goods_id']
                        ], [
                            'stock' => $count
                        ], $supplier_goods_info['goods_id']);
                        $goods_server->syncSupplierGoodsStock($supplier_goods_info['goods_id'],$supplier_sku_id);
                    }else{
                        $goods_key = 'goods_'.$order_goods['goods_id'].'_'.$order_goods['sku_id'];
                        //将库存塞回去队列
                        for($i=0; $i<$order_goods['num']; $i++){
                    ...
                    $store_redis_key = 'store_goods_'.$order_goods['goods_id'].'_'.$order_goods['sku_id'];
                    //将库存塞回去队列
                    for($i=0; $i<$order_goods['num']; $i++){
            ...
            debugFile($e->getMessage(), 'createLuckySpell-0-7-1-2-3-0-00', 1111112);
            recordErrorLog($e);
    ...
    {
        //供应商
        if ($this->supplier_id && getAddons('supplier',$this->website_id)){
...
//                    var_dump($data);
    ...
    public function getSpecInfo($spec_value_id, $goods_id, $order_type = 1)
    {
        ...
        if ($order_type == 10) {
            ...
            $goodsModel = new VslGoodsModel();//单条查询
        }
...
//                    'channel_weight' => $channel_weight?:0,
                ...
                $order_mdl = new VslOrderModel();
                $order_data = $order_mdl->getInfo(['order_id' => $order_id], '*');
                if (($order_data['goods_money'] == 0 && getAddons('channel', $this->website_id) && $order_data['buy_type'] == 2) || (getAddons('channel', $this->website_id) && $order_data['buy_type'] == 0 && !empty($sku_info['channel_id']))) {//渠道商自提的并且金额为0或者 零售的订单（有减库存就有记录）
                    //将订单商品
                    $channel = new Channel();
...
//                    }
    ...
    public function getOrderGoodsData($condition, $field='*')
    {
        ...
        return $orderGoodsModel->getQuery($condition, $field);
    }
}