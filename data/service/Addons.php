...
class Addons extends BaseService {
    ...
    public function getModuleList($search_text = '') {
        ...
        $group_is_system = Session::get($modules . 'group_is_system');
    ...
    public function createOrder($addons_id, $time) {
        ...
        //加入rabbitmq延时队列控制订单超时未支付关闭
        $this->incrementOrderDelayClose($order_id);
        $pay = new UnifyPay();
     ...
     * @param $order_id
     */
    public function incrementOrderDelayClose($order_id)
    {
        if(config('is_high_powered')){
            $config['delay_exchange_name'] = config('delay_order_close.delay_exchange_name');
            $config['delay_queue_name'] = config('delay_order_close.delay_queue_name');
            $config['delay_routing_key'] = config('delay_order_close.delay_routing_key');
            //自动关闭时间
            $delay_time = 30 * 60 * 1000;//毫秒
            ...
            $back_url = $url.'/rabbitTask/incrementOrdersClose';
            $custom_type = 'increment_order_close';
            delayPushData($config, $delay_time, $data, $back_url, $custom_type);
        }
    }
    public function createOrderForMessage($num) {
        $config = new Config();
        $info = $config->getConfigMaster(0, 'MOBILEMESSAGE', 0, 1);
     ...
     * 获取应用列表
     */
    public function getAddonsList ($condition, $field='*', $order = '')
    {
        $sys_addons = new SysAddonsModel();
...
//             'addons' => $addons,
     ...
     * 获取addons信息
     */
    public function getAddonsDataByName ($addons_name,$field='*')
    {
        $sys_addons = new SysAddonsModel();
        return $sys_addons->getInfo(['name' => $addons_name], $field);
    }
    
    /**
     * 获取多条应用名信息
     * @param string $addons_name [ ,分割的多个应用名]
     * @param string $field
     * @return mixed
     */
    public function getAddonsDatasByName ($addons_name,$field='*')
    {
        $addons_name_arr = explode(',',$addons_name);
        $sys_addons = new SysAddonsModel();
        return $sys_addons->getQuery(['name'=> ['in',$addons_name_arr]], $field);
    }
    
    /**
     * 供应商查询应用列表
     */
    public function getModuleListForSupplier($search_text = '') {
        $supplier_addons = 'delivery,goodhelper';//商品助手，发货助手
        $returnArr = $this->getAddonsDatasByName($supplier_addons,'id');
        $addonsArr = array_column(objToArr($returnArr),'id');
        $addons_ids = implode(',',objToArr($addonsArr));
        $addonsModel = new SysAddonsModel();
        $addonsCategory[] = [
            'category_name' => '常用功能',
            'addons' => $addons_ids,
