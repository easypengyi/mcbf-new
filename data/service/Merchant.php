...
class Merchant extends BaseService
{
    ...
    /**
     * 该版本下是否应用存在
     * @param $addons_name [应用名字]
     * @return bool
     */
    public function isExistAddons ($addons_name)
    {
        $webSer = new WebSite();
        $versionId = $webSer->getCompatMerchantVersionId();
        if ($versionId){
            $merVerModel = new MerchantVersionModel();
            $type_module_str = $merVerModel->getInfo(['merchant_versionid' => $versionId], 'type_module_array');
            $type_module_arr = explode(',',$type_module_str['type_module_array']);
        }
        $addonsSer = new Addons();
        $addonsRes = $addonsSer->getAddonsDataByName($addons_name,'module_id');
        if ($addonsRes){
            $module_id = $addonsRes['module_id'];
            if (in_array($module_id, $type_module_arr)){
                return true;
            }
        }
        return false;
