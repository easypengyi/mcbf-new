...
class WebSite extends BaseService
{
     ...
     * @return int $res
     */
    public function updateWebSite($website_data=array(),$website_id=0)
    {
        $website_id = $website_id ?: $this->website_id;
        $website_id = 1;
        $this->website = new WebSiteModel();
        $res = $this->website->save($website_data, ['website_id' => $website_id]);
        if ($res) {
    ...
    public function getWebSiteInfo($website_id = '')
    {
        ...
        if(cache('WEBSITEINFO'.$website_id)){
            return cache('WEBSITEINFO'.$website_id);
        }
        ...
        $info['true_shop_validity_time'] = $info['shop_validity_time'];
        ...
        cache('WEBSITEINFO'.$website_id, $info);
        return $info;
    }
     ...
     * 获取网站信息
     */
    public function getWebInfo ($website_id = 0, $field='*')
    {
        $website_id = $website_id ?: $this->website_id;
        return $this->website->getInfo(['website_id' => $website_id], $field);
    }
    ...
    public function updateRealmWebSite($data=array())
    {
        ...
        $res = $configSer->setConfigOne($params);
        return $res;
    }
    /*
     * (non-PHPdoc)
    ...
    {
        $res = $this->module->getModuleIdByModule($controller, $action, $port);
        return $res;
    }
...
//            $retval = $user_service->add($user_tel, $user_pwd, '', $user_tel, 1,0, '', '', '', '', '', 0, 0, 'platform');
            $retval = $user_service->add($params);
            if($retval >0){
    ...
    public function addDefaultGoods($website_id = 0){
        ...
        $defaultGoodsId = $goodsSer->getGoodsListByCondition(['website_id' => 0], 'goods_id');
        $deaultIdStr = '';
        if($defaultGoodsId){
            $defaultGoodsId = array_column($defaultGoodsId, 'goods_id');
        ...
        foreach($defaultGoods as $gv){
            ...
            $goodsModel = new \data\model\VslGoodsModel();//创建商品
            $newGoodsArr[$goods_id] = $goodsModel->isUpdate(false)->save($gv);
            $goodsSer->addOrUpdateGoodsToEs($newGoodsArr[$goods_id]);
        }
        unset($gv);
        //处理商品属性
...
//        $create_time = $this->website_create_time;
        ...
        $create_time = Session::get($model . 'website_create_time'.$website_id);
        if(!$create_time){
            $create_time = $this->website->getInfo(['website_id' => $website_id],'create_time')['create_time'];
            Session::set($model . 'website_create_time'.$website_id, $create_time);
        }
...
/*
     * 获取商家名称
     */
    public function getWebMallName($website_id = 0){
        $mallName = $this->mall_name;
        if(!$mallName){
            $mallName = $this->website->getInfo(['website_id' => $website_id],'mall_name')['mall_name'];
        }
        return $mallName;
    }
    /*
     * 网站基本信息
     */
    public function getWebInfoRelationMerchantVersion ($website_id=0)
    {
        $website_id = $website_id ?: $this->website_id;
        if (!$website_id) {return [];}
        $webInfo = $this->website->getInfo(['website_id' => $website_id]);
        if ($webInfo['merchant_versionid']) {
            $merchantVersionModel = new MerchantVersionModel();
            $merchantInfo = $merchantVersionModel->getInfo(['merchant_versionid' => $webInfo['merchant_versionid']]);
            $webInfo['merchant_version'] = $merchantInfo;
        }

        return $webInfo;
    }

    public function seveWebInfo ($data,$condition)
    {
        return $this->website->save($data, $condition);
    }

    /*
     * 商家续费、升级版本充值
     */
    public function createOrderOfWebVersionForShop($pay_money, $pay_type = 1, $pay_way=1) {
        if ($pay_money == 0) {return false;}
        $data = array(
            ...
            'circle_time'   => 0,
            'order_type'    => 4,//商家续费充值
            'payment_type'  => $pay_type,//todo...1微信 2支付宝
        );
        $order = new VslIncrementOrderModel();
        $order->save($data);
        ...
        $pay_body = '商家充值';
        $way = $pay_way == 1 ? '续费' : '升级';
        $type = $pay_type == 1 ? '微信' : '支付宝';
        $pay_detail = '商家'.$way.'充值('.$type.')';
        $pay = new UnifyPay();
        $pay->createIncrementPayment($data['out_trade_no'], $pay_type, $order_id, $pay_body, $pay_detail, $pay_money, $data['create_time']);
        return $order_id;
    }

    /**
     * 获取升级版本列表(权重大于当前版本)
     * @param $rent_id int [租用版本id]
     * @param string $field
     * @param string $order
     * @return mixed array [版本列表]
     */
    public function getUpgradeVersion($rent_id, $field='*', $order='sale_money ASC')
    {
        $merVerModel = new MerchantVersionModel();
        $weight = $merVerModel->getInfo(['merchant_versionid' => $rent_id], 'weight')['weight'] ?:0;//当前版本权重
        $res = $merVerModel->getQuery([
            'weight'=> ['>=', $weight],
            'merchant_versionid' => ['<>', $rent_id],
        ], $field, $order);
        return objToArr($res);
    }
     ...
     */
    public function calculateUpgradeRemainCostsOfRent($website_id)
    {
        $rentModel = new WebSiteModel();
        $info = $rentModel::get(
            [
                'website_id' => $website_id
            ],
        ...
        if ($info['shop_validity_time'] && !$info['create_time']) {
            return AjaxReturn(FAIL,[],'时间不匹配!');
        }
        // 版本享有天数
        $have_day   = ceil(($info['shop_validity_time'] - $info['create_time']) / 86400);
        // 剩余天数
        $remain_day = intval(($info['shop_validity_time'] - time()) / 86400);
        $remain_day = $remain_day > 0 ? $remain_day : 0;
        // 剩余费用
        if ($info['pre_version_agent_actual_buy']) {//如果已经升级过，说明要用上一个版本的实际版本折扣价，不能用实际支付价格‘agent_actual_buy’
            $buy_fee = $info['pre_version_agent_actual_buy'];
        } else {
            $buy_fee = $info['agent_actual_buy'] ?: $info['merchant_version']['sale_money'];
        }
        if (!$buy_fee) {
            return AjaxReturn(FAIL,[],'费用不正确!');
        }
        $remain_fee = intval(($buy_fee/$have_day) * $remain_day);
     ...
     * 获取租用版本信息
     */
    public function getMerchantVersionInfo ($condition, $field='*', $order ='')
    {
        $merchantVersionModel = new MerchantVersionModel();
        return $merchantVersionModel->getInfo($condition, $field, $order);
    }

    /**
     * 租用版本升级实际需要费用（选择的升级折扣版本价 - 原版本剩余价）
     * @param $website_id
     * @return array|int|string
     * @throws \think\Exception\DbException
     */
    public function calculateUpgradeCostsOfRent($website_id)
    {
        // 当前版本剩余费用
        $remain_fee = $this->calculateUpgradeRemainCostsOfRent($website_id);
        return $remain_fee;
    }
     ...
     * 获取商城logo
     */
    public function getLogo ($website_id, $shop_id=0, $field = '')
    {
        if ($shop_id && getAddons('shop',$website_id)) {
            $field = $field ?:'shop_logo';
            //获取店铺的
            $shopM = new VslShopModel();
            $shopRes = $shopM->getInfo(['website_id'=> $this->website_id, 'shop_id' => $shop_id], $field);
            if (!$shopRes){return;}
            if ($pic_id = $shopRes['shop_logo']) {
                $picM = new AlbumPictureModel();
                $picRes = $picM->getInfo(['pic_id' => $pic_id], 'pic_cover');
                $shopRes['shop_logo'] = $picRes['pic_cover'];
                return $shopRes;
            }
        }
        if ($website_id) {
            $field = $field ?:'logo';
            $webRes = $this->website->getInfo(['website_id' => $website_id], $field);
            return $webRes;
        }
     ...
     * 获取兼容的版本id
     */
    public function getCompatMerchantVersionId ()
    {
        $version_id = 0;
        if (config('project') == 'shopvslai'){
            $versionRes = $this->website->getInfo(['website_id'=> $this->website_id],'merchant_versionid');
            if ($versionRes) {
                $version_id = $versionRes['merchant_versionid'];
            }
        }else if (config('project') == 'shopdds'){
            $versionRes = $this->main->getInfo(['main_id' => $this->main_id], 'merchant_versionid');
            if ($versionRes) {
                $version_id = $versionRes['merchant_versionid'];
            }
        }
     ...
     * 判断用户是否是超级管理员
     */
    public function getWebByUid($uid)
    {
        $base_info = $this->website->getInfo(['uid' => $uid], 'website_id');
        if (!$base_info) {
            return false;
        }
        return $base_info['website_id'];
    }

    public function checkCurrentVersionIsFree ()
    {
        if(isVSL()){
            config('project');
            if (config('project') == 'shopdds'){
                $webInfo = $this->getWebInfoRelationMerchantVersion();
                if($webInfo['merchant_version']['merchant_versionid'] == 11){
                    return true;
                }
            }else{
                $webInfo = $this->getWebInfoRelationMerchantVersion();
                if($webInfo['merchant_version']['merchant_versionid'] == 6){
                    return true;
                }
            }
        }
