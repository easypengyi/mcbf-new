...
class UnifyPay extends BaseService
{
    ...
    public function getPayConfig()
    {
        ...
        $wchat_pay = $config->getConfig(0, 'WPAY', $this->website_id);
        $ali_pay = $config->getConfig(0, 'ALIPAY', $this->website_id);
		$globe_pay = $config->getConfig(0, 'GLOPAY', $this->website_id);
        $data_config = array(
    ...
    {
        $config = new WebConfig();
        $wchat_pay = $config->getConfigMaster(0, 'WPAY', 0);
        $ali_pay = $config->getConfigMaster(0, 'ALIPAY', 0);
		$globe_pay = $config->getConfig(0, 'GLOPAY', $this->website_id);
        $data_config = array(
            'wchat_pay_config' => $wchat_pay,
            'ali_pay_config'   => $ali_pay,
 ...
 * @param unknown $pay_money
 */
    public function createPayment($shop_id, $out_trade_no, $pay_body, $pay_detail, $pay_money, $type, $type_alis_id, $create_time='', $website_id=0)
    {
        if(!$website_id){
...
//                    $assistant->payOnlineBusinessAssistantApply($out_trade_no);
                ...
                case 6:
                    ...
                    $membercard->buyMembercard($out_trade_no,$pay_type);
                    break;
                case 7:
                    //会员卡充值
                    $membercard = new \addons\membercard\server\Membercard();
                    $membercard->rechargeMembercard($out_trade_no,$pay_type);
                    break;
                case 99:
                    //购买等级
                    $payGradeServer = new PayGradeServer();
                    $payGradeServer->orderOnLinePay($out_trade_no, $pay_type);
                    break;
                default:
                    break;
            }
...
//                    $assistant->payOnlineBusinessAssistantApply($out_trade_no);
     ...
     * @param unknown $sign
     */
    public function checkJoinPaySign($post_obj, $sign,$website_id=0)
    {
        $joinpay = new Joinpay();
        $retval = $joinpay->checkJoinPaySign($post_obj,$sign,$website_id);
        return $retval;
    }
    /**
     * 聚合微信支付
     */
    public function joinPay($out_trade_no, $trade_type, $red_url,$realm_ip='',$joinPay_info='',$return_url=''){
        //渠道商订单号 QD前缀
        if (strstr($out_trade_no, 'QD')) {//渠道商订单
            $data = $this->getChannelPayInfo($out_trade_no);
        } elseif (strstr($out_trade_no, 'DH')) {//兑换订单
        ...
        if($trade_type == 'JSAPI'){
            ...
            $product_id = '';
            $retval = $joinpay->setWeiXinPay($data['pay_body'], $data['pay_detail'], $data['pay_money']*100, $out_trade_no, $red_url, $trade_type, $openid, $product_id,$joinPay_info,$realm_ip);
            return $retval;
        }else if($trade_type == 'ALIH5'){
            $joinpay = new Joinpay();
            $product_id = '';
            $openid = '';
            $retval = $joinpay->setWapPay($data['pay_body'], $data['pay_detail'], $data['pay_money']*100, $out_trade_no, $red_url, $trade_type, $openid, $product_id,$joinPay_info,$realm_ip,$return_url);
            return $retval;
        }else if($trade_type == 'WEIXIN_APP'){
            $joinpay = new Joinpay();
            $product_id = '';
            $openid = '';
            $retval = $joinpay->setWeiXinPay($data['pay_body'], $data['pay_detail'], $data['pay_money']*100, $out_trade_no, $red_url, $trade_type, $openid, $product_id,$joinPay_info,$realm_ip,$return_url);
            return $retval;
        }else{
            $res['code'] = -1;
            $res['message'] = '请在微信公众号内打开，并完成支付';
            return $res;
        }
    }
	/* (non-PHPdoc)
...
//                echo "不支持的返回状态，创建订单二维码返回异常!!!";
    ...
    public function tlpayNotify($a,$website_id){
        ...
        $config = new WebConfig();
        $info = $config->getConfig(0, 'TLPAY', $website_id, 1);
    ...
    public function checkSigns($post_obj, $sign)
    {
        $weixin_pay = new WeiXinPay();
...
//                    $assistant->payOnlineBusinessAssistantApply($out_trade_no);
    ...
    /**
     * 通联支付查询支付结果 or 汇聚支付查询支付结果
     * @param type $out_trade_no
     * @param type $website_id
     * @return type
     */
    public function tlCheckPayResult($out_trade_no,$website_id)
    {

        $webConfig = new WebConfig();
        $joinPay = $webConfig->getConfig(0, 'JOINPAY', $website_id);
        
        if($joinPay['is_use'] == 1 && $joinPay['value']['fastpay_is_use'] == 1){
            $pay = new Joinpay();
            $result = $pay->payQuery($joinPay['value'],$out_trade_no,$website_id); 
            debugLog($result,'汇聚支付-tlCheckPayResult');
            if($result['resp_code'] == 'SUCCESS' && $result['biz_code'] == 'JS000000'){
                if($result['order_status'] == 'P1000' || $result['order_status'] == 'P3000'){
                    return 2;
                }else{
                    return 0;
                }
        ...
        $tl_pay = new tlPay();
        $retval = $tl_pay->payQuery($out_trade_no,$website_id);
        debugLog($retval,'tlCheckPayResult');
        if($retval['retcode'] == 'FAIL'){
            return $retval['retcode'];
        }
        if($retval['trxstatus'] == '0000'){
            return 2;
        }
        return 1;
    }
    ...
    /**
     * 使用GlobePay支付时，需变更外部交易号再次调起外部交易号时用这个方法
     */
    public function GlobePay($out_trade_no, $type, $red_url, $realm_ip)
    {
        $order = new VslOrderModel();
        $orderInfo = $order->getInfo(['out_trade_no' => $out_trade_no], 'shop_id,website_id,order_type,order_id');
        if ($orderInfo['order_type'] == 5 && getAddons('groupshopping', $this->website_id)) {
        ...
        $order_ids = $order->Query(['out_trade_no' => $out_trade_no],'order_id');
        $order->isUpdate(true)->save(['out_trade_no' => $new_out_trade_no],['order_id' => ['IN',$order_ids]]);
        foreach ($order_ids as $v) {
            $this->modifyNo($v, $new_out_trade_no);
        }

        $gbpay = new globalpay();
        if ($type == 6) {
            $data = $this->globePayMir($new_out_trade_no);
            if ($data) {
                $user  = new UserModel();
                $openid = $user->getInfo(['uid'=>$this->uid],'mp_open_id')['mp_open_id'];
                ...
                $res=$gbpay->getparams($payconfig['value'],$new_out_trade_no,$param);
                if($res['result_code']=='SUCCESS'){
                    return $res;
                }else{
                    if($res['return_code']=='DUPLICATE_ORDER_ID') {
                        //订单重复，重新生成外部交易号去支付
                        $res = $this->GlobePay($new_out_trade_no, $type, $red_url, $realm_ip);
                        if($res['result_code']=='SUCCESS') {
                            return $res;
                        }else{
                            $data['code'] = -1;
            ...
            if ($data) {
                ...
                if($type == 1){
                    $res=$gbpay->getpayurl($payconfig['value'],$new_out_trade_no,$param,1,$realm_ip);
                }
                if($type == 2){
                    $res=$gbpay->getpayurl($payconfig['value'],$new_out_trade_no,$param,2,$realm_ip);
                }
                if($res['result_code']=='SUCCESS'){
                    return $res;
                }else{
