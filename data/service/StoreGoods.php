...
class StoreGoods extends BaseService
{
     ...
     * 商品管理-已上架商品/仓库中商品
     */
    public function getGoodsList($page_index, $page_size, $condition, $field, $order = '', $stock_type, $store_id, $type)
    {
        if ($condition['store_id']) {
            ...
            //从门店所属店铺中筛选出的商品
            $goodsSer = new Goods();
            $list = $goodsSer->getPageGoodsList($page_index, $page_size, $condition, $order, $field);
        }

        $goods_info = $list['data'];

        //如果是使用店铺统一库存，已上架商品中则只显示店铺有勾选当前门店的商品
        if ($stock_type == 2 && $type == 1) {
        ...
        //如果是使用店铺统一库存，仓库中商品中则只显示店铺未勾选当前门店的商品
        if ($stock_type == 2 && $type == 2) {
            foreach ($goods_info as $k => $v) {
                $v['store_list'] = explode(',', $v['store_list']);
                if (in_array($store_id, $v['store_list'])) {
                    unset($goods_info[$k]);
     ...
     * 商品管理-商品上架
     */
    public function goodsOnline($condition, $store_id)
    {
        $goodsSer = new Goods();
        if ($condition['store_id']) {
            //上架前先判断库存以及平台的状态
            ...
            $state = $goodsSer->getGoodsDetailByCondition(['goods_id' => $condition['goods_id'], 'website_id' => $this->website_id], 'state')['state'];
            if ($stock > 0 && $state == 1) {
                $data = [
                    'state' => 1
                ];
            ...
            $store_list = $goodsSer->getGoodsDetailByCondition($condition, 'store_list')['store_list'];
            if ($store_list) {
                $store_list = $store_list . ',' . $store_id;
            } else {
                $store_list = $store_id;
            }
            $res = $goodsSer->updateGoods($condition, ['store_list' => $store_list]);
            if ($res > 0) {
                return SUCCESS;
            } else {
                return UPDATA_FAIL;
     ...
     * 商品管理-商品下架
     */
    public function goodsOffline($condition, $store_id)
    {
        if ($condition['store_id']) {
            $data = [
                'state' => 0
    ...
    public function goodsDel($condition)
    {
        ...
        $goodsSer = new Goods();
        $store_list = $goodsSer->getGoodsDetailById($condition['goods_id']);
        //取消此商品对应的核销门店
        ...
        $res2 = $goodsSer->updateGoods(['goods_id' => $condition['goods_id']], ['store_list' => $new_store_list], $condition['goods_id']);
    ...
    */
    public function goodsSave($data, $condition)
    {
        ...
        if (empty($goods_info)) {
            //保存到门店商品表
            $goodsSer = new Goods();
            $goods = $goodsSer->getGoodsDetailByCondition(['goods_id' => $data['goods_id']], 'goods_name,category_id,category_id_1,category_id_2,category_id_3,picture,sales,img_id_array');
            ...
            foreach ($data['sku_list'] as $k => $v) {
                ...
                $store_redis_key = 'store_goods_'.$data['goods_id'].'_'.$v['sku_id'];
                $redis->set($store_redis_key, $v['stock']);
            }
            unset($v);
            ...
            //把添加的商品对应的门店存到goods表
            $old_store_list = $goodsSer->getGoodsDetailById($data['goods_id'], 'goods_id,goods_name,price,collects,store_list')['store_list'];
            if (empty($old_store_list)) {
                $new_store_list = $condition['store_id'];
            } else {
                $new_store_list = $old_store_list . ',' . $condition['store_id'];
            }
            $res2 = $goodsSer->updateGoods(['goods_id' => $data['goods_id']], ['store_list' => $new_store_list], $data['goods_id']);
            if ($res && $res1 && $res2) {
                return 1;
            }
        } else {
        ...
        if ($status == 0) {
            ...
            if ($card_info) {
                foreach ($card_info as $k => $v) {
                    ...
                    $data = array_column(objToArr($data), null, 'record_id');
                    $new_card_list = $data;
                    foreach ($new_card_list as $key => $val) {
                        $albumPictureModel = new AlbumPictureModel();
    ...
    public function getAddGoodsCategoryList($condition)
    {
        ...
        $goodsCategoryIds = array_unique(array_column($goodsCategoryIds, 'category_id_1'));
    ...
    */
    public function getAddGoodsList($page_index, $page_size, $condition, $store_id)
    {
        //从商品表中筛选出的商品
        $goodsSer = new Goods();
        $list = $goodsSer->getPageGoodsList($page_index, $page_size, $condition, '', 'goods_id,goods_name,price,stock,sales,picture,store_list');
        $goods_info = $list['data'];
        ...
        //关联相册表，查出商品对应的图片
        foreach ($goods_info as $key => $val) {
            $goods_info[$key]['goods_img'] = __IMG($val['album_picture']['pic_cover_small']);
        }
        unset($val);
        return [
            'goods_info' => $goods_info,
