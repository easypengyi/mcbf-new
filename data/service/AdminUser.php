...
class AdminUser extends User
{
     ...
     */
    public function getchildModuleQuery($moduleid, $port = '')
    {
        ...
        if (empty($module_list) || $module_list == false) {
            ...
            if ($this->is_admin) {
                $list = $auth_group->getAuthList($moduleid, $port);
                $new_list = $list;
            } else {
...
//        $uid = $this->add($user_name, $user_password, '', $mobile, 1, 0,'', $qq, '', '', '', $instance_id, $this->website_id, $port);
    ...
    public function getAdminUserInfo($condition, $field = "*")
    {
        ...
        $user = new UserModel();
        $user_info = $user->getInfo($condition,'user_password,user_tel,user_qq,port');
        ...
        $admin_user_info ['port'] = $user_info['port'];
