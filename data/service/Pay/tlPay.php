...
use addons\supplier\model\VslSupplierBankAccountModel;
...
class tlPay extends PayParam
{
   ...
   public function setTlSigning($accttype,$acctno,$idno,$acctname,$mobile,$validdate,$cvv2,$uid,$website_id){
    ...
    /**
     * 通联通单笔代付 (优化，依赖注入对象)
     * @param     $withdraw_no [交易号]
     * @param     $uid
     * @param     $bank_id
     * @param     $money
     * @param int $shop_id [-1:供应商，0:商家端，其他:店铺端]
     * @return mixed
     */
    public function tlWithdraw($withdraw_no,$uid,$bank_id,$money,$shop_id=0){
        if($shop_id>0){
            ...
            /*供应商*/
            $bank = new VslSupplierBankAccountModel();
            $bank_info = $bank->getInfo(['id'=>$bank_id]);
        }else{
            $bank = new VslMemberBankAccountModel();
            $bank_info = $bank->getInfo(['id'=>$bank_id]);
        ...
        $config_info = $configSer->getTopTlConfig($bank_info['website_id']);
...
//            print_r("验签结果：验签失败，请检查通联公钥证书是否正确");
            ...
            return AjaxWXReturn(-2,[],$e->getMessage());
        }
...
//            echo '<br/>Verified: <font color=red>Failed</font>.';
    ...
    //商户支付申请确认
    public function payQuery($out_trade_no,$website_id){
        $config = new ConfigSer();
        $info = $config->getConfig(0, 'TLPAY', $website_id, 1);
        $params = array();
        $params["cusid"] = $info['tl_cusid'];
        ...
        $params["sign"] = $this->SignArray($params,$info['tl_key']);//签名
        $paramsStr = $this->ToUrlParams($params);
        $url = "https://vsp.allinpay.com/apiweb/qpay/query";
        $rsp = tl_request($url, $paramsStr);
        $rspArray = json_decode($rsp, true);
        if($this->validSign($rspArray,$info['tl_key'])){
            return $rspArray;
