...
class GlobePay extends PayParam
{
	...
	public function getpayurl($payconfig,$out_trade_no,$param,$type,$realm_ip)
    {
		...
		if($result['result_code']=='SUCCESS'){
			...
			$nonce_str=$GlobePayApi->getNonceStr();
			$sign=$input->makeSign_new($payconfig,$time,$nonce_str);
			$result['pay_url']=$result['pay_url'].'?redirect='.urlencode($realm_ip . '/wap/packages/pay/result?out_trade_no=' . $out_trade_no).'&time='.$time.'&nonce_str='.$nonce_str.'&sign='.$sign.'&directpay=true';
		}
        return $result;
    }
	public function checkSignMp($response, $sign,$website_id){
		$config = new config();
        $payconfig = $config->getConfig(0, 'GPPAY', $website_id, 1);//小程序GlobePay配置
		$GlobePayApi = new GlobePayApi();
		$input = new GlobePayDataBase();
		$time=$response['time'];
	...
	public function checkSign($response, $sign,$website_id){
		$config = new config();
        $payconfig = $config->getConfig(0, 'GLOPAY', $website_id, 1);//GlobePay配置
		$GlobePayApi = new GlobePayApi();
		$input = new GlobePayDataBase();
		$time=$response['time'];
		$nonce_str=$response['nonce_str'];
