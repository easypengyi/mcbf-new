...
use data\extend\aop\alipay_api\AlipayFundTransUniTransferRequest;
...
class AlipayTradeService extends PayParam {
    ...
    function __construct($website_id = 0) {
        ...
        $this->alipay_public_key_cert = $alipay_configs['alipay_public_key_cert'];
        $this->charset = $alipay_configs['charset'];
        $this->signtype = $alipay_configs['sign_type'];
        $this->app_cert_sn = $alipay_configs['app_cert_sn'];
        $this->alipay_root_cert_sn = $alipay_configs['alipay_root_cert_sn'];
        $this->alipay_cert_type = $alipay_configs['ali_cert_type'];
        ...
        if (!$this->alipay_cert_type && (empty($this->alipay_public_key) || trim($this->alipay_public_key) == "")) {
            return false;
        }
        ...
        if ($this->alipay_cert_type){
            ...
            if (empty($this->alipay_public_key_cert) || trim($this->alipay_public_key_cert) == "") {
                return false;
            }
        }
    ...
    public function getAlipayConfigForPc() {
        $alipay_config = array(
            ...
            //调用getCertSN获取证书序列号
            'app_cert_sn' => $this->ali_cert_sn,
            //调用getRootCertSN获取支付宝根证书序列号
            'alipay_root_cert_sn' => $this->ali_root_cert_sn,
            //使用公钥证书的支付宝公钥
            'alipay_public_key_cert' => $this->ali_public_key_cert,
            //验签方式
            'ali_cert_type' => $this->ali_cert_type,
        );
    ...
    function aopclientRequestExecute($request, $ispage = false, $type = 1) {
        if($this->alipay_cert_type){
            return $this->aopclientRequestExecuteCert($request, $ispage, $type);
        }
        $aop = new AopClient();
     ...
     * @return $response 支付宝返回的信息
     */
    function aopclientRequestExecuteCert($request, $ispage = false, $type = 1) {

        $aop = new AopCertClient ();
        $aop->gatewayUrl = $this->gateway_url;
        $aop->appId = $this->app_id;
        $aop->rsaPrivateKey = $this->private_key;
        $aop->alipayrsaPublicKey = $this->alipay_public_key_cert;
        $aop->appCertSN = $this->app_cert_sn; //调用getCertSN获取证书序列号
        $aop->alipayRootCertSN = $this->alipay_root_cert_sn; //调用getRootCertSN获取支付宝根证书序列号
        $aop->apiVersion = "1.0";
        $aop->postCharset = $this->charset;
        $aop->format = $this->format;
        $aop->signType = $this->signtype;
    ...
    function aopclientRequestExecuteApp($request) {
        if($this->alipay_cert_type){
            return $this->aopclientRequestExecuteAppCert($request);
        }
        $aop = new AopClient();
        $aop->gatewayUrl = $this->gateway_url;
        $aop->appId = $this->app_id;
     ...
     * @return type
     */
    function aopclientRequestExecuteAppCert($request) {
        $aop = new AopCertClient();
        $aop->gatewayUrl = $this->gateway_url;
        $aop->appId = $this->app_id;
        $aop->rsaPrivateKey = $this->private_key;
    ...
    function aopclientRequestExecutes($request) {
        $aop = new AopClient ();
        $config = new Config();
        $ali_config = $config->getConfigMaster(0, 'ALIPAY', 0, 1);
        $aop->gatewayUrl = $this->gateway_url;
    ...
    /**
     * alipay.fund.trans.uni.transfer(单笔转账到支付宝账户接口(新))
     * @param $builder 业务参数，使用buildmodel中的对象生成。
     * @return $response 支付宝返回的信息
     */
    function TransferCert($builder) {
        $biz_content = $builder->getBizContent();
        //打印业务参数
        $this->writeLog($biz_content);
        $request = new AlipayFundTransUniTransferRequest();
        $request->setBizContent($biz_content);
        $response = $this->aopclientRequestExecuteCert($request);
...
// 交易异常，或发生系统错误
    ...
    function check($arr, $website_id) {
        $aop = new AopClient();
        $config = new Config();
        $alipay_config = $config->getConfig(0, 'ALIPAY', $website_id);
    ...
    function checks($arr) {
        $aop = new AopClient();
        $config = new Config();
        $alipay = $config->getConfigMaster(0, 'ALIPAY', 0);
