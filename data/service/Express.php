...
class Express extends BaseService
{
    ...
    {
        if(is_array($condition) && !array_key_exists('supplier_id',$condition)){
    ...
    public function addShippingFeeNew(array $data)
    {
        ...
        try {
            $data = array(
                ...
                'supplier_id' => $data['supplier_id'] ?:0,//供应商
                'create_time' => time()
            );
    ...
    {
        if(is_array($where) && !array_key_exists('supplier_id',$where)){
    ...
    {
        $supplier_id = $this->getRequestModel() == 'supplier' ? $this->supplier_id : 0;
        ...
        $viewObj = $vsl_express_company->alias('nec')
            ->join('vsl_express_company_shop_relation nr', 'nec.co_id=nr.co_id AND nr.website_id=' . $website_id . ' AND nr.shop_id=' . $shop_id . ' AND nr.supplier_id=' . $supplier_id, 'LEFT')
