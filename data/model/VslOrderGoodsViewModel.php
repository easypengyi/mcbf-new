...
class VslOrderGoodsViewModel extends BaseModel {
    ...
    public function getOrderGoodsRankList($page_index, $page_size, $condition, $order){
        ...
        if($condition['no.supplier_id']){
            $condition1['supplier_id'] = $condition['no.supplier_id'];
        }
        $goodsSer = new \data\service\Goods();
        $list['data1'] = $goodsSer->getPageGoodsList(1, 0, $condition1, 'real_sales desc', '*');
        return $list;
    }
    /**
     * 获取列表
    ...
    public function getOrderGoodsRankQuery($page_index, $page_size, $condition, $order)
    {
        ...
        ->field('nog.goods_name, sum(nog.num) as sumCount,sum(nog.real_money) as sumMoney,nog.goods_id,sap.pic_cover_micro')
        ->group('nog.goods_id');
        $list = $this->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
        return $list;
