...
class VslUserLikesModel extends BaseModel {
    ...
    {
        $list1 = Db::name('vsl_thing_circle_collection')
        ->alias('tcc')
		...
		$count = count($list);
		$start=($page_index-1)*$page_size;//偏移量，当前页-1乘以每页显示条数
		$list = array_slice($newlist,$start,$page_size);
        if($list){
        ...
        return ['data'=>$list,'total_count' => $count,'page_count' => ceil($count/$page_size),];
    }
}