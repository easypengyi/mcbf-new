...
class VslMemberViewModel extends BaseModel {
    ...
    public function getViewQuery($page_index, $page_size, $condition, $order, $field = '')
    {
        ...
        $viewObj = $this->alias('nm')
                        ...
                        ->join('vsl_shop sp','nm.uid= sp.uid','left')
                        ->field($field);
     ...
     * @return \data\model\unknown
     */
    public function getDistributorViewList2($page_index, $page_size, $condition, $order){
        $queryList = $this->getDistributorViewQuery2($page_index, $page_size, $condition, $order);
        $queryCount = $this->getDistributorViewCount2($condition);
        $list = $this->setReturnList($queryList, $queryCount, $page_size);
        return $list;
    }
    public function getDistributorViewQuery2($page_index, $page_size, $condition, $order)
    {
        //设置查询视图
        $viewObj = $this->alias('nm')
            ->join('vsl_distrbutor_agent_level nml','nm.agent_distributor_level_id = nml.id','left')
            ->join('vsl_member_level ml','nm.member_level = ml.level_id','left')
            ->join('sys_user us','nm.uid = us.uid','left')
            ->field('nm.*, nml.level_name,ml.level_name as member_level_name,us.user_headimg,us.nick_name,us.user_name,us.real_name');
        $list = $this->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
    ...
    public function getDistributorViewCount2($condition)
    {
        $viewObj = $this->alias('nm')
            ->join('vsl_distrbutor_agent_level nml','nm.agent_distributor_level_id = nml.id','left')
            ->join('vsl_member_level ml','nm.member_level = ml.level_id','left')
