...
class VslOrderModel extends BaseModel {
    ...
    public function getViewList2($page_index, $page_size, $condition, $order, $field = ''){
        ...
        $queryList = $this->getViewQuery2($page_index, $page_size, $condition, $order, $field);
        $queryCount = $this->getViewCount2($condition);
        $list = $this->setReturnList($queryList, $queryCount, $page_size);
        return $list;
    }
    /**
     ...
     * @return \data\model\multitype:number
     */
    public function getViewQuery2($page_index, $page_size, $condition, $order, $field = '')
    {
        //设置查询视图
        if(empty($field)) {
            $viewObj = $this->alias('nm')
            ...
            $viewObj = $this->alias('nm')
                ...
                ->join('vsl_store_assistant sa','nm.assistant_id=sa.assistant_id','left')
                ->field($field);
        }
        $list = $this->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
        return $list;
     ...
     * @return \data\model\unknown
     */
    public function getViewCount2($condition)
    {
        $viewObj = $this->alias('nm')
            ->join('vsl_member su','nm.buyer_id= su.uid','left')
            ->field('nm.order_id');
     ...
     * 获取金额总和
     */
    public function getViewCount3($condition)
    {
        // $viewObj = $this->alias('nm')
            // ->join('vsl_member su','nm.buyer_id= su.uid','left')
            // ->field('sum(nm.pay_money) as all_pay_money');
        $viewObj = $this->alias('nm')
            ->field('sum(nm.pay_money) as all_pay_money')
            ->where($condition)
            ->select();
